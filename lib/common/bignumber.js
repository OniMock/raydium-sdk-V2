...
__export(bignumber_exports, {
  ...
  parseNumberInfo: () => parseNumberInfo,
  recursivelyDecimalToFraction: () => recursivelyDecimalToFraction,
  shakeFractionDecimal: () => shakeFractionDecimal,
  tenExponential: () => tenExponential,
  toBN: () => toBN,
  toFraction: () => toFraction2,
  toPercent: () => toPercent,
...
function validateAndParsePublicKey({
  ...
  if (publicKey instanceof import_web33.PublicKey) {
  ...
  if (typeof publicKey === "string") {
    if (publicKey === import_web33.PublicKey.default.toBase58())
      return import_web33.PublicKey.default;
    try {
      const key = new import_web33.PublicKey(publicKey);
      return key;
    } catch {
...
function notInnerObject(v) {
  return typeof v === "object" && v !== null && ![Token, TokenAmount, import_web310.PublicKey, Fraction, import_bn5.default, Price, Percent].some((o) => typeof o === "object" && v instanceof o);
}
...
var TokenAmount = class extends Fraction {
  ...
  constructor(token, amount, isRaw = true, name) {
    let parsedAmount = new import_bn7.default(0);
    const multiplier = BN_TEN.pow(new import_bn7.default(token.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new import_bn7.default(0);
      let fractionalAmount = new import_bn7.default(0);
...
var CurrencyAmount = class extends Fraction {
  ...
  constructor(currency, amount, isRaw = true, name) {
    let parsedAmount = new import_bn7.default(0);
    const multiplier = BN_TEN.pow(new import_bn7.default(currency.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new import_bn7.default(0);
...
function parseNumberInfo(n) {
  ...
  if (n instanceof import_bn8.default) {
...
function toBN(n, decimal = 0) {
  if (n instanceof import_bn8.default)
    return n;
  return new import_bn8.default(shakeFractionDecimal(toFraction2(n).mul(BN_TEN.pow(new import_bn8.default(String(decimal))))));
}
...
function toPercent(n, options) {
  ...
  return new Percent(new import_bn8.default(numerator), new import_bn8.default(denominator).mul(options?.alreadyDecimaled ? new import_bn8.default(100) : new import_bn8.default(1)));
}
function toTokenPrice(params) {
  const { token, numberPrice, decimalDone } = params;
  const usdCurrency = new Token({ mint: "", decimals: 6, symbol: "usd", name: "usd", skipMint: true });
  const { numerator, denominator } = parseNumberInfo(numberPrice);
  const parsedNumerator = decimalDone ? new import_bn8.default(numerator).mul(BN_TEN.pow(new import_bn8.default(token.decimals))) : numerator;
  const parsedDenominator = new import_bn8.default(denominator).mul(BN_TEN.pow(new import_bn8.default(usdCurrency.decimals)));
  return new Price({
