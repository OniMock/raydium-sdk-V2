...
function validateAndParsePublicKey({
  ...
  if (publicKey instanceof PublicKey3) {
  ...
  if (typeof publicKey === "string") {
    if (publicKey === PublicKey3.default.toBase58())
      return PublicKey3.default;
    try {
      const key = new PublicKey3(publicKey);
      return key;
    } catch {
...
function notInnerObject(v) {
  return typeof v === "object" && v !== null && ![Token, TokenAmount, PublicKey10, Fraction, BN5, Price, Percent].some((o) => typeof o === "object" && v instanceof o);
}
...
var TokenAmount = class extends Fraction {
  ...
  constructor(token, amount, isRaw = true, name) {
    let parsedAmount = new BN7(0);
    const multiplier = BN_TEN.pow(new BN7(token.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new BN7(0);
      let fractionalAmount = new BN7(0);
...
var CurrencyAmount = class extends Fraction {
  ...
  constructor(currency, amount, isRaw = true, name) {
    let parsedAmount = new BN7(0);
    const multiplier = BN_TEN.pow(new BN7(currency.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new BN7(0);
...
function toBN(n, decimal = 0) {
  if (n instanceof BN8)
    return n;
  return new BN8(shakeFractionDecimal(toFraction2(n).mul(BN_TEN.pow(new BN8(String(decimal))))));
}
...
function toPercent(n, options) {
  ...
  return new Percent(new BN8(numerator), new BN8(denominator).mul(options?.alreadyDecimaled ? new BN8(100) : new BN8(1)));
}
function toTokenPrice(params) {
  const { token, numberPrice, decimalDone } = params;
  const usdCurrency = new Token({ mint: "", decimals: 6, symbol: "usd", name: "usd", skipMint: true });
  const { numerator, denominator } = parseNumberInfo(numberPrice);
  const parsedNumerator = decimalDone ? new BN8(numerator).mul(BN_TEN.pow(new BN8(token.decimals))) : numerator;
  const parsedDenominator = new BN8(denominator).mul(BN_TEN.pow(new BN8(usdCurrency.decimals)));
  return new Price({
