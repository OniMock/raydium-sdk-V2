...
var TokenAmount = class extends Fraction {
  ...
  constructor(token, amount, isRaw = true, name) {
    let parsedAmount = new BN8(0);
    const multiplier = BN_TEN.pow(new BN8(token.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new BN8(0);
      let fractionalAmount = new BN8(0);
