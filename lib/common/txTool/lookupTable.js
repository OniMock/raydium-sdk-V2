...
var lookupTable_exports = {};
__export(lookupTable_exports, {
  LOOKUP_TABLE_CACHE: () => LOOKUP_TABLE_CACHE,
  getMultipleLookupTableInfo: () => getMultipleLookupTableInfo
});
module.exports = __toCommonJS(lookupTable_exports);
      ...
      return unsafeRes.result.value.map((accountInfo) => {
        if (accountInfo) {
          ...
          return {
            ...
            executable,
            lamports,
            owner: new import_web39.PublicKey(owner),
            rentEpoch
          };
        }
        return null;
...
async function getMultipleLookupTableInfo({
  ...
  const dataInfos = await getMultipleAccountsInfo(connection, [...new Set(address.map((i) => i.toString()))].map((i) => new import_web310.PublicKey(i)));
  const outDict = {};
  for (let i = 0; i < address.length; i++) {
    ...
    if (!info)
      continue;
    const lookupAccount = new import_web310.AddressLookupTableAccount({
      key,
      state: import_web310.AddressLookupTableAccount.deserialize(info.data)
    });
