...
module.exports = __toCommonJS(txUtils_exports);
...
function addComputeBudget(config) {
  ...
  if (config.microLamports) {
    ins.push(import_web3.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: config.microLamports }));
    insTypes.push(InstructionType.SetComputeUnitPrice);
  }
  if (config.units) {
    ins.push(import_web3.ComputeBudgetProgram.setComputeUnitLimit({ units: config.units }));
...
function checkV0TxSize({
  ...
  recentBlockhash = import_web3.Keypair.generate().publicKey.toString()
}) {
  const transactionMessage = new import_web3.TransactionMessage({
    payerKey: payer,
    recentBlockhash,
    instructions
  });
  ...
  try {
    const buildLength = Buffer.from(new import_web3.VersionedTransaction(messageV0).serialize()).toString("base64").length;
  ...
  transactions.forEach((transaction) => {
    if (transaction instanceof import_web3.Transaction) {
      if (!transaction.recentBlockhash)
        transaction.recentBlockhash = import_spl_token.TOKEN_PROGRAM_ID.toBase58();
      if (!transaction.feePayer)
        transaction.feePayer = import_web3.Keypair.generate().publicKey;
    }
...
function transformTxToBase64(tx) {
  ...
  if (tx instanceof import_web3.VersionedTransaction)
    serialized = toBuffer(serialized);
  return serialized.toString("base64");
}
