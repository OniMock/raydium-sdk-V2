...
function createLogger(moduleName) {
  let logger16 = get(moduleLoggers, moduleName);
  if (!logger16) {
    const logLevel = get(moduleLevels, moduleName);
    logger16 = new Logger({ name: moduleName, logLevel });
    set(moduleLoggers, moduleName, logger16);
  }
  return logger16;
}
function setLoggerLevel(moduleName, level) {
  set(moduleLevels, moduleName, level);
  const logger16 = get(moduleLoggers, moduleName);
  if (logger16)
    logger16.level = level;
}
...
var CurrencyAmount = class extends Fraction {
  currency;
  logger;
  constructor(currency, amount, isRaw = true, name) {
    let parsedAmount = new BN3(0);
    const multiplier = BN_TEN.pow(new BN3(currency.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new BN3(0);
...
function toFractionWithDecimals(value) {
  if (value instanceof Percent)
    return { fr: new Fraction(value.numerator, value.denominator) };
  if (value instanceof Price)
    return { fr: value.adjusted };
  if (value instanceof TokenAmount)
...
function parseNumberInfo(n) {
  ...
  const s = String(n);
  const [, sign2 = "", int = "", dec = ""] = s.replace(",", "").match(/(-?)(\d*)\.?(\d*)/) ?? [];
  const denominator = "1" + "0".repeat(dec.length);
  const numerator = sign2 + (int === "0" ? "" : int) + dec || "0";
  return { denominator, numerator, sign: sign2, int, dec };
}
...
function shakeFractionDecimal(n) {
  const [, sign2 = "", int = ""] = n.toFixed(2).match(/(-?)(\d*)\.?(\d*)/) ?? [];
  return `${sign2}${int}`;
}
function toBN(n, decimal = 0) {
  if (n instanceof BN6)
    return n;
  return new BN6(shakeFractionDecimal(toFraction2(n).mul(BN_TEN.pow(new BN6(String(decimal))))));
}
function toFraction2(value) {
  if (value instanceof Percent)
    return new Fraction(value.numerator, value.denominator);
  if (value instanceof Price)
    return value.adjusted;
  if (value instanceof TokenAmount)
    try {
      return toFraction2(value.toExact());
    } catch {
      return new Fraction(BN_ZERO);
    }
  if (value instanceof Fraction)
...
function ceilDiv(tokenAmount, feeNumerator, feeDenominator) {
  return tokenAmount.mul(feeNumerator).add(feeDenominator).sub(new BN6(1)).div(feeDenominator);
}
function floorDiv(tokenAmount, feeNumerator, feeDenominator) {
  return tokenAmount.mul(feeNumerator).div(feeDenominator);
}
function toPercent(n, options) {
  const { numerator, denominator } = parseNumberInfo(n);
  return new Percent(new BN6(numerator), new BN6(denominator).mul(options?.alreadyDecimaled ? new BN6(100) : new BN6(1)));
}
function toTokenPrice(params) {
  const { token, numberPrice, decimalDone } = params;
  const usdCurrency = new Token({ mint: "", decimals: 6, symbol: "usd", name: "usd", skipMint: true });
  const { numerator, denominator } = parseNumberInfo(numberPrice);
  const parsedNumerator = decimalDone ? new BN6(numerator).mul(BN_TEN.pow(new BN6(token.decimals))) : numerator;
  const parsedDenominator = new BN6(denominator).mul(BN_TEN.pow(new BN6(usdCurrency.decimals)));
  return new Price({
...
function toUsdCurrency(amount) {
  const usdCurrency = new Currency({ decimals: 6, symbol: "usd", name: "usd" });
  const amountBigNumber = toBN(mul2(amount, 10 ** usdCurrency.decimals));
  return new CurrencyAmount(usdCurrency, amountBigNumber);
}
function toTotalPrice(amount, price) {
  if (!price || !amount)
    return toUsdCurrency(0);
  return toUsdCurrency(mul2(amount, price));
}
...
function isDecimal2(val) {
  return val instanceof decimal_default;
}
function recursivelyDecimalToFraction(info) {
  return isDecimal2(info) ? decimalToFraction(info) : Array.isArray(info) ? info.map((k) => recursivelyDecimalToFraction(k)) : notInnerObject(info) ? Object.fromEntries(Object.entries(info).map(([k, v]) => [k, recursivelyDecimalToFraction(v)])) : info;
}
...
function offsetDateTime(baseDate, offset2) {
  const timestamp = getTime(baseDate);
  const offsetedTimestamp = timestamp + (offset2.days ? offset2.days * 24 * 60 * 60 * 1e3 : 0) + (offset2.hours ? offset2.hours * 60 * 60 * 1e3 : 0) + (offset2.minutes ? offset2.minutes * 60 * 1e3 : 0) + (offset2.seconds ? offset2.seconds * 1e3 : 0) + (offset2.milliseconds ? offset2.milliseconds : 0);
...
var TxVersion = /* @__PURE__ */ ((TxVersion3) => {
  TxVersion3[TxVersion3["V0"] = 0] = "V0";
  TxVersion3[TxVersion3["LEGACY"] = 1] = "LEGACY";
  return TxVersion3;
})(TxVersion || {});
var InstructionType = {
...
async function simulateMultipleInstruction(connection, instructions, keyword, batchRequest = true) {
  const feePayer = new PublicKey6("RaydiumSimuLateTransaction11111111111111111");
  const transactions = [];
...
// src/api/type.ts
var JupTokenType = /* @__PURE__ */ ((JupTokenType3) => {
  JupTokenType3["ALL"] = "all";
  JupTokenType3["Strict"] = "strict";
  return JupTokenType3;
})(JupTokenType || {});
...
// src/api/api.ts
import { TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID6 } from "@solana/spl-token";
var logger6 = createLogger("Raydium_Api");
var poolKeysCache = /* @__PURE__ */ new Map();
async function endlessRetry(name, call, interval = 1e3) {
  let result;
  while (result == void 0) {
    try {
      logger6.debug(`Request ${name} through endlessRetry`);
      result = await call();
    } catch (err) {
      logger6.error(`Request ${name} failed, retry after ${interval} ms`, err);
...
var Api = class {
  ...
  constructor({ cluster, timeout, logRequests, logCount, urlConfigs }) {
    ...
    this.api = axios2.create({ baseURL: this.urlConfigs.BASE_HOST || API_URLS.BASE_HOST, timeout });
  ...
  async getBlockSlotCountForSecond(endpointUrl) {
    if (!endpointUrl)
      return 2;
    const res = await axios2.post(endpointUrl, {
  ...
  async getJupTokenList() {
    ...
    return r.map((t) => ({
      ...t,
      chainId: 101,
      programId: t.tags.includes("token-2022") ? TOKEN_2022_PROGRAM_ID.toBase58() : TOKEN_PROGRAM_ID6.toBase58()
    }));
  }
...
import { PublicKey as PublicKey14, SystemProgram as SystemProgram4 } from "@solana/web3.js";
import {
  AccountLayout,
  createAssociatedTokenAccountInstruction,
  TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID9,
...
import { PublicKey as PublicKey13, SystemProgram as SystemProgram3 } from "@solana/web3.js";
import BN11 from "bn.js";
import {
  createCloseAccountInstruction,
  createInitializeAccountInstruction,
  createTransferInstruction,
  TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID8
} from "@solana/spl-token";

// src/raydium/account/util.ts
import { Keypair as Keypair2, PublicKey as PublicKey12 } from "@solana/web3.js";
...
function parseTokenAccountResp({ owner, solAccountResp, tokenAccountResp }) {
  ...
  if (solAccountResp) {
    tokenAccounts.push({
      mint: PublicKey12.default,
      amount: new BN10(String(solAccountResp.lamports)),
      isNative: true,
...
function generatePubKey({
  fromPublicKey,
  programId = TOKEN_PROGRAM_ID7,
  assignSeed
}) {
  const seed = assignSeed ? btoa(assignSeed).slice(0, 32) : Keypair2.generate().publicKey.toBase58().slice(0, 32);
...
function createWithSeed(fromPublicKey, seed, programId) {
  ...
  return new PublicKey12(publicKeyBytes);
}

// src/raydium/account/instruction.ts
function initTokenAccountInstruction(params) {
  const { mint, tokenAccount, owner, programId = TOKEN_PROGRAM_ID8 } = params;
  return createInitializeAccountInstruction(tokenAccount, mint, owner, programId);
}
function closeAccountInstruction(params) {
  const { tokenAccount, payer, multiSigners = [], owner, programId = TOKEN_PROGRAM_ID8 } = params;
...
async function createWSolAccountInstructions(params) {
  ...
  const lamports = parseBigNumberish(amount).add(new BN11(balanceNeeded));
  const newAccount = generatePubKey({ fromPublicKey: payer, programId: TOKEN_PROGRAM_ID8 });
  return {
    addresses: { newAccount: newAccount.publicKey },
    signers: [],
    instructions: [
      SystemProgram3.createAccountWithSeed({
        fromPubkey: payer,
        basePubkey: payer,
        ...
        programId: TOKEN_PROGRAM_ID8
      }),
      initTokenAccountInstruction({
        mint: new PublicKey13(TOKEN_WSOL.address),
...
var Account = class extends ModuleBase {
  ...
  async fetchWalletTokenAccounts(config2) {
    ...
    const [solAccountResp, ownerTokenAccountResp, ownerToken2022AccountResp] = await Promise.all([
      ...
      this.scope.connection.getTokenAccountsByOwner(this.scope.ownerPubKey, { programId: TOKEN_PROGRAM_ID9 }, customConfig.commitment),
      this.scope.connection.getTokenAccountsByOwner(this.scope.ownerPubKey, { programId: TOKEN_2022_PROGRAM_ID2 }, customConfig.commitment)
    ]);
  ...
  async getOrCreateTokenAccount(params) {
    ...
    const tokenProgram = new PublicKey14(params.tokenProgram || TOKEN_PROGRAM_ID9);
    const ata = this.getAssociatedTokenAccount(mint, new PublicKey14(tokenProgram));
    ...
    if (associatedOnly) {
      ...
      if (checkCreateATAOwner) {
        ...
        } else if (ataInfo.owner.equals(tokenProgram) && AccountLayout.decode(ataInfo.data).mint.equals(mint) && AccountLayout.decode(ataInfo.data).owner.equals(owner)) {
        } else {
      ...
      const createAccountIns = SystemProgram4.createAccountWithSeed({
        fromPubkey: owner,
        basePubkey: owner,
  ...
  async handleTokenAccount(params) {
    ...
    const ata = this.getAssociatedTokenAccount(mint, programId);
    if (new PublicKey14(WSOLMint).equals(mint)) {
      const txInstruction = await createWSolAccountInstructions({
        connection: this.scope.connection,
        owner: this.scope.ownerPubKey,
        payer,
      ...
      const instructions = [];
      const _createATAIns = createAssociatedTokenAccountInstruction(this.scope.ownerPubKey, ata, this.scope.ownerPubKey, mint, programId);
      if (checkCreateATAOwner) {
        const ataInfo = await this.scope.connection.getAccountInfo(ata);
        if (ataInfo === null) {
          instructions.push(_createATAIns);
        } else if (ataInfo.owner.equals(TOKEN_PROGRAM_ID9) && AccountLayout.decode(ataInfo.data).mint.equals(mint) && AccountLayout.decode(ataInfo.data).owner.equals(this.scope.ownerPubKey)) {
        } else {
          throw Error(`create ata check error -> mint: ${mint.toString()}, ata: ${ata.toString()}`);
        }
      } else {
  ...
  async processTokenAccount(props) {
    const { mint, programId = TOKEN_PROGRAM_ID9, amount, useSOLBalance, handleTokenAccount, feePayer } = props;
    let tokenAccount;
    const txBuilder = this.createTxBuilder(feePayer);
    if (mint.equals(new PublicKey14(WSOLMint)) && useSOLBalance) {
...
import { PublicKey as PublicKey17, SystemProgram as SystemProgram6 } from "@solana/web3.js";
import { createAssociatedTokenAccountInstruction as createAssociatedTokenAccountInstruction3 } from "@solana/spl-token";
...
var FARM_LOCK_MINT = new PublicKey15("4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R");
var FARM_LOCK_VAULT = new PublicKey15("FrspKwj8i3pNmKwXreTveC4fu7KL5ZbGeXdZBe2XViu1");
...
import {
  ...
  TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID10
} from "@solana/spl-token";
...
function updateFarmPoolInfo(poolInfo, lpVault, slot, chainTime) {
  if (poolInfo.version === 3 || poolInfo.version === 5) {
    if (poolInfo.lastSlot.gte(new BN12(slot)))
      return poolInfo;
    const spread = new BN12(slot).sub(poolInfo.lastSlot);
    poolInfo.lastSlot = new BN12(slot);
    for (const itemRewardInfo of poolInfo.rewardInfos) {
      if (lpVault.amount.eq(new BN12(0)))
        continue;
      const reward = itemRewardInfo.perSlotReward.mul(spread);
      itemRewardInfo.perShareReward = itemRewardInfo.perShareReward.add(reward.mul(new BN12(10).pow(new BN12(poolInfo.version === 3 ? 9 : 15))).div(lpVault.amount));
      itemRewardInfo.totalReward = itemRewardInfo.totalReward.add(reward);
    }
  } else if (poolInfo.version === 6) {
    for (const itemRewardInfo of poolInfo.rewardInfos) {
      if (itemRewardInfo.rewardState.eq(new BN12(0)))
        continue;
      const updateTime = BN12.min(new BN12(chainTime), itemRewardInfo.rewardEndTime);
...
async function fetchMultipleFarmInfoAndUpdate({
  ...
  for (const [poolId, { state, ledger }] of Object.entries(poolsInfo)) {
    if (ledger) {
      const multiplier = state.version === 6 ? state.rewardMultiplier : state.rewardInfos.length === 1 ? tenBN.pow(new BN12(9)) : tenBN.pow(new BN12(15));
...
function createAssociatedLedgerAccountInstruction(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: SystemProgram5.programId, isWritable: false }),
...
function makeCreateFarmInstruction(params) {
  ...
  return {
    instruction: new TransactionInstruction5({ programId: params.programId, keys, data }),
...
function voterStakeRegistryDeposit(programId, registrar, voter, voterVault, depositToken, depositAuthority, userStakerInfoV2, pool, votingMint, votingMintAuthority, stakeProgramId, depositEntryIndex, amount) {
  ...
  const keys = [
    ...
    { pubkey: TOKEN_PROGRAM_ID10, isSigner: false, isWritable: false },
...
async function makeDepositTokenInstruction({
  ...
  if (depositTokenAccountInfo === null) {
    instructions.push(createAssociatedTokenAccountInstruction2(owner, depositToken, owner, votingMint));
  }
...
async function makeWithdrawTokenInstruction({
  ...
  const ownerInfo = farmLedgerLayoutV3_2.decode(ownerAccountInfo.data);
  if (ownerInfo.voteLockedBalance.eq(new BN13(0))) {
...
function makeRestartRewardInstruction({
  ...
  return new TransactionInstruction5({ programId: farmKeys.programId, keys, data });
}
...
function makeDepositWithdrawInstruction(params) {
  ...
  const [programId, id] = [new PublicKey16(farmInfo.programId), new PublicKey16(farmInfo.id)];
  ...
  const keys = version === 6 ? [
    accountMeta({ pubkey: TOKEN_PROGRAM_ID10, isWritable: false }),
    ...deposit ? [accountMeta({ pubkey: SystemProgram5.programId, isWritable: false })] : [],
    accountMeta({ pubkey: id }),
    accountMeta({ pubkey: new PublicKey16(farmKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new PublicKey16(farmKeys.lpVault) }),
    ...
    accountMeta({ pubkey: new PublicKey16(farmKeys.rewardInfos[0].vault) }),
  ...
  if (version === 5) {
    for (let index = 1; index < farmKeys.rewardInfos.length; index++) {
      ...
      keys.push(accountMeta({ pubkey: new PublicKey16(farmKeys.rewardInfos[index].vault) }));
    }
  }
  if (version === 6) {
  ...
  return new TransactionInstruction5({ programId, keys, data });
}
...
function makeWithdrawInstructionV4(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: new PublicKey16(farmKeys.rewardInfos[1].vault) })
  ];
  const data = Buffer.alloc(dataLayout.span);
  dataLayout.encode({
    instruction: 2,
...
var Farm = class extends ModuleBase {
  ...
  async create({
    ...
    this.checkDisabled();
    this.scope.checkOwner();
    const lpMint = new PublicKey17(propPoolInfo.lpMint.address);
    const poolInfo = {
      lpMint,
    ...
    const { publicKey: authority, nonce } = getAssociatedAuthority({
      programId: new PublicKey17(poolInfo.programId),
    ...
    for (const rewardInfo of poolInfo.rewardInfos) {
      ...
      const rewardMint = rewardInfo.mint.equals(SOLMint) ? new PublicKey17(TOKEN_WSOL.address) : rewardInfo.mint;
      rewardInfoKey.push({
        rewardMint,
    ...
    const { account: lockUserAccount, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
      mint: new PublicKey17(poolInfo.lockInfo.lockMint),
      owner: this.scope.ownerPubKey,
      skipCloseAccount: false,
      createInfo: {
        payer: this.scope.ownerPubKey,
  ...
  async restartReward({
    ...
    const payerPubKey = payer || this.scope.ownerPubKey;
    const rewardMint = newRewardInfo.mint.equals(SOLMint) ? new PublicKey17(TOKEN_WSOL.address) : newRewardInfo.mint;
    const rewardInfoIndex = farmKeys.rewardInfos.findIndex((item) => new PublicKey17(item.mint.address).equals(rewardMint));
  ...
  async restartRewards({
    ...
    for (const itemReward of newRewardInfos) {
      const rewardMint = itemReward.mint.equals(SOLMint) ? new PublicKey17(TOKEN_WSOL.address) : itemReward.mint;
      const rewardInfoIndex = farmKeys.rewardInfos.findIndex((item) => new PublicKey17(item.mint.address).equals(rewardMint));
      const rewardInfo = farmInfoKeys.rewardInfos[rewardInfoIndex];
      if (!rewardInfo)
        this.logAndCreateError("configuration does not exist", "rewardMint", rewardMint);
  ...
  async addNewRewardToken(params) {
    ...
    const rewardVault = getAssociatedLedgerPoolAccount({
      programId: new PublicKey17(farmInfo.programId),
      poolId: new PublicKey17(farmInfo.id),
      mint: rewardMint,
      type: "rewardVault"
    });
    const { rewardPubKey: userRewardTokenPub, newInstruction } = await this._getUserRewardInfo({
  ...
  async deposit(params) {
    ...
    const [farmProgramId, farmId] = [new PublicKey17(farmInfo.programId), new PublicKey17(farmInfo.id)];
    ...
    for (const itemReward of rewardInfos) {
      ...
      if (!ownerRewardAccount) {
        const { account: _ownerRewardAccount, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
          ...
          mint: new PublicKey17(itemReward.mint.address),
    ...
    const insParams = {
      ...
      rewardAccounts,
      userAuxiliaryLedgers: userAuxiliaryLedgers?.map((key) => new PublicKey17(key))
    };
  ...
  async withdraw(params) {
    ...
    if (version !== 4) {
      ...
      if (!ledgerData) {
        if (version !== 6) {
          const { instruction, instructionType } = createAssociatedLedgerAccountInstruction({
            id: new PublicKey17(farmKeys.id),
            programId: new PublicKey17(farmKeys.programId),
            version,
            ledger,
            owner: this.scope.ownerPubKey
          });
  ...
  async withdrawFarmReward({
    ...
    const txBuilder = this.createTxBuilder(feePayer);
    let userRewardToken;
    if (withdrawMint.equals(SOLMint) || withdrawMint.equals(PublicKey17.default)) {
      const txInstruction = await createWSolAccountInstructions({
        connection: this.scope.connection,
        owner: this.scope.ownerPubKey,
        payer: this.scope.ownerPubKey,
      ...
      if (selectUserRewardToken === null) {
        ...
        txBuilder.addInstruction({
          instructions: [
            createAssociatedTokenAccountInstruction3(this.scope.ownerPubKey, userRewardToken, this.scope.ownerPubKey, withdrawMint)
          ],
          instructionTypes: [InstructionType.CreateATA]
        });
      } else {
...
import { PublicKey as PublicKey27 } from "@solana/web3.js";
import { AccountLayout as AccountLayout2, NATIVE_MINT, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID17 } from "@solana/spl-token";

// src/raydium/clmm/instrument.ts
import { Keypair as Keypair3, PublicKey as PublicKey19, SystemProgram as SystemProgram7, TransactionInstruction as TransactionInstruction6 } from "@solana/web3.js";
import BN19 from "bn.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID2, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID4, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID11 } from "@solana/spl-token";
...
var ZERO = new BN14(0);
var ONE = new BN14(1);
var NEGATIVE_ONE = new BN14(-1);
var Q64 = new BN14(1).shln(64);
var Q128 = new BN14(1).shln(128);
var MaxU64 = Q64.sub(ONE);
var U64Resolution = 64;
var MaxUint128 = Q128.subn(1);
var MIN_TICK = -443636;
var MAX_TICK = -MIN_TICK;
var MIN_SQRT_PRICE_X64 = new BN14("4295048016");
var MAX_SQRT_PRICE_X64 = new BN14("79226673521066979257578248091");
var MIN_SQRT_PRICE_X64_ADD_ONE = new BN14("4295048017");
var MAX_SQRT_PRICE_X64_SUB_ONE = new BN14("79226673521066979257578248090");
...
var FEE_RATE_DENOMINATOR = new BN14(10).pow(new BN14(6));
...
var U64_IGNORE_RANGE = new BN14("18446744073700000000");
...
var PositionUtils = class {
  static getfeeGrowthInside(poolState, tickLowerState, tickUpperState) {
    let feeGrowthBelowX64A = new BN15(0);
    let feeGrowthBelowX64B = new BN15(0);
    ...
    let feeGrowthAboveX64A = new BN15(0);
    let feeGrowthAboveX64B = new BN15(0);
  ...
  static getRewardGrowthInside(tickCurrentIndex, tickLowerState, tickUpperState, rewardInfos) {
    ...
    for (let i = 0; i < rewardInfos.length; i++) {
      let rewardGrowthsBelow = new BN15(0);
      ...
      let rewardGrowthsAbove = new BN15(0);
  ...
  static getAmountsFromLiquidity({
    ...
    const amounts = LiquidityMath.getAmountsFromLiquidity(sqrtPriceX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, add3);
    ...
    const [amountSlippageA, amountSlippageB] = [
      getTransferAmountFeeV2(new BN15(new decimal_default(amounts.amountA.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintA.extensions?.feeConfig, epochInfo, true),
      getTransferAmountFeeV2(new BN15(new decimal_default(amounts.amountB.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintB.extensions?.feeConfig, epochInfo, true)
    ];
    return {
      liquidity,
      amountA,
...
var PoolUtils = class {
  ...
  static async updatePoolRewardInfos({
    ...
    for (let i = 0; i < rewardInfos.length; i++) {
      ...
      const itemReward = {
        ...
        tokenProgramId: new PublicKey18(apiRewardProgram)
      };
      if (itemReward.tokenMint.equals(PublicKey18.default))
        continue;
      if (chainTime <= itemReward.openTime.toNumber() || poolLiquidity.eq(ZERO)) {
        nRewardInfo.push(itemReward);
        continue;
      }
      const latestUpdateTime = new BN16(Math.min(itemReward.endTime.toNumber(), chainTime));
        ...
        poolInfoA.positionAccount = [
          ...poolInfoA.positionAccount ?? [],
          {
            ...
            rewardInfos: position.rewardInfos.map((i) => ({
              ...i,
              pendingReward: new BN16(0)
            })),
            leverage,
            tokenFeeAmountA: new BN16(0),
            tokenFeeAmountB: new BN16(0)
          }
        ];
      ...
      if (updateOwnerRewardAndFee) {
        ...
        for (const { state, positionAccount } of pools) {
          ...
          for (const itemPA of positionAccount) {
            ...
            itemPA.tokenFeeAmountA = tokenFeeAmountA.gte(new BN16(0)) ? tokenFeeAmountA : new BN16(0);
            itemPA.tokenFeeAmountB = tokenFeeAmountB.gte(new BN16(0)) ? tokenFeeAmountB : new BN16(0);
            for (let i = 0; i < rewardInfos.length; i++) {
              itemPA.rewardInfos[i].pendingReward = rewardInfos[i].gte(new BN16(0)) ? rewardInfos[i] : new BN16(0);
            }
          }
        }
      }
  ...
  static computeAmountOut({
    ...
    if (priceLimit.equals(new decimal_default(0))) {
      sqrtPriceLimitX64 = isBaseIn ? MIN_SQRT_PRICE_X64.add(new BN16(1)) : MAX_SQRT_PRICE_X64.sub(new BN16(1));
    } else {
    ...
    const _minAmountOut = _expectedAmountOut.mul(new BN16(Math.floor((1 - slippage) * 1e10))).div(new BN16(1e10));
  ...
  static computeAmountOutFormat({
    ...
    const [baseToken, outToken] = [
      new Token({
        ...
        isToken2022: inputMint.programId === TOKEN_2022_PROGRAM_ID3.toBase58()
      }),
      new Token({
        ...outMint,
        mint: outMint.address,
        isToken2022: outMint.programId === TOKEN_2022_PROGRAM_ID3.toBase58()
      })
    ];
    const {
      allTrade,
      ...
      poolInfo,
      tickArrayCache,
      baseMint: new PublicKey18(inputMint.address),
      amountIn,
      slippage,
      epochInfo,
    ...
    const _currentPrice = new Price({
      baseToken,
      denominator: new BN16(10).pow(new BN16(20 + baseToken.decimals)),
      quoteToken: outToken,
  ...
  static computeAmountIn({
    ...
    const _executionPrice = SqrtPriceMath.sqrtPriceX64ToPrice(_executionPriceX64, poolInfo.mintA.decimals, poolInfo.mintB.decimals);
    const executionPrice = isBaseIn ? _executionPrice : new decimal_default(1).div(_executionPrice);
    const _maxAmountIn = _expectedAmountIn.mul(new BN16(Math.floor((1 + slippage) * 1e10))).div(new BN16(1e10));
  ...
  static async getLiquidityAmountOutFromAmountIn({
    ...
    const _amount = new BN16(new decimal_default(addFeeAmount.amount.sub(addFeeAmount.fee ?? ZERO).toString()).toFixed(0));
    let liquidity;
    if (sqrtPriceX64.lte(sqrtPriceX64A)) {
      liquidity = inputA ? LiquidityMath.getLiquidityFromTokenAmountA(sqrtPriceX64A, sqrtPriceX64B, _amount, !add3) : new BN16(0);
    } else if (sqrtPriceX64.lte(sqrtPriceX64B)) {
      const liquidity0 = LiquidityMath.getLiquidityFromTokenAmountA(sqrtPriceX64, sqrtPriceX64B, _amount, !add3);
      ...
      liquidity = inputA ? new BN16(0) : LiquidityMath.getLiquidityFromTokenAmountB(sqrtPriceX64A, sqrtPriceX64B, _amount);
    }
  ...
  static async getAmountsFromLiquidity({
    ...
    const sqrtPriceX64B = SqrtPriceMath.getSqrtPriceX64FromTick(tickUpper);
    const coefficientRe = add3 ? 1 + slippage : 1 - slippage;
    const amounts = LiquidityMath.getAmountsFromLiquidity(SqrtPriceMath.priceToSqrtPriceX64(new decimal_default(poolInfo.price), poolInfo.mintA.decimals, poolInfo.mintB.decimals), sqrtPriceX64A, sqrtPriceX64B, liquidity, add3);
    const [amountA, amountB] = [
      getTransferAmountFeeV2(amounts.amountA, poolInfo.mintA.extensions?.feeConfig, epochInfo, true),
      getTransferAmountFeeV2(amounts.amountB, poolInfo.mintB.extensions?.feeConfig, epochInfo, true)
    ];
  ...
  static async fetchComputeMultipleClmmInfo({
    ...
    const fetchRpcList = poolList.filter((p) => !rpcDataMap[p.id]).map((p) => new PublicKey18(p.id));
    ...
    const pdaList = poolList.map((poolInfo) => getPdaExBitmapAccount(new PublicKey18(poolInfo.programId), new PublicKey18(poolInfo.id)).publicKey);
    ...
    return poolList.reduce((acc, cur) => ({
      ...
      [cur.id]: {
        ...rpcDataMap[cur.id],
        id: new PublicKey18(cur.id),
        version: 6,
        programId: new PublicKey18(cur.programId),
        mintA: cur.mintA,
        mintB: cur.mintB,
        ammConfig: {
          ...cur.config,
          id: new PublicKey18(cur.config.id),
          fundOwner: ""
        },
        currentPrice: new decimal_default(cur.price),
        exBitmapAccount: getPdaExBitmapAccount(new PublicKey18(cur.programId), new PublicKey18(cur.id)).publicKey,
        exBitmapInfo: exBitData[getPdaExBitmapAccount(new PublicKey18(cur.programId), new PublicKey18(cur.id)).publicKey.toBase58()],
...
var MathUtil = class {
  ...
  static decimalToX64(num) {
    return new BN17(num.mul(decimal_default.pow(2, 64)).floor().toFixed());
  }
...
var SqrtPriceMath = class {
  ...
  static getSqrtPriceX64FromTick(tick) {
    ...
    let ratio = (tickAbs & 1) != 0 ? new BN17("18445821805675395072") : new BN17("18446744073709551616");
    if ((tickAbs & 2) != 0)
      ratio = mulRightShift(ratio, new BN17("18444899583751176192"));
    if ((tickAbs & 4) != 0)
      ratio = mulRightShift(ratio, new BN17("18443055278223355904"));
    if ((tickAbs & 8) != 0)
      ratio = mulRightShift(ratio, new BN17("18439367220385607680"));
    if ((tickAbs & 16) != 0)
      ratio = mulRightShift(ratio, new BN17("18431993317065453568"));
    if ((tickAbs & 32) != 0)
      ratio = mulRightShift(ratio, new BN17("18417254355718170624"));
    if ((tickAbs & 64) != 0)
      ratio = mulRightShift(ratio, new BN17("18387811781193609216"));
    if ((tickAbs & 128) != 0)
      ratio = mulRightShift(ratio, new BN17("18329067761203558400"));
    if ((tickAbs & 256) != 0)
      ratio = mulRightShift(ratio, new BN17("18212142134806163456"));
    if ((tickAbs & 512) != 0)
      ratio = mulRightShift(ratio, new BN17("17980523815641700352"));
    if ((tickAbs & 1024) != 0)
      ratio = mulRightShift(ratio, new BN17("17526086738831433728"));
    if ((tickAbs & 2048) != 0)
      ratio = mulRightShift(ratio, new BN17("16651378430235570176"));
    if ((tickAbs & 4096) != 0)
      ratio = mulRightShift(ratio, new BN17("15030750278694412288"));
    if ((tickAbs & 8192) != 0)
      ratio = mulRightShift(ratio, new BN17("12247334978884435968"));
    if ((tickAbs & 16384) != 0)
      ratio = mulRightShift(ratio, new BN17("8131365268886854656"));
    if ((tickAbs & 32768) != 0)
      ratio = mulRightShift(ratio, new BN17("3584323654725218816"));
    if ((tickAbs & 65536) != 0)
      ratio = mulRightShift(ratio, new BN17("696457651848324352"));
    if ((tickAbs & 131072) != 0)
      ratio = mulRightShift(ratio, new BN17("26294789957507116"));
    if ((tickAbs & 262144) != 0)
      ratio = mulRightShift(ratio, new BN17("37481735321082"));
    if (tick > 0)
  ...
  static getTickFromSqrtPriceX64(sqrtPriceX64) {
    ...
    const adjustedMsb = new BN17(msb - 64);
    const log2pIntegerX32 = signedLeftShift(adjustedMsb, 32, 128);
    let bit = new BN17("8000000000000000", "hex");
    let precision = 0;
    let log2pFractionX64 = new BN17(0);
    let r = msb >= 64 ? sqrtPriceX64.shrn(msb - 63) : sqrtPriceX64.shln(63 - msb);
    while (bit.gt(new BN17(0)) && precision < BIT_PRECISION) {
      r = r.mul(r);
    ...
    const logbpX64 = log2pX32.mul(new BN17(LOG_B_2_X32));
    const tickLow = signedRightShift(logbpX64.sub(new BN17(LOG_B_P_ERR_MARGIN_LOWER_X64)), 64, 128).toNumber();
    const tickHigh = signedRightShift(logbpX64.add(new BN17(LOG_B_P_ERR_MARGIN_UPPER_X64)), 64, 128).toNumber();
    if (tickLow == tickHigh) {
      return tickLow;
    } else {
...
var LiquidityMath = class {
  ...
  static getAmountsFromLiquidityWithSlippage(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, amountMax, roundUp, amountSlippage) {
    ...
    const amount0Slippage = new BN17(new decimal_default(amountA.toString()).mul(coefficient).toFixed(0));
    const amount1Slippage = new BN17(new decimal_default(amountB.toString()).mul(coefficient).toFixed(0));
    return {
  ...
  static getAmountsOutFromLiquidity({
    ...
    const [amountSlippageA, amountSlippageB] = [
      getTransferAmountFeeV2(new BN17(new decimal_default(amounts.amountA.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintA.extensions?.feeConfig, epochInfo, amountAddFee),
      getTransferAmountFeeV2(new BN17(new decimal_default(amounts.amountB.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintB.extensions?.feeConfig, epochInfo, amountAddFee)
    ];
    return {
      liquidity,
      amountA,
...
var SwapMath = class {
  ...
  static swapStepCompute(sqrtPriceX64Current, sqrtPriceX64Target, liquidity, amountRemaining, feeRate, zeroForOne) {
    const swapStep = {
      sqrtPriceX64Next: new BN17(0),
      amountIn: new BN17(0),
      amountOut: new BN17(0),
      feeAmount: new BN17(0)
    };
    const baseInput = amountRemaining.gte(ZERO);
    if (baseInput) {
      const amountRemainingSubtractFee = MathUtil.mulDivFloor(amountRemaining, FEE_RATE_DENOMINATOR.sub(new BN17(feeRate.toString())), FEE_RATE_DENOMINATOR);
    ...
    if (baseInput && !swapStep.sqrtPriceX64Next.eq(sqrtPriceX64Target)) {
      ...
      swapStep.feeAmount = MathUtil.mulDivCeil(swapStep.amountIn, new BN17(feeRate), FEE_RATE_DENOMINATOR.sub(new BN17(feeRate)));
    }
...
var ClmmInstrument = class {
  ...
  static async createPoolInstructions(props) {
    ...
    const [mintAAddress, mintBAddress] = [new PublicKey19(mintA.address), new PublicKey19(mintB.address)];
    ...
    const ins = [
      this.createPoolInstruction(programId, poolId, owner, ammConfigId, observationId, mintAAddress, mintAVault, new PublicKey19(mintA.programId || TOKEN_PROGRAM_ID11), mintBAddress, mintBVault, new PublicKey19(mintB.programId || TOKEN_PROGRAM_ID11), exBitmapAccount, initialPriceX64, extendMintAccount)
    ];
    return {
      signers: [],
      instructions: ins,
  ...
  static openPositionFromLiquidityInstruction(programId, payer, poolId, positionNftOwner, positionNftMint, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerTokenAccountA, ownerTokenAccountB, tokenVaultA, tokenVaultB, tokenMintA, tokenMintB, tickLowerIndex, tickUpperIndex, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, exTickArrayBitmap) {
    ...
    const keys = [
      ...
      { pubkey: RENT_PROGRAM_ID, isSigner: false, isWritable: false },
      { pubkey: SystemProgram7.programId, isSigner: false, isWritable: false },
      { pubkey: TOKEN_PROGRAM_ID11, isSigner: false, isWritable: false },
      { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID2, isSigner: false, isWritable: false },
      { pubkey: METADATA_PROGRAM_ID, isSigner: false, isWritable: false },
      { pubkey: TOKEN_2022_PROGRAM_ID4, isSigner: false, isWritable: false },
  ...
  static async openPositionInstructions({
    ...
    const signers = [];
    const [programId, id] = [new PublicKey19(poolInfo.programId), new PublicKey19(poolInfo.id)];
    let nftMintAccount;
    if (getEphemeralSigners) {
      nftMintAccount = new PublicKey19((await getEphemeralSigners(1))[0]);
    } else {
      const _k = Keypair3.generate();
      signers.push(_k);
    ...
    const { publicKey: positionNftAccount } = nft2022 ? getATAAddress(ownerInfo.wallet, nftMintAccount, TOKEN_2022_PROGRAM_ID4) : getATAAddress(ownerInfo.wallet, nftMintAccount, TOKEN_PROGRAM_ID11);
    ...
    const ins = nft2022 ? this.openPositionFromLiquidityInstruction22(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0) : this.openPositionFromLiquidityInstruction(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
  ...
  static async openPositionFromBaseInstructions({
    ...
    const { publicKey: personalPosition } = getPdaPersonalPositionAddress(programId, nftMintAccount);
    const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(programId, id, tickLower, tickUpper);
    const ins = nft2022 ? this.openPositionFromBaseInstruction22(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, withMetadata, base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0) : this.openPositionFromBaseInstruction(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, withMetadata, base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0);
    return {
  ...
  static async openPositionFromLiquidityInstructions({
    ...
    const { publicKey: personalPosition } = getPdaPersonalPositionAddress(programId, nftMintAccount);
    const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(programId, id, tickLower, tickUpper);
    const ins = nft2022 ? this.openPositionFromLiquidityInstruction22(programId, ownerInfo.wallet, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolKeys.mintA.address), new PublicKey19(poolKeys.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0) : this.openPositionFromLiquidityInstruction(programId, ownerInfo.wallet, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolKeys.mintA.address), new PublicKey19(poolKeys.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0);
    return {
  ...
  static closePositionInstruction(programId, positionNftOwner, positionNftMint, positionNftAccount, personalPosition, nft2022) {
    ...
    const keys = [
      ...
      { pubkey: personalPosition, isSigner: false, isWritable: true },
      { pubkey: SystemProgram7.programId, isSigner: false, isWritable: false },
      { pubkey: nft2022 ? TOKEN_2022_PROGRAM_ID4 : TOKEN_PROGRAM_ID11, isSigner: false, isWritable: false }
    ];
    const data = Buffer.alloc(dataLayout.span);
    dataLayout.encode({}, data);
  ...
  static closePositionInstructions({
    ...
    const programId = new PublicKey19(poolInfo.programId);
    const positionNftAccount = nft2022 ? getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, TOKEN_2022_PROGRAM_ID4).publicKey : getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, TOKEN_PROGRAM_ID11).publicKey;
  ...
  static increasePositionFromLiquidityInstructions({
    ...
    const { publicKey: tickArrayLower } = getPdaTickArrayAddress(programId, id, tickArrayLowerStartIndex);
    const { publicKey: tickArrayUpper } = getPdaTickArrayAddress(programId, id, tickArrayUpperStartIndex);
    const { publicKey: positionNftAccount } = nft2022 ? getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, TOKEN_2022_PROGRAM_ID4) : getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, TOKEN_PROGRAM_ID11);
    const { publicKey: personalPosition } = getPdaPersonalPositionAddress(programId, ownerPosition.nftMint);
    ...
    const ins = this.increasePositionFromLiquidityInstruction(programId, ownerInfo.wallet, positionNftAccount, personalPosition, id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address), liquidity, amountMaxA, amountMaxB, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0);
    return {
  ...
  static increasePositionFromBaseInstructions({
    ...
    return {
      ...
      instructions: [
        this.increasePositionFromBaseInstruction(programId, ownerInfo.wallet, positionNftAccount, personalPosition, id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address), base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
          tickArrayLowerStartIndex,
          tickArrayUpperStartIndex
        ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0)
      ],
  ...
  static decreaseLiquidityInstructions({
    ...
    const [poolProgramId, id] = [new PublicKey19(poolInfo.programId), new PublicKey19(poolInfo.id)];
    const tickArrayLowerStartIndex = TickUtils.getTickArrayStartIndexByTick(ownerPosition.tickLower, poolInfo.config.tickSpacing);
    const tickArrayUpperStartIndex = TickUtils.getTickArrayStartIndexByTick(ownerPosition.tickUpper, poolInfo.config.tickSpacing);
    ...
    const { publicKey: positionNftAccount } = nft2022 ? getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, TOKEN_2022_PROGRAM_ID4) : getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, programId);
    ...
    for (let i = 0; i < poolInfo.rewardDefaultInfos.length; i++) {
      rewardAccounts.push({
        poolRewardVault: new PublicKey19(poolKeys.rewardInfos[i].vault),
        ownerRewardVault: ownerInfo.rewardAccounts[i],
        rewardMint: new PublicKey19(poolInfo.rewardDefaultInfos[i].mint.address)
      });
    }
    const ins = [];
    const decreaseIns = this.decreaseLiquidityInstruction(poolProgramId, ownerInfo.wallet, positionNftAccount, personalPosition, id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B), new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address), rewardAccounts, liquidity, amountMinA, amountMinB, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
  ...
  static makeSwapBaseInInstructions({
    ...
    const [programId, id] = [new PublicKey19(poolInfo.programId), new PublicKey19(poolInfo.id)];
    const [mintAVault, mintBVault] = [new PublicKey19(poolKeys.vault.A), new PublicKey19(poolKeys.vault.B)];
    const [mintA, mintB] = [new PublicKey19(poolInfo.mintA.address), new PublicKey19(poolInfo.mintB.address)];
    const isInputMintA = poolInfo.mintA.address === inputMint.toString();
    const ins = [
      this.swapInstruction(programId, ownerInfo.wallet, id, new PublicKey19(poolInfo.config.id), isInputMintA ? ownerInfo.tokenAccountA : ownerInfo.tokenAccountB, isInputMintA ? ownerInfo.tokenAccountB : ownerInfo.tokenAccountA, isInputMintA ? mintAVault : mintBVault, isInputMintA ? mintBVault : mintAVault, isInputMintA ? mintA : mintB, isInputMintA ? mintB : mintA, remainingAccounts, observationId, amountIn, amountOutMin, sqrtPriceLimitX64, true, getPdaExBitmapAccount(programId, id).publicKey)
    ];
    return {
      signers: [],
      instructions: ins,
  ...
  static initRewardInstructions({
    ...
    const ins = [
      this.initRewardInstruction(programId, ownerInfo.wallet, id, operationId, new PublicKey19(poolInfo.config.id), ownerInfo.tokenAccount, rewardInfo.programId, rewardInfo.mint, poolRewardVault, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64)
    ];
    return {
  ...
  static setRewardInstructions({
    ...
    for (let index = 0; index < poolInfo.rewardDefaultInfos.length; index++)
      if (poolInfo.rewardDefaultInfos[index].mint.address === rewardInfo.mint.toString()) {
        rewardIndex = index;
        rewardVault = new PublicKey19(poolKeys.rewardInfos[index].vault);
        rewardMint = new PublicKey19(poolKeys.rewardInfos[index].mint.address);
      }
    ...
    const operationId = getPdaOperationAccount(programId).publicKey;
    const ins = [
      this.setRewardInstruction(programId, ownerInfo.wallet, id, operationId, new PublicKey19(poolInfo.config.id), ownerInfo.tokenAccount, rewardVault, rewardMint, rewardIndex, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64)
    ];
    return {
  ...
  static async makeLockPositions({
    ...
    let lockNftMint;
    if (getEphemeralSigners) {
      lockNftMint = new PublicKey19((await getEphemeralSigners(1))[0]);
    } else {
      const _k = Keypair3.generate();
      signers.push(_k);
      lockNftMint = _k.publicKey;
    }
    const positionNftAccount = nft2022 ? getATAAddress(wallet, nftMint, TOKEN_2022_PROGRAM_ID4).publicKey : getATAAddress(wallet, nftMint, TOKEN_PROGRAM_ID11).publicKey;
    ...
    const lockNftAccount = getATAAddress(wallet, lockNftMint, TOKEN_PROGRAM_ID11).publicKey;
    ...
    const ins = ClmmInstrument.lockPositionInstructionV2({
      ...
      authPositionNftAccount: getATAAddress(authProgramId, nftMint, nft2022 ? TOKEN_2022_PROGRAM_ID4 : TOKEN_PROGRAM_ID11).publicKey,
      positionNftProgram: nft2022 ? TOKEN_2022_PROGRAM_ID4 : TOKEN_PROGRAM_ID11
    });
    return {
      address: {
        positionId,
  ...
  static lockPositionInstruction({
    ...
    const { publicKey: nftAccount } = getATAAddress(owner, positionNft, TOKEN_PROGRAM_ID11);
  ...
  static harvestLockPositionInstruction(props) {
    const [poolProgramId, poolId] = [new PublicKey19(props.poolKeys.programId), new PublicKey19(props.poolKeys.id)];
    ...
    const { publicKey: nftAccount } = getATAAddress(props.owner, props.ownerPosition.nftMint, TOKEN_PROGRAM_ID11);
    ...
    for (let i = 0; i < props.poolKeys.rewardInfos.length; i++) {
      rewardAccounts.push({
        poolRewardVault: new PublicKey19(props.poolKeys.rewardInfos[i].vault),
        ownerRewardVault: props.ownerRewardAccounts[i],
        rewardMint: new PublicKey19(props.poolKeys.rewardInfos[i].mint.address)
      });
    }
    const remainingAccounts = [
      ...rewardAccounts.map((i) => [
    ...
    const keys = [
      ...
      { pubkey: poolId, isSigner: false, isWritable: true },
      { pubkey: protocolPosition, isSigner: false, isWritable: true },
      { pubkey: new PublicKey19(props.poolKeys.vault.A), isSigner: false, isWritable: true },
      { pubkey: new PublicKey19(props.poolKeys.vault.B), isSigner: false, isWritable: true },
      { pubkey: tickArrayLower, isSigner: false, isWritable: true },
      { pubkey: tickArrayUpper, isSigner: false, isWritable: true },
      ...
      { pubkey: new PublicKey19(props.poolKeys.mintA.address), isSigner: false, isWritable: false },
      { pubkey: new PublicKey19(props.poolKeys.mintB.address), isSigner: false, isWritable: false },
      ...remainingAccounts
    ];
    return new TransactionInstruction6({
      keys,
...
import { MintLayout, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID12 } from "@solana/spl-token";
var parseTokenInfo = async ({
  connection,
  mint
}) => {
  const accountData = await connection.getAccountInfo(new PublicKey20(mint));
...
var LIQUIDITY_FEES_NUMERATOR = new BN20(25);
var LIQUIDITY_FEES_DENOMINATOR = new BN20(1e4);
...
import { PublicKey as PublicKey22, SystemProgram as SystemProgram8, SYSVAR_RENT_PUBKEY as SYSVAR_RENT_PUBKEY3, TransactionInstruction as TransactionInstruction7 } from "@solana/web3.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID3, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID13 } from "@solana/spl-token";
...
var MODEL_DATA_PUBKEY = new PublicKey21("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo");
...
function makeAddLiquidityInstruction(params) {
  ...
  const keys = [
    accountMeta({ pubkey: TOKEN_PROGRAM_ID13, isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolInfo.id) }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.targetOrders) }),
    accountMeta({ pubkey: new PublicKey22(poolInfo.lpMint.address) }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.vault.A) }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.vault.B) })
  ];
  if (poolInfo.pooltype.includes("StablePool")) {
    keys.push(accountMeta({ pubkey: MODEL_DATA_PUBKEY }));
  }
  keys.push(accountMeta({ pubkey: new PublicKey22(poolInfo.marketId), isWritable: false }), accountMeta({ pubkey: userKeys.baseTokenAccount }), accountMeta({ pubkey: userKeys.quoteTokenAccount }), accountMeta({ pubkey: userKeys.lpTokenAccount }), accountMeta({ pubkey: userKeys.owner, isWritable: false, isSigner: true }), accountMeta({ pubkey: new PublicKey22(poolKeys.marketEventQueue), isWritable: false }));
  return new TransactionInstruction7({
    programId: new PublicKey22(poolInfo.programId),
    keys,
    data
  });
}
function removeLiquidityInstruction(params) {
  ...
  return new TransactionInstruction7({ programId: poolKeys.programId, keys: [] });
}
function createPoolV4InstructionV2({
  ...
  const keys = [
    { pubkey: TOKEN_PROGRAM_ID13, isSigner: false, isWritable: false },
    { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID3, isSigner: false, isWritable: false },
    { pubkey: SystemProgram8.programId, isSigner: false, isWritable: false },
    { pubkey: RENT_PROGRAM_ID, isSigner: false, isWritable: false },
  ...
  return {
    instruction: new TransactionInstruction7({
      keys,
      programId,
      data
    }),
...
function simulatePoolInfoInstruction(poolKeys) {
  ...
  const keys = [
    accountMeta({ pubkey: new PublicKey22(poolKeys.id), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.vault.A), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.vault.B), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.mintLp.address), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.marketId), isWritable: false }),
    accountMeta({ pubkey: new PublicKey22(poolKeys.marketEventQueue), isWritable: false })
  ];
  return new TransactionInstruction7({
    programId: new PublicKey22(poolKeys.programId),
    keys,
    data
  });
}
...
function makeInitPoolInstructionV4({
  ...
  const keys = [
    accountMeta({ pubkey: TOKEN_PROGRAM_ID13, isWritable: false }),
    accountMeta({ pubkey: SystemProgram8.programId, isWritable: false }),
    accountMeta({ pubkey: SYSVAR_RENT_PUBKEY3, isWritable: false }),
    accountMeta({ pubkey: poolKeys.id }),
    accountMeta({ pubkey: poolKeys.authority, isWritable: false }),
    accountMeta({ pubkey: poolKeys.openOrders }),
    accountMeta({ pubkey: poolKeys.mintLp.address }),
...
function getAssociatedPoolKeys({
  ...
  return {
    ...
    marketId,
    marketAuthority,
    lookupTableAccount: PublicKey24.default,
  ...
  return await Promise.all(poolKeysList.map(async (itemPoolKey) => {
    if (itemPoolKey.modelDataAccount) {
      ...
      const poolsInfo = logs.map((log3) => {
        ...
        const status = new BN21(parseSimulateValue(json, "status"));
        ...
        const baseReserve = new BN21(parseSimulateValue(json, "pool_coin_amount"));
        const quoteReserve = new BN21(parseSimulateValue(json, "pool_pc_amount"));
        const lpSupply = new BN21(parseSimulateValue(json, "pool_lp_supply"));
        let startTime = "0";
        try {
        ...
        return {
          ...
          quoteReserve,
          lpSupply,
          startTime: new BN21(startTime)
        };
      })[0];
      return poolsInfo;
    } else {
      const [poolAcc, vaultAccA, vaultAccB, mintAccLp] = await connection.getMultipleAccountsInfo([
        new PublicKey24(itemPoolKey.id),
        new PublicKey24(itemPoolKey.vault.A),
        new PublicKey24(itemPoolKey.vault.B),
        new PublicKey24(itemPoolKey.mintLp.address)
      ]);
      if (poolAcc === null)
...
var toAmmComputePoolInfo = (poolData) => {
  const data = {};
  const tokenProgramStr = TOKEN_PROGRAM_ID14.toBase58();
...
import { TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID16 } from "@solana/spl-token";
...
import { createInitializeAccountInstruction as createInitializeAccountInstruction2, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID15 } from "@solana/spl-token";
...
async function makeCreateMarketInstruction({
  ...
  tx1.add(SystemProgram9.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
  ...
  }), SystemProgram9.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
    ...
    space: 165,
    programId: TOKEN_PROGRAM_ID15
  }), createInitializeAccountInstruction2(marketInfo.baseVault.publicKey, marketInfo.baseMint, marketInfo.vaultOwner), createInitializeAccountInstruction2(marketInfo.quoteVault.publicKey, marketInfo.quoteMint, marketInfo.vaultOwner), SystemProgram9.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
  ...
  tx2.add(SystemProgram9.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
...
var MarketV2 = class extends ModuleBase {
  async create({
    ...
    const feeRateBps = 0;
    const quoteDustThreshold = new BN22(100);
    function getVaultOwnerAndNonce() {
      const vaultSignerNonce2 = new BN22(0);
      while (true) {
        try {
          const vaultOwner2 = PublicKey26.createProgramAddressSync([market.publicKey.toBuffer(), vaultSignerNonce2.toArrayLike(Buffer, "le", 8)], dexProgramId);
          return { vaultOwner: vaultOwner2, vaultSignerNonce: vaultSignerNonce2 };
        } catch (e) {
          vaultSignerNonce2.iaddn(1);
          if (vaultSignerNonce2.gt(new BN22(25555)))
            throw Error("find vault owner error");
        }
      }
    }
    const { vaultOwner, vaultSignerNonce } = getVaultOwnerAndNonce();
    const baseLotSize = new BN22(Math.round(10 ** baseInfo.decimals * lotSize));
    const quoteLotSize = new BN22(Math.round(lotSize * 10 ** quoteInfo.decimals * tickSize));
    ...
    if (txVersion === 0 /* V0 */)
      return txBuilder.sizeCheckBuildV0({
        ...
        address: {
          ...
          baseMint: new PublicKey26(baseInfo.mint),
          quoteMint: new PublicKey26(quoteInfo.mint)
        }
      });
...
var LiquidityModule = class extends ModuleBase {
  ...
  async addLiquidity(params) {
    ...
    const lpTokenAccount = await account.getCreatedTokenAccount({
      mint: new PublicKey27(poolInfo.lpMint.address)
    });
  ...
  async removeLiquidity(params) {
    ...
    const [baseMint, quoteMint, lpMint] = [
      new PublicKey27(poolInfo.mintA.address),
      new PublicKey27(poolInfo.mintB.address),
      new PublicKey27(poolInfo.lpMint.address)
    ];
  ...
  async removeAllLpAndCreateClmmPosition({
    ...
    payer,
    userAuxiliaryLedgers,
    tokenProgram = TOKEN_PROGRAM_ID17,
    ...
    for (const item of this.scope.account.tokenAccountRawInfos) {
      if (mintToAccount[item.accountInfo.mint.toString()] === void 0 || getATAAddress(this.scope.ownerPubKey, item.accountInfo.mint, TOKEN_PROGRAM_ID17).publicKey.equals(item.pubkey)) {
    ...
    const { account: baseTokenAccount, instructionParams: ownerTokenAccountBaseInstruction } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: TOKEN_PROGRAM_ID17,
      mint: new PublicKey27(poolInfo.mintA.address),
      owner: this.scope.ownerPubKey,
    ...
    const { account: quoteTokenAccount, instructionParams: ownerTokenAccountQuoteInstruction } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: TOKEN_PROGRAM_ID17,
      mint: new PublicKey27(poolInfo.mintB.address),
      owner: this.scope.ownerPubKey,
    ...
    if (farmInfo !== void 0 && !userFarmLpAmount?.isZero()) {
      ...
      const ledger = getAssociatedLedgerAccount({
        programId: new PublicKey27(farmInfo.programId),
        poolId: new PublicKey27(farmInfo.id),
        owner: this.scope.ownerPubKey,
        version: farmVersion
      });
      let ledgerInfo = void 0;
      ...
      if (farmVersion !== 6 && !ledgerInfo) {
        const { instruction, instructionType } = createAssociatedLedgerAccountInstruction({
          id: new PublicKey27(farmInfo.id),
          programId: new PublicKey27(farmInfo.programId),
          version: farmVersion,
          ledger,
          owner: this.scope.ownerPubKey
      ...
      for (const item of farmInfo.rewardInfos) {
        ...
        if (mintToAccount[item.mint.address])
          ...
          const { account: farmRewardAccount, instructionParams: ownerTokenAccountFarmInstruction } = await this.scope.account.getOrCreateTokenAccount({
            mint: new PublicKey27(item.mint.address),
            tokenProgram,
            owner: this.scope.ownerPubKey,
  ...
  async createMarketAndPoolV4({
    ...
    function getVaultOwnerAndNonce() {
      ...
      while (true) {
        try {
          const vaultOwner2 = PublicKey27.createProgramAddressSync([market.publicKey.toBuffer(), vaultSignerNonce2.toArrayLike(Buffer, "le", 8)], marketProgram);
          return { vaultOwner: vaultOwner2, vaultSignerNonce: vaultSignerNonce2 };
        } catch (e) {
          vaultSignerNonce2.iaddn(1);
    ...
    if (txVersion === 0 /* V0 */)
      return txBuilder.sizeCheckBuildV0({
        ...
        address: {
          ...
          baseVault: baseVault.publicKey,
          quoteVault: quoteVault.publicKey,
          baseMint: new PublicKey27(baseMintInfo.mint),
          quoteMint: new PublicKey27(quoteMintInfo.mint),
          ...createPoolKeys
        }
      });
    return txBuilder.sizeCheckBuild({
  ...
  async swap({
    ...
    const { account: _tokenAccountIn, instructionParams: ownerTokenAccountBaseInstruction } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: TOKEN_PROGRAM_ID17,
      mint: new PublicKey27(tokenIn.address),
      owner: this.scope.ownerPubKey,
    ...
    const { account: _tokenAccountOut, instructionParams: ownerTokenAccountQuoteInstruction } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: TOKEN_PROGRAM_ID17,
      mint: new PublicKey27(tokenOut.address),
      owner: this.scope.ownerPubKey,
      createInfo: {
        payer: this.scope.ownerPubKey,
        amount: 0
  ...
  async getRpcPoolInfos(poolIds, config2) {
    const accounts = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, poolIds.map((i) => ({ pubkey: new PublicKey27(i) })), config2);
    const poolInfos = {};
    ...
    const vaultInfo = {};
    const vaultAccountInfo = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, needFetchVaults.map((i) => ({ pubkey: new PublicKey27(i) })), config2);
    for (let i = 0; i < needFetchVaults.length; i++) {
      ...
      vaultInfo[String(needFetchVaults[i])] = new BN23(AccountLayout2.decode(vaultItemInfo.data).amount.toString());
    }
    const returnData = {};
...
import { PublicKey as PublicKey28 } from "@solana/web3.js";
import BN24 from "bn.js";
import { AccountLayout as AccountLayout3, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID5, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID18 } from "@solana/spl-token";
var Clmm = class extends ModuleBase {
  ...
  async createPool(props) {
    const {
      programId,
      owner = this.scope.owner?.publicKey || PublicKey28.default,
      mint1,
      mint2,
      ammConfig,
      initialPrice,
    ...
    const txBuilder = this.createTxBuilder(feePayer);
    const [mintA, mintB, initPrice] = new BN24(new PublicKey28(mint1.address).toBuffer()).gt(new BN24(new PublicKey28(mint2.address).toBuffer())) ? [mint2, mint1, new decimal_default(1).div(initialPrice)] : [mint1, mint2, initialPrice];
    const initialPriceX64 = SqrtPriceMath.priceToSqrtPriceX64(initPrice, mintA.decimals, mintB.decimals);
    const extendMintAccount = [];
    const fetchAccounts = [];
    if (mintA.programId === TOKEN_2022_PROGRAM_ID5.toBase58())
      fetchAccounts.push(getPdaMintExAccount(programId, new PublicKey28(mintA.address)).publicKey);
    if (mintB.programId === TOKEN_2022_PROGRAM_ID5.toBase58())
      fetchAccounts.push(getPdaMintExAccount(programId, new PublicKey28(mintB.address)).publicKey);
  ...
  async openPositionFromBase({
    ...
    const { account: _ownerTokenAccountA, instructionParams: _tokenAccountAInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey28(poolInfo.mintA.address),
      owner: this.scope.ownerPubKey,
    ...
    const { account: _ownerTokenAccountB, instructionParams: _tokenAccountBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey28(poolInfo.mintB.address),
      owner: this.scope.ownerPubKey,
  ...
  async increasePositionFromLiquidity(props) {
    ...
    const ins = ClmmInstrument.increasePositionFromLiquidityInstructions({
      ...
      amountMaxA,
      amountMaxB,
      nft2022: (await this.scope.connection.getAccountInfo(ownerPosition.nftMint))?.owner.equals(TOKEN_2022_PROGRAM_ID5)
    });
  ...
  async decreaseLiquidity(props) {
    ...
    for (const itemReward of poolInfo.rewardDefaultInfos) {
      ...
      if (itemReward.mint.address === poolInfo.mintA.address)
        ...
        const { account: _ownerRewardAccount, instructionParams: ownerRewardAccountInstructions } = await this.scope.account.getOrCreateTokenAccount({
          tokenProgram: new PublicKey28(itemReward.mint.programId),
          mint: new PublicKey28(itemReward.mint.address),
          notUseTokenAccount: rewardUseSOLBalance,
          owner: this.scope.ownerPubKey,
          createInfo: {
            payer: this.scope.ownerPubKey,
    ...
    const poolKeys = propPoolKeys ?? await this.getClmmPoolKeys(poolInfo.id);
    const nft2022 = (await this.scope.connection.getAccountInfo(ownerPosition.nftMint))?.owner.equals(TOKEN_2022_PROGRAM_ID5);
  ...
  async harvestLockPosition(props) {
    ...
    const { account: _ownerTokenAccountA, instructionParams: accountAInstructions } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey28(poolKeys.mintA.address),
      notUseTokenAccount: mintAUseSOLBalance,
      owner: this.scope.ownerPubKey,
      createInfo: {
        payer: this.scope.ownerPubKey,
    ...
    const { account: _ownerTokenAccountB, instructionParams: accountBInstructions } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey28(poolKeys.mintB.address),
      notUseTokenAccount: mintBUseSOLBalance,
      owner: this.scope.ownerPubKey,
      createInfo: {
        payer: this.scope.ownerPubKey,
    ...
    const lockNftAccount = getATAAddress(this.scope.ownerPubKey, lockData.lockNftMint, TOKEN_PROGRAM_ID18).publicKey;
    ...
    const { publicKey: tickArrayLower } = getPdaTickArrayAddress(new PublicKey28(poolKeys.programId), lockData.poolId, tickArrayLowerStartIndex);
    const { publicKey: tickArrayUpper } = getPdaTickArrayAddress(new PublicKey28(poolKeys.programId), lockData.poolId, tickArrayUpperStartIndex);
    const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(new PublicKey28(poolKeys.programId), lockData.poolId, position.tickLower, position.tickUpper);
    const rewardAccountsFullInfo = [];
    for (let i = 0; i < poolKeys.rewardInfos.length; i++) {
      rewardAccountsFullInfo.push({
        poolRewardVault: new PublicKey28(poolKeys.rewardInfos[i].vault),
        ownerRewardVault: rewardAccounts[i],
        rewardMint: new PublicKey28(poolKeys.rewardInfos[i].mint.address)
      });
    }
    const harvestLockIns = await ClmmInstrument.harvestLockPositionInstructionV2({
      ...
      protocolPosition,
      vaultA: new PublicKey28(poolKeys.vault.A),
      vaultB: new PublicKey28(poolKeys.vault.B),
      tickArrayLower,
      tickArrayUpper,
      userVaultA: ownerTokenAccountA,
      userVaultB: ownerTokenAccountB,
      mintA: new PublicKey28(poolKeys.mintA.address),
      mintB: new PublicKey28(poolKeys.mintB.address),
  ...
  async initReward({
    ...
    const { account: ownerRewardAccount, instructionParams: ownerRewardAccountIns } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: new PublicKey28(rewardInfo.mint.address),
      mint: new PublicKey28(rewardInfo.mint.address),
      ...
      createInfo: rewardMintUseSOLBalance ? {
        ...
        amount: new BN24(new decimal_default(_baseRewardAmount.toFixed(0)).gte(_baseRewardAmount) ? _baseRewardAmount.toFixed(0) : _baseRewardAmount.add(1).toFixed(0))
      } : void 0,
    ...
    const insInfo = ClmmInstrument.initRewardInstructions({
      ...
      rewardInfo: {
        programId: new PublicKey28(rewardInfo.mint.programId),
        mint: new PublicKey28(rewardInfo.mint.address),
        openTime: rewardInfo.openTime,
        endTime: rewardInfo.endTime,
  ...
  async initRewards({
    ...
    for (const rewardInfo of rewardInfos) {
      ...
      const _baseRewardAmount = rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime);
      const { account: ownerRewardAccount, instructionParams: ownerRewardAccountIns } = await this.scope.account.getOrCreateTokenAccount({
        tokenProgram: new PublicKey28(rewardInfo.mint.programId),
        mint: new PublicKey28(rewardInfo.mint.address),
        notUseTokenAccount: !!rewardMintUseSOLBalance,
        skipCloseAccount: !rewardMintUseSOLBalance,
        owner: this.scope.ownerPubKey,
  ...
  async setReward({
    ...
    const { account: ownerRewardAccount, instructionParams: ownerRewardIns } = await this.scope.account.getOrCreateTokenAccount({
      ...
      createInfo: rewardMintUseSOLBalance ? {
        payer: ownerInfo.feePayer || this.scope.ownerPubKey,
        amount: new BN24(new decimal_default(rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime).toFixed(0)).gte(rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime)) ? rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime).toFixed(0) : rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime).add(1).toFixed(0))
      } : void 0,
      associatedOnly: rewardMintUseSOLBalance ? false : associatedOnly,
      checkCreateATAOwner
    });
  ...
  async swap({
    ...
    if (!priceLimit || priceLimit.equals(new decimal_default(0))) {
      sqrtPriceLimitX64 = baseIn ? MIN_SQRT_PRICE_X64.add(new BN24(1)) : MAX_SQRT_PRICE_X64.sub(new BN24(1));
    } else {
      sqrtPriceLimitX64 = SqrtPriceMath.priceToSqrtPriceX64(priceLimit, poolInfo.mintA.decimals, poolInfo.mintB.decimals);
    }
    ...
    txBuilder.addInstruction(ClmmInstrument.makeSwapBaseInInstructions({
      ...
      inputMint: new PublicKey28(inputMint),
      amountIn,
      amountOutMin,
      sqrtPriceLimitX64,
      remainingAccounts
  ...
  async swapBaseOut({
    ...
    let sqrtPriceLimitX64;
    if (!priceLimit || priceLimit.equals(new decimal_default(0))) {
      sqrtPriceLimitX64 = outputMint.toString() === poolInfo.mintB.address ? MIN_SQRT_PRICE_X64.add(new BN24(1)) : MAX_SQRT_PRICE_X64.sub(new BN24(1));
    } else {
      sqrtPriceLimitX64 = SqrtPriceMath.priceToSqrtPriceX64(priceLimit, poolInfo.mintA.decimals, poolInfo.mintB.decimals);
    }
    let ownerTokenAccountA;
    ...
    txBuilder.addInstruction(ClmmInstrument.makeSwapBaseOutInstructions({
      ...
      outputMint: new PublicKey28(outputMint),
      amountOut,
      amountInMax,
      sqrtPriceLimitX64,
      remainingAccounts
    ...
    for (const itemInfo of Object.values(allPoolInfo)) {
      ...
      for (const itemPosition of allPositions[itemInfo.id]) {
        ...
        if (lockData) {
          ...
          const { publicKey: tickArrayLower } = getPdaTickArrayAddress(new PublicKey28(poolKeys.programId), lockData.poolId, tickArrayLowerStartIndex);
          const { publicKey: tickArrayUpper } = getPdaTickArrayAddress(new PublicKey28(poolKeys.programId), lockData.poolId, tickArrayUpperStartIndex);
          const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(new PublicKey28(poolKeys.programId), lockData.poolId, itemPosition.tickLower, itemPosition.tickUpper);
          ...
          const insData = ClmmInstrument.decreaseLiquidityInstructions({
            ...
            liquidity: new BN24(0),
            amountMinA: new BN24(0),
            amountMinB: new BN24(0),
            nft2022: record[itemPosition.nftMint.toBase58()]?.equals(TOKEN_2022_PROGRAM_ID5)
          });
  ...
  async getWhiteListMint({ programId }) {
    ...
    return whitelistMintsInfo.whitelistMints.filter((i) => !i.equals(PublicKey28.default));
  }
  async getOwnerPositionInfo({
    programId
  }) {
    await this.scope.account.fetchWalletTokenAccounts();
    const balanceMints = this.scope.account.tokenAccountRawInfos.filter((acc) => acc.accountInfo.amount.eq(new BN24(1)));
    const allPositionKey = balanceMints.map((acc) => getPdaPersonalPositionAddress(new PublicKey28(programId), acc.accountInfo.mint).publicKey);
  ...
  async getRpcClmmPoolInfos({
    ...
    const accounts = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, poolIds.map((i) => ({ pubkey: new PublicKey28(i) })), config2);
    const returnData = {};
    for (let i = 0; i < poolIds.length; i++) {
      const item = accounts[i];
      if (item === null || !item.accountInfo)
  ...
  async getComputeClmmPoolInfos({
    ...
    const res = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, Array.from(configSet).map((s) => ({ pubkey: new PublicKey28(s) })));
    const clmmConfigs = {};
    res.forEach((acc) => {
      if (!acc.accountInfo)
        return;
      ...
      poolList: Object.keys(clmmPoolsRpcInfo).map((poolId) => {
        ...
        return {
          ...
          mintA: toApiV3Token({
            ...
            programId: mintInfos[mintA].programId.toBase58() || TOKEN_PROGRAM_ID18.toBase58(),
            extensions: {
          ...
          mintB: toApiV3Token({
            ...
            programId: mintInfos[mintB].programId.toBase58() || TOKEN_PROGRAM_ID18.toBase58(),
            extensions: {
  ...
  async getPoolInfoFromRpc(poolId) {
    ...
    const mintInfos = await fetchMultipleMintInfos({
      connection: this.scope.connection,
      mints: Array.from(mintSet).map((m) => new PublicKey28(m))
    });
    ...
    poolInfo.mintAmountA = Number(AccountLayout3.decode(vaultData[0].accountInfo.data).amount.toString());
    poolInfo.mintAmountB = Number(AccountLayout3.decode(vaultData[1].accountInfo?.data).amount.toString());
    const poolKeys = {
      ...
      rewardInfos: computeClmmPoolInfo[poolId].rewardInfos.filter((r) => !r.tokenVault.equals(PublicKey28.default)).map((r) => ({
        mint: toApiV3Token({ address: r.tokenMint.toBase58(), programId: TOKEN_PROGRAM_ID18.toBase58(), decimals: 10 }),
...
import { PublicKey as PublicKey30 } from "@solana/web3.js";
import {
  AccountLayout as AccountLayout4,
  createAssociatedTokenAccountIdempotentInstruction,
  getAssociatedTokenAddressSync,
  NATIVE_MINT as NATIVE_MINT2,
  TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID20
} from "@solana/spl-token";

// src/raydium/cpmm/curve/calculator.ts
import BN26 from "bn.js";
import Decimal3 from "decimal.js-light";

// src/raydium/cpmm/curve/constantProduct.ts
import BN25 from "bn.js";
...
function checkedCeilDiv(dividend, rhs) {
  ...
  if (remainder.gt(ZERO3)) {
    quotient = quotient.add(new BN25(1));
    rhs = dividend.div(quotient);
    remainder = checkedRem(dividend, quotient);
    if (remainder.gt(ZERO3)) {
      rhs = rhs.add(new BN25(1));
    }
  }
  return [quotient, rhs];
}
var ZERO3 = new BN25(0);
  ...
  static lpTokensToTradingTokens(lpTokenAmount, lpTokenSupply, swapTokenAmount0, swapTokenAmount1, roundDirection) {
    ...
    if (roundDirection === 0 /* Floor */) {
      ...
      if (tokenRemainder0.gt(ZERO3) && tokenAmount0.gt(ZERO3)) {
        tokenAmount0 = tokenAmount0.add(new BN25(1));
      }
      ...
      if (token1Remainder.gt(ZERO3) && tokenAmount1.gt(ZERO3)) {
        tokenAmount1 = tokenAmount1.add(new BN25(1));
      }
      return { tokenAmount0, tokenAmount1 };
    }
...
var CurveCalculator = class {
  ...
  static swapBaseOut({
    ...
    const currentPrice = new Decimal3(reserveOutAmount.toString()).div(10 ** reserveOutDecimals).div(new Decimal3(reserveInAmount.toString()).div(10 ** reserveInDecimals));
    const amountRealOut = outputAmount.gte(reserveOutAmount) ? reserveOutAmount.sub(new BN26(1)) : outputAmount;
    ...
    const amountIn = BNDivCeil(amountInWithoutFee.mul(new BN26(1e6)), new BN26(1e6).sub(tradeFeeRate));
    const fee = amountIn.sub(amountInWithoutFee);
    const executionPrice = new Decimal3(amountRealOut.toString()).div(10 ** reserveOutDecimals).div(new Decimal3(amountIn.toString()).div(10 ** reserveInDecimals));
...
// src/raydium/cpmm/cpmm.ts
import BN27 from "bn.js";

// src/raydium/cpmm/instruction.ts
import { PublicKey as PublicKey29, TransactionInstruction as TransactionInstruction10, Keypair as Keypair5, SystemProgram as SystemProgram10 } from "@solana/web3.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID4, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID6, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID19 } from "@solana/spl-token";
...
function makeCreateCpmmPoolInInstruction(programId, creator, configId, authority, poolId, mintA, mintB, lpMint, userVaultA, userVaultB, userLpAccount, vaultA, vaultB, createPoolFeeAccount, mintProgramA, mintProgramB, observationId, amountMaxA, amountMaxB, openTime) {
  ...
  const keys = [
    ...
    { pubkey: observationId, isSigner: false, isWritable: true },
    { pubkey: TOKEN_PROGRAM_ID19, isSigner: false, isWritable: false },
    { pubkey: mintProgramA, isSigner: false, isWritable: false },
    { pubkey: mintProgramB, isSigner: false, isWritable: false },
    { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID4, isSigner: false, isWritable: false },
...
function makeDepositCpmmInInstruction(programId, owner, authority, poolId, userLpAccount, userVaultA, userVaultB, vaultA, vaultB, mintA, mintB, lpMint, lpAmount, amountMaxA, amountMaxB) {
  ...
  const keys = [
    ...
    { pubkey: vaultB, isSigner: false, isWritable: true },
    { pubkey: TOKEN_PROGRAM_ID19, isSigner: false, isWritable: false },
    { pubkey: TOKEN_2022_PROGRAM_ID6, isSigner: false, isWritable: false },
    { pubkey: mintA, isSigner: false, isWritable: false },
    { pubkey: mintB, isSigner: false, isWritable: false },
    { pubkey: lpMint, isSigner: false, isWritable: true }
  ];
...
async function makeCpmmLockInstruction(props) {
  ...
  const signers = [];
  const [poolId, lpMint] = [new PublicKey29(poolInfo.id), new PublicKey29(poolInfo.lpMint.address)];
  let nftMintAccount;
  if (getEphemeralSigners) {
    nftMintAccount = new PublicKey29((await getEphemeralSigners(1))[0]);
  } else {
    const _k = Keypair5.generate();
    signers.push(_k);
    nftMintAccount = _k.publicKey;
  }
  const { publicKey: nftAccount } = getATAAddress(feeNftOwner, nftMintAccount, TOKEN_PROGRAM_ID19);
  const { publicKey: metadataAccount } = getPdaMetadataKey(nftMintAccount);
  const { publicKey: lockPda } = getCpLockPda(props.lockProgram, nftMintAccount);
  const { publicKey: userLpVault } = getATAAddress(ownerInfo.wallet, lpMint, TOKEN_PROGRAM_ID19);
  const { publicKey: lockLpVault } = getATAAddress(props.lockAuthProgram, lpMint, TOKEN_PROGRAM_ID19);
  const ins = cpmmLockPositionInstruction({
    ...
    userLpVault,
    lockLpVault,
    poolVaultA: new PublicKey29(poolKeys.vault.A),
    poolVaultB: new PublicKey29(poolKeys.vault.B),
    metadataAccount,
...
function cpmmLockPositionInstruction({
  ...
  const keys = [
    ...
    { pubkey: metadataAccount, isSigner: false, isWritable: true },
    { pubkey: RENT_PROGRAM_ID, isSigner: false, isWritable: false },
    { pubkey: SystemProgram10.programId, isSigner: false, isWritable: false },
    { pubkey: TOKEN_PROGRAM_ID19, isSigner: false, isWritable: false },
    { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID4, isSigner: false, isWritable: false },
    { pubkey: METADATA_PROGRAM_ID, isSigner: false, isWritable: false }
  ];
...
var CpmmModule = class extends ModuleBase {
  ...
  async getRpcPoolInfo(poolId, accountInfos, fetchConfigInfo) {
    ...
    const poolAccount = poolId ? (await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, [{ pubkey: new PublicKey30(poolId) }]))[0] : accountInfos.marketAccount;
    ...
    if (poolId) {
      accountsToFetch.push(...vaultKeys.map((key) => ({ pubkey: new PublicKey30(key) })));
      if (fetchConfigInfo) {
        accountsToFetch.push({ pubkey: new PublicKey30(poolInfo.configId) });
      }
    }
    let fetchedAccounts = [];
    ...
    const vaultAAmount = new BN27(AccountLayout4.decode(vaultAInfo.data).amount.toString());
    const vaultBAmount = new BN27(AccountLayout4.decode(vaultBInfo.data).amount.toString());
  ...
  async getRpcPoolInfos(poolIds, fetchConfigInfo) {
    const accounts = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, poolIds.map((i) => ({ pubkey: new PublicKey30(i) })));
    const poolInfos = {};
    ...
    if (fetchConfigInfo) {
      ...
      const configState = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, configIds.map((i) => ({ pubkey: new PublicKey30(i) })));
    ...
    const vaultInfo = {};
    const vaultAccountInfo = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, needFetchVaults.map((i) => ({ pubkey: new PublicKey30(i) })));
    for (let i = 0; i < needFetchVaults.length; i++) {
      const vaultItemInfo = vaultAccountInfo[i].accountInfo;
      if (vaultItemInfo === null)
        throw Error("fetch vault info error: " + needFetchVaults[i]);
      vaultInfo[String(needFetchVaults[i])] = new BN27(AccountLayout4.decode(vaultItemInfo.data).amount.toString());
    }
    const returnData = {};
    for (const [id, info] of Object.entries(poolInfos)) {
  ...
  async getPoolInfoFromRpc(poolId, rpcData, mintInfos) {
    ...
    const lpMint = toApiV3Token({
      ...
      programId: TOKEN_PROGRAM_ID20.toBase58()
    });
    const configInfo = {
    ...
    return {
      ...
      poolKeys: {
        ...
        mintLp: lpMint,
        config: configInfo,
        observationId: getPdaObservationId(rpcData.programId, new PublicKey30(poolId)).publicKey.toBase58()
      },
      rpcData
    };
  }
  async createPool({
    ...
    const payer = ownerInfo.feePayer || this.scope.owner?.publicKey;
    const isFront = new BN27(new PublicKey30(params.mintA.address).toBuffer()).lte(new BN27(new PublicKey30(params.mintB.address).toBuffer()));
    ...
    const mintAUseSOLBalance = ownerInfo.useSOLBalance && mintA.address === NATIVE_MINT2.toBase58();
    const mintBUseSOLBalance = ownerInfo.useSOLBalance && mintB.address === NATIVE_MINT2.toBase58();
    const [mintAPubkey, mintBPubkey] = [new PublicKey30(mintA.address), new PublicKey30(mintB.address)];
    const txBuilder = this.createTxBuilder(feePayer);
    ...
    const { account: userVaultB, instructionParams: userVaultBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      mint: new PublicKey30(mintB.address),
    ...
    const poolKeys = getCreatePoolKeys({
      poolId,
      programId,
      configId: new PublicKey30(feeConfig.id),
      mintA: mintAPubkey,
      mintB: mintBPubkey
    });
    txBuilder.addInstruction({
      instructions: [
        makeCreateCpmmPoolInInstruction(programId, this.scope.ownerPubKey, new PublicKey30(feeConfig.id), poolKeys.authority, poolKeys.poolId, mintAPubkey, mintBPubkey, poolKeys.lpMint, userVaultA, userVaultB, getATAAddress(this.scope.ownerPubKey, poolKeys.lpMint).publicKey, poolKeys.vaultA, poolKeys.vaultB, poolFeeAccount, new PublicKey30(mintA.programId ?? TOKEN_PROGRAM_ID20), new PublicKey30(mintB.programId ?? TOKEN_PROGRAM_ID20), poolKeys.observationId, mintAAmount, mintBAmount, startTime)
      ],
  ...
  async addLiquidity(params) {
    ...
    const mintAUseSOLBalance = poolInfo.mintA.address === NATIVE_MINT2.toString();
    const mintBUseSOLBalance = poolInfo.mintB.address === NATIVE_MINT2.toString();
    const txBuilder = this.createTxBuilder(feePayer);
    const [mintA, mintB] = [new PublicKey30(poolInfo.mintA.address), new PublicKey30(poolInfo.mintB.address)];
    const { account: tokenAccountA, instructionParams: _tokenAccountAInstruction } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: poolInfo.mintA.programId,
      mint: new PublicKey30(poolInfo.mintA.address),
      owner: this.scope.ownerPubKey,
    ...
    const { account: tokenAccountB, instructionParams: _tokenAccountBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: poolInfo.mintB.programId,
      mint: new PublicKey30(poolInfo.mintB.address),
      owner: this.scope.ownerPubKey,
    ...
    if (!tokenAccountA && !tokenAccountB)
      this.logAndCreateError("cannot found target token accounts", "tokenAccounts", account.tokenAccounts);
    const lpTokenAccount = await account.getCreatedTokenAccount({
      mint: new PublicKey30(poolInfo.lpMint.address)
    });
    const { tokenAccount: _lpTokenAccount, ...lpInstruction } = await account.handleTokenAccount({
      side: "out",
      amount: 0,
    ...
    const _slippage = new Percent(new BN27(1)).sub(slippage);
    txBuilder.addInstruction({
      instructions: [
        makeDepositCpmmInInstruction(new PublicKey30(poolInfo.programId), this.scope.ownerPubKey, new PublicKey30(poolKeys.authority), new PublicKey30(poolInfo.id), _lpTokenAccount, tokenAccountA, tokenAccountB, new PublicKey30(poolKeys.vault.A), new PublicKey30(poolKeys.vault.B), mintA, mintB, new PublicKey30(poolInfo.lpMint.address), computeResult ? computeResult?.liquidity : _slippage.mul(liquidity).quotient, baseIn ? inputAmountFee.amount : anotherAmount, baseIn ? anotherAmount : inputAmountFee.amount)
      ],
  ...
  async withdrawLiquidity(params) {
    ...
    txBuilder.addInstruction({
      instructions: [
        makeWithdrawCpmmInInstruction(new PublicKey30(poolInfo.programId), this.scope.ownerPubKey, new PublicKey30(poolKeys.authority), new PublicKey30(poolInfo.id), lpTokenAccount, tokenAccountA, tokenAccountB, new PublicKey30(poolKeys.vault.A), new PublicKey30(poolKeys.vault.B), mintA, mintB, new PublicKey30(poolInfo.lpMint.address), lpAmount, amountMintA.sub(mintAAmountFee.fee ?? new BN27(0)), amountMintB.sub(mintBAmountFee.fee ?? new BN27(0)))
      ],
  ...
  async swap(params) {
    ...
    if (!fixedOut) {
      swapResult.destinationAmountSwapped = swapResult.destinationAmountSwapped.mul(new BN27((1 - slippage) * 1e4)).div(new BN27(1e4));
    } else {
      swapResult.sourceAmountSwapped = swapResult.sourceAmountSwapped.mul(new BN27((1 + slippage) * 1e4)).div(new BN27(1e4));
    }
    const [mintA, mintB] = [new PublicKey30(poolInfo.mintA.address), new PublicKey30(poolInfo.mintB.address)];
    let mintATokenAcc;
    let mintBTokenAcc;
    if (useIdempotent) {
      const tokenProgramA = new PublicKey30(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID20);
      const tokenProgramB = new PublicKey30(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID20);
      const mintAIsSOL = mintA.equals(NATIVE_MINT2);
      const mintBIsSOL = mintB.equals(NATIVE_MINT2);
      ...
      if (!mintAIsSOL) {
        txBuilder.addInstruction({
          instructions: [
            createAssociatedTokenAccountIdempotentInstruction(this.scope.ownerPubKey, mintATokenAcc, this.scope.ownerPubKey, mintA, tokenProgramA)
          ],
          instructionTypes: [InstructionType.CreateATA]
        });
      }
      if (!mintBIsSOL) {
        txBuilder.addInstruction({
          instructions: [
            createAssociatedTokenAccountIdempotentInstruction(this.scope.ownerPubKey, mintBTokenAcc, this.scope.ownerPubKey, mintB, tokenProgramB)
          ],
          instructionTypes: [InstructionType.CreateATA]
        });
      }
      ...
      const { account: mintATokenAccResult, instructionParams: mintATokenAccInstruction } = await this.scope.account.getOrCreateTokenAccount({
        mint: mintA,
        tokenProgram: new PublicKey30(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID20),
        owner: this.scope.ownerPubKey,
        createInfo: mintAUseSOLBalance || !baseIn ? {
          payer: this.scope.ownerPubKey,
      ...
      const { account: mintBTokenAccResult, instructionParams: mintBTokenAccInstruction } = await this.scope.account.getOrCreateTokenAccount({
        mint: mintB,
        tokenProgram: new PublicKey30(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID20),
        owner: this.scope.ownerPubKey,
        createInfo: mintBUseSOLBalance || baseIn ? {
          payer: this.scope.ownerPubKey,
    ...
    const sourceTokenProgram = baseIn ? new PublicKey30(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID20) : new PublicKey30(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID20);
    const destinationTokenProgram = baseIn ? new PublicKey30(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID20) : new PublicKey30(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID20);
    txBuilder.addInstruction({
      instructions: [
        !fixedOut ? makeSwapCpmmBaseInInstruction(new PublicKey30(poolInfo.programId), this.scope.ownerPubKey, new PublicKey30(poolKeys.authority), new PublicKey30(poolKeys.config.id), new PublicKey30(poolInfo.id), sourceTokenAccount, destinationTokenAccount, new PublicKey30(sourceVault), new PublicKey30(destinationVault), sourceTokenProgram, destinationTokenProgram, sourceMint, destinationMint, getPdaObservationId(new PublicKey30(poolInfo.programId), new PublicKey30(poolInfo.id)).publicKey, inputAmount, swapResult.destinationAmountSwapped) : makeSwapCpmmBaseOutInstruction(new PublicKey30(poolInfo.programId), this.scope.ownerPubKey, new PublicKey30(poolKeys.authority), new PublicKey30(poolKeys.config.id), new PublicKey30(poolInfo.id), sourceTokenAccount, destinationTokenAccount, new PublicKey30(sourceVault), new PublicKey30(destinationVault), sourceTokenProgram, destinationTokenProgram, sourceMint, destinationMint, getPdaObservationId(new PublicKey30(poolInfo.programId), new PublicKey30(poolInfo.id)).publicKey, swapResult.sourceAmountSwapped, swapResult.destinationAmountSwapped)
      ],
  ...
  async harvestLockLp(params) {
    ...
    const poolKeys = params.poolKeys ?? await this.getCpmmPoolKeys(poolInfo.id);
    const { publicKey: nftAccount } = getATAAddress(feePayer, nftMint, TOKEN_PROGRAM_ID20);
    const { publicKey: lockPda } = getCpLockPda(programId, nftMint);
    const { publicKey: lockLpVault } = getATAAddress(authProgram, new PublicKey30(poolInfo.lpMint.address), TOKEN_PROGRAM_ID20);
    txBuilder.addInstruction({
      instructions: [
        collectCpFeeInstruction({
          ...
          nftAccount,
          lockPda,
          poolId: new PublicKey30(poolInfo.id),
          mintLp: new PublicKey30(poolKeys.mintLp.address),
          userVaultA: tokenAccountA,
          userVaultB: tokenAccountB,
          poolVaultA: new PublicKey30(poolKeys.vault.A),
          poolVaultB: new PublicKey30(poolKeys.vault.B),
          mintA,
          mintB,
          lockLpVault,
          lpFeeAmount,
  ...
  computeSwapAmount({
    ...
    const minAmountOut = swapResult.destinationAmountSwapped.mul(new BN27((1 - slippage) * 1e4)).div(new BN27(1e4));
    return {
  ...
  computePairAmount({
    ...
    const inputAmount = new BN27(new decimal_default(amount).mul(10 ** poolInfo[baseIn ? "mintA" : "mintB"].decimals).mul(coefficient).toFixed(0));
    const inputAmountFee = getTransferAmountFeeV2(inputAmount, poolInfo[baseIn ? "mintA" : "mintB"].extensions.feeConfig, epochInfo, false);
    const _inputAmountWithoutFee = inputAmount.sub(inputAmountFee.fee ?? new BN27(0));
    const lpAmount = new BN27(new decimal_default(poolInfo.lpAmount).mul(10 ** poolInfo.lpMint.decimals).toFixed(0, decimal_default.ROUND_DOWN));
    this.logDebug("baseReserve:", baseReserve.toString(), "quoteReserve:", quoteReserve.toString());
    ...
    const _slippage = new Percent(new BN27(1)).add(slippage);
    const _slippageMin = new Percent(new BN27(1)).sub(slippage);
    const slippageAdjustedAmount = getTransferAmountFeeV2(_slippage.mul(anotherAmountFee.amount.sub(anotherAmountFee.fee ?? new BN27(0))).quotient, poolInfo[baseIn ? "mintB" : "mintA"].extensions.feeConfig, epochInfo, true);
    const slippageAdjustedMinAmount = getTransferAmountFeeV2(_slippageMin.mul(anotherAmountFee.amount.sub(anotherAmountFee.fee ?? new BN27(0))).quotient, poolInfo[baseIn ? "mintB" : "mintA"].extensions.feeConfig, epochInfo, true);
...
import { createTransferInstruction as createTransferInstruction2, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID22, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID8 } from "@solana/spl-token";
import BN29 from "bn.js";
...
import { PublicKey as PublicKey31 } from "@solana/web3.js";
...
var Market = class {
  ...
  static getAssociatedAuthority({ programId, marketId }) {
    ...
    while (nonce < 100) {
      try {
        const seedsWithNonce = seeds.concat(Buffer.from([nonce]), Buffer.alloc(7));
        publicKey2 = PublicKey31.createProgramAddressSync(seedsWithNonce, programId);
      } catch (err) {
        if (err instanceof TypeError) {
          throw err;
        }
    ...
    return { publicKey: PublicKey31.default, nonce };
  }
};

// src/raydium/tradeV2/instrument.ts
import { PublicKey as PublicKey32, SystemProgram as SystemProgram11, TransactionInstruction as TransactionInstruction11 } from "@solana/web3.js";
import BN28 from "bn.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID5, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID7, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID21 } from "@solana/spl-token";
function route1Instruction(programId, poolInfoA, poolKeyA, poolKeyB, userSourceToken, userRouteToken, userPdaAccount, ownerWallet, inputMint, amountIn, amountOut, tickArrayA) {
  ...
  const keys = [
    { pubkey: SystemProgram11.programId, isSigner: false, isWritable: false },
    { pubkey: TOKEN_PROGRAM_ID21, isSigner: false, isWritable: false },
    { pubkey: new PublicKey32(poolKeyA.programId), isSigner: false, isWritable: false },
    { pubkey: new PublicKey32(poolKeyA.id), isSigner: false, isWritable: true },
    { pubkey: new PublicKey32(poolKeyB.id), isSigner: false, isWritable: true },
  ...
  if (poolInfoA.type === "Concentrated") {
    ...
    keys.push(...[
      ...
      { pubkey: poolKey.id, isSigner: false, isWritable: true },
      { pubkey: new PublicKey32("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo"), isSigner: false, isWritable: false },
...
function route2Instruction(programId, poolInfoB, poolKeyA, poolKeyB, userRouteToken, userDestinationToken, userPdaAccount, ownerWallet, routeMint, tickArrayB) {
  ...
  const keys = [
    { pubkey: SystemProgram11.programId, isSigner: false, isWritable: false },
    { pubkey: TOKEN_PROGRAM_ID21, isSigner: false, isWritable: false },
    { pubkey: new PublicKey32(String(poolKeyB.programId)), isSigner: false, isWritable: false },
    { pubkey: new PublicKey32(String(poolKeyB.id)), isSigner: false, isWritable: true },
    { pubkey: new PublicKey32(String(poolKeyA.id)), isSigner: false, isWritable: true },
    { pubkey: userRouteToken, isSigner: false, isWritable: true },
...
function routeInstruction(programId, wallet, userSourceToken, userRouteToken, userDestinationToken, inputMint, routeMint, outputMint, poolInfoA, poolInfoB, poolKeyA, poolKeyB, amountIn, amountOut, remainingAccounts) {
  ...
  const keys = [
    accountMeta({ pubkey: TOKEN_PROGRAM_ID21, isWritable: false }),
    accountMeta({ pubkey: TOKEN_2022_PROGRAM_ID7, isWritable: false }),
    accountMeta({ pubkey: ASSOCIATED_TOKEN_PROGRAM_ID5, isWritable: false }),
    accountMeta({ pubkey: SystemProgram11.programId, isWritable: false }),
  ...
  for (let index = 0; index < poolInfos.length; index++) {
    ...
    keys.push(accountMeta({ pubkey: new PublicKey32(_poolInfo.programId), isWritable: false }));
    if (index === poolInfos.length - 1) {
      keys.push(accountMeta({ pubkey: userDestinationToken }));
    } else {
      keys.push(accountMeta({ pubkey: userRouteToken }));
    }
    keys.push(accountMeta({ pubkey: new PublicKey32(routeMints[index]) }));
    keys.push(accountMeta({ pubkey: new PublicKey32(routeMints[index + 1]) }));
    if (_poolInfo.version === 6) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? _poolKey.vault.A : _poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? _poolKey.vault.B : _poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolInfo.observationId) }));
      keys.push(accountMeta({ pubkey: MEMO_PROGRAM_ID2 }));
      keys.push(accountMeta({
        pubkey: getPdaExBitmapAccount(new PublicKey32(_poolInfo.programId), new PublicKey32(_poolInfo.id)).publicKey
      }));
      ...
      for (const item of remainingAccounts[index] ?? []) {
        keys.push(accountMeta({ pubkey: new PublicKey32(item) }));
      }
    } else if (_poolInfo.version === 5) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketProgramId) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketAuthority) }));
      keys.push(accountMeta({ pubkey: LIQUIDITY_POOL_PROGRAM_ID_V5_MODEL, isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.marketQuoteVault) }));
    } else if (_poolInfo.version === 4) {
      const _poolKey = poolKeys[index];
      const isSupportIdOnly = _poolInfo.status !== 1;
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketProgramId) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketAuthority) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(isSupportIdOnly ? _poolKey.id : _poolKey.marketQuoteVault) }));
    } else if (_poolInfo.version === 7) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.authority) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? _poolKey.vault.A : _poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? _poolKey.vault.B : _poolKey.vault.A) }));
...
function clmmPriceLimitX64InsData(x64Price, inputIsA) {
  if (x64Price) {
    if (inputIsA) {
      const _m = new BN28(x64Price).div(new BN28(25));
      return _m.gt(MIN_SQRT_PRICE_X64_ADD_ONE) ? _m : MIN_SQRT_PRICE_X64_ADD_ONE;
    } else {
      const _m = new BN28(x64Price).mul(new BN28(25));
  ...
  if (swapInfo.routeType === "amm") {
    if (swapInfo.poolInfo[0].version === 6) {
      ...
      return ClmmInstrument.makeSwapBaseInInstructions({
        ...
        amountOutMin: swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new BN28(0)),
        sqrtPriceLimitX64,
      ...
      return {
        ...
        instructions: [
          makeAMMSwapInstruction({
            ...
            amountIn: swapInfo.amountIn.amount.raw,
            amountOut: swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new BN28(0)),
            fixedSide: "in"
          })
        ],
    ...
    return {
      signers: [],
      instructions: [
        routeInstruction(routeProgram, ownerInfo.wallet, ownerInfo.sourceToken, ownerInfo.routeToken, ownerInfo.destinationToken, inputMint.toString(), swapInfo.middleToken.mint.toString(), swapInfo.outputMint.toString(), poolInfo1, poolInfo2, poolKey1, poolKey2, swapInfo.amountIn.amount.raw, swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new BN28(0)), swapInfo.remainingAccounts)
      ],
...
function swapBaseInAutoAccount({
  ...
  for (let index = 0; index < poolKeys.length; index++) {
    ...
    keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.programId), isWritable: false }));
    if (index === poolKeys.length - 1) {
      ...
      if (cacheAccount[mint] === void 0) {
        const ata = getATAAddress(wallet, new PublicKey32(mint), _poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58() || _poolKey.programId === ALL_PROGRAM_ID.CREATE_CPMM_POOL_PROGRAM.toBase58() ? new PublicKey32(inputIsA ? _poolKey.mintB.programId : _poolKey.mintA.programId) : TOKEN_PROGRAM_ID21).publicKey;
        cacheAccount[mint] = ata;
      }
      keys.push(accountMeta({ pubkey: cacheAccount[mint] }));
    }
    keys.push(accountMeta({ pubkey: new PublicKey32(_routeInfo.inputMint) }));
    keys.push(accountMeta({ pubkey: new PublicKey32(_routeInfo.outputMint) }));
    if (_poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58()) {
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? poolKey.vault.A : poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? poolKey.vault.B : poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.observationId) }));
      keys.push(accountMeta({ pubkey: MEMO_PROGRAM_ID2, isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.exBitmapAccount) }));
      ...
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketProgramId), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketAuthority), isWritable: false }));
      keys.push(accountMeta({ pubkey: LIQUIDITY_POOL_PROGRAM_ID_V5_MODEL, isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.marketQuoteVault) }));
      ...
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.authority) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? poolKey.vault.A : poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey32(inputIsA ? poolKey.vault.B : poolKey.vault.A) }));
  ...
  dataLayout.encode({
    insId: 0,
    amountIn: amount,
    amountOut: new BN28(routeInfo.data.otherAmountThreshold),
    clmmPriceLimit
  }, data);
  return new TransactionInstruction11({
    keys,
...
function swapBaseOutAutoAccount({
  ...
  for (let index = poolKeys.length - 1; index >= 0; index--) {
    ...
    const _poolKey = poolKeys[index];
    const inputIsA = _routeInfo.inputMint === _poolKey.mintA.address;
    keys.push(accountMeta({ pubkey: new PublicKey32(_poolKey.programId) }));
    if (index === 0) {
      keys.push(accountMeta({ pubkey: cacheAccount[_routeInfo.inputMint] }));
    } else {
      const mint = _routeInfo.inputMint;
      if (cacheAccount[mint] === void 0) {
        const ata = getATAAddress(wallet, new PublicKey32(mint), _poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58() || _poolKey.programId === ALL_PROGRAM_ID.CREATE_CPMM_POOL_PROGRAM.toBase58() ? new PublicKey32(inputIsA ? _poolKey.mintA.programId : _poolKey.mintB.programId) : TOKEN_PROGRAM_ID21).publicKey;
        cacheAccount[mint] = ata;
      }
      keys.push(accountMeta({ pubkey: cacheAccount[mint] }));
    }
  ...
  dataLayout.encode({
    insId: 1,
    amountIn: new BN28(routeInfo.data.otherAmountThreshold),
    amountOut: new BN28(routeInfo.data.outputAmount),
    clmmPriceLimit
  }, data);
  return new TransactionInstruction11({
    keys,
...
var TradeV2 = class extends ModuleBase {
  ...
  async swap({
    ...
    const { account: sourceAcc, instructionParams: sourceAccInsParams } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: amountIn.amount.token.isToken2022 ? TOKEN_2022_PROGRAM_ID8 : TOKEN_PROGRAM_ID22,
      mint: inputMint,
    ...
    if (swapInfo.routeType === "route" && !isOutputSol) {
      destinationAcc = this.scope.account.getAssociatedTokenAccount(outputMint, amountOut.amount.token.isToken2022 ? TOKEN_2022_PROGRAM_ID8 : TOKEN_PROGRAM_ID22);
    } else {
      const { account, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
        tokenProgram: amountOut.amount.token.isToken2022 ? TOKEN_2022_PROGRAM_ID8 : TOKEN_PROGRAM_ID22,
        mint: outputMint,
    ...
    if (swapInfo.routeType === "route") {
      ...
      routeTokenAcc = this.scope.account.getAssociatedTokenAccount(middleMint.mint, middleMint.isToken2022 ? TOKEN_2022_PROGRAM_ID8 : TOKEN_PROGRAM_ID22);
    }
  ...
  getAllRoute({
    ...
    inputMint = inputMint.toString() === PublicKey33.default.toString() ? WSOLMint : inputMint;
    outputMint = outputMint.toString() === PublicKey33.default.toString() ? WSOLMint : outputMint;
    const needSimulate = {};
    const needTickArray = {};
    const cpmmPoolList = {};
    const directPath = [];
    ...
    for (const itemClmmPool of clmmPools ?? []) {
      ...
      if (itemClmmPool.mintA.equals(inputMint)) {
        ...
        if (routePathDict[t] === void 0)
          routePathDict[t] = {
            mintProgram: TOKEN_PROGRAM_ID22,
            in: [],
            out: [],
            mDecimals: 0
          };
  ...
  async fetchSwapRoutesData({
    ...
    Object.values(ammSimulateCache).forEach((p) => {
      ...
      mintInfos[p.mintA.address] = {
        address: new PublicKey33(p.mintA.address),
        programId: TOKEN_PROGRAM_ID22,
        mintAuthority: null,
        supply: BigInt(0),
      ...
      mintInfos[p.mintB.address] = {
        address: new PublicKey33(p.mintB.address),
        programId: TOKEN_PROGRAM_ID22,
        mintAuthority: null,
        supply: BigInt(0),
    ...
    Object.values(cpmmPoolsRpcInfo).forEach((p) => {
      ...
      if (p.mintProgramA.equals(TOKEN_PROGRAM_ID22)) {
        ...
        mintSet.add(mintA);
      if (p.mintProgramB.equals(TOKEN_PROGRAM_ID22)) {
    ...
    const fetchMintInfoRes = await fetchMultipleMintInfos({
      connection: this.scope.connection,
      mints: Array.from(mintSet).map((m) => new PublicKey33(m))
    });
    mintInfos = {
      ...mintInfos,
      ...fetchMintInfoRes
  ...
  getAllRouteComputeAmountOut({
    ...
    const _amountInFee = feeConfig === void 0 ? new BN29(0) : inputTokenAmount.raw.mul(new BN29(feeConfig.feeBps.toNumber())).div(new BN29(1e4));
  ...
  async computePoolToPoolKeys({
    ...
    if (ammMarketFetchKeys.size > 0) {
      const marketAccount = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, Array.from(ammMarketFetchKeys).map((p) => ({ pubkey: new PublicKey33(p) })));
    ...
    pools.forEach((pool) => {
      if (pool.version === 6) {
        ...
        const ammKeys = {
          ...
          authority: getLiquidityAssociatedAuthority({ programId: new PublicKey33(pool.programId) }).publicKey.toString(),
        ...
        poolKeys.push({
          ...
          mintLp: toApiV3Token({
            ...
            programId: TOKEN_PROGRAM_ID22.toBase58(),
...
import { PublicKey as PublicKey34, Transaction as Transaction4, TransactionInstruction as TransactionInstruction12 } from "@solana/web3.js";
import { TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID23 } from "@solana/spl-token";
import BN30 from "bn.js";
var _Utils1216 = class extends ModuleBase {
  ...
  static getPdaOwnerId(programId, poolId, owner, version) {
    return findProgramAddress([
      ...
      poolId.toBuffer(),
      owner.toBuffer(),
      Buffer.from(new BN30(version).toArray())
    ], programId);
  }
  static async getAllInfo({
    ...
    for (let index = 0; index < pdaInfo.length; index++) {
      ...
      const hasCanClaimToken = itemOwnerInfo.tokenInfo.map((i2) => i2.debtAmount.gt(new BN30(0))).filter((i2) => !i2).length !== 3;
  ...
  async makeClaimAllTransaction({
    ...
    if (forecastTransactionSize(instructions, [wallet, ...signers.map((s) => s.publicKey)])) {
      ...
      return [
        {
          transaction: new Transaction4().add(...instructions.slice(0, txBuilder.AllTxData.instructions.length - 1)),
          signer: signers
        },
        {
          transaction: new Transaction4().add(...instructions.slice(txBuilder.AllTxData.instructions.length - 1)),
          signer: []
        },
        { transaction: new Transaction4().add(...txBuilder.AllTxData.endInstructions), signer: [] }
      ];
    }
  }
  static makeClaimInstruction({
    ...
    const keys = [
      ...
      { pubkey: TOKEN_PROGRAM_ID23, isSigner: false, isWritable: false }
    ];
    const data = Buffer.alloc(dataLayout.span);
    dataLayout.encode({}, data);
...
import { PublicKey as PublicKey36 } from "@solana/web3.js";
import BN31 from "bn.js";

// src/raydium/ido/instruction.ts
import { SYSVAR_CLOCK_PUBKEY as SYSVAR_CLOCK_PUBKEY2, TransactionInstruction as TransactionInstruction13 } from "@solana/web3.js";
import { TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID24 } from "@solana/spl-token";
...
function makePurchaseInstruction({
  ...
  const keys = [
    { pubkey: SYSTEM_PROGRAM_ID, isSigner: false, isWritable: false },
    { pubkey: TOKEN_PROGRAM_ID24, isSigner: false, isWritable: false },
    { pubkey: RENT_PROGRAM_ID, isSigner: false, isWritable: false },
  ...
  return new TransactionInstruction13({ keys, programId, data });
}
...
var MarketV22 = class extends ModuleBase {
  async claim({
    ...
    const [hasUnClaimedProject, hasUnClaimedBuy] = [!new BN31(ownerInfo.coin).isZero(), !new BN31(ownerInfo.pc).isZero()];
        ...
        instructions: [
          ...hasUnClaimedProject ? [
            makeClaimInstruction({ programId: poolConfigKey.programId }, {
              ...
              userIdoInfo: new PublicKey36(ownerInfo.userIdoInfo),
              userOwner: this.scope.ownerPubKey
            })
          ] : [],
          ...hasUnClaimedBuy ? [
            makeClaimInstruction({ programId: new PublicKey36(idoKeys.programId) }, {
              idoId: poolConfigKey.id,
              authority: poolConfigKey.authority,
    ...
    const keys = {
      ...
      userKeys: {
        ...
        ledgerAccount: new PublicKey36(ownerInfo.userIdoInfo),
        owner: this.scope.ownerPubKey
      }
    };
    return txBuilder.addInstruction({
...
import { SystemProgram as SystemProgram12, TransactionInstruction as TransactionInstruction14 } from "@solana/web3.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID6 } from "@solana/spl-token";
import BN32 from "bn.js";
var anchorDataBuf4 = {
...
function initialize(programId, payer, creator, configId, platformId, auth, poolId, mintA, mintB, vaultA, vaultB, metadataId, tokenProgramA, tokenProgramB, decimals, name, symbol, uri, curveParam, totalLockedAmount, cliffPeriod, unlockPeriod) {
  ...
  const keys = [
    ...
    { pubkey: METADATA_PROGRAM_ID, isSigner: false, isWritable: false },
    { pubkey: SystemProgram12.programId, isSigner: false, isWritable: false },
    { pubkey: RENT_PROGRAM_ID, isSigner: false, isWritable: false },
...
function buyExactInInstruction(programId, owner, auth, configId, platformId, poolId, userTokenAccountA, userTokenAccountB, vaultA, vaultB, mintA, mintB, tokenProgramA, tokenProgramB, amountB, minAmountA, shareFeeRate, shareFeeReceiver) {
  ...
  dataLayout.encode({
    amountB,
    minAmountA,
    shareFeeRate: shareFeeRate ?? new BN32(0)
  }, data);
  return new TransactionInstruction14({
    keys,
    programId,
...
function claimVestedToken(programId, owner, auth, poolId, vestingRecord, userTokenAccountA, vaultA, mintA, tokenProgramA) {
  ...
  const keys = [
    ...
    { pubkey: tokenProgramA, isSigner: false, isWritable: false },
    { pubkey: SystemProgram12.programId, isSigner: false, isWritable: false },
    { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID6, isSigner: false, isWritable: false }
  ];
  const data = Buffer.alloc(dataLayout.span);
  dataLayout.encode({}, data);
  return new TransactionInstruction14({
...
function claimPlatformFee(programId, platformClaimFeeWallet, auth, poolId, platformId, vaultB, userTokenAccountB, mintB, tokenProgramB) {
  const keys = [
    ...
    { pubkey: SystemProgram12.programId, isSigner: false, isWritable: true },
    { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID6, isSigner: false, isWritable: true }
  ];
  return new TransactionInstruction14({
    keys,
    programId,
...
import { NATIVE_MINT as NATIVE_MINT3, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID25, createAssociatedTokenAccountIdempotentInstruction as createAssociatedTokenAccountIdempotentInstruction2 } from "@solana/spl-token";
import BN38 from "bn.js";
import { PublicKey as PublicKey38 } from "@solana/web3.js";
...
var LaunchPadConstantProductCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (x0.lt(new BN33(0)) || y0.lt(new BN33(0)))
...
var FixedPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (supplyMinusLocked.lte(new BN34(0)))
      throw Error("invalid input 1");
    const denominator = new BN34(2).mul(totalFundRaising).sub(migrateFee);
...
import BN36 from "bn.js";

// src/raydium/launchpad/curve/func.ts
import BN35 from "bn.js";
var MathLaunch = class {
  ...
  static getPriceX64({ price, decimalA, decimalB }) {
    ...
    const priceX64 = new BN35(priceWithDecimals.mul(this._Q64).toFixed(0));
    return priceX64;
  }
};
__publicField(MathLaunch, "_Q64", new decimal_default(new BN35(1).shln(64).toString()));
function checkPoolToAmm({
  ...
  const liquidity = new BN35(new decimal_default(migrateAmountA.mul(totalFundRaisingB).toString()).sqrt().toFixed(0));
  if (migrateType === "amm") {
    if (liquidity.gt(new BN35(10).pow(new BN35(decimalsA))))
      return true;
  } else if (migrateType === "cpmm") {
    if (liquidity.gt(new BN35(100)))
      return true;
  } else {
...
var LinearPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    const supplyMinusLocked = supply.sub(totalLockedAmount);
    if (supplyMinusLocked.lte(new BN36(0)))
      throw Error("supplyMinusLocked need gt 0");
    const denominator = totalFundRaising.mul(new BN36(3)).sub(migrateFee);
    const numerator = totalFundRaising.mul(new BN36(2)).mul(supplyMinusLocked);
    const totalSellExpect = numerator.div(denominator);
    const totalSellSquared = totalSellExpect.mul(totalSellExpect);
    const a = totalFundRaising.mul(new BN36(2)).mul(Q64).div(totalSellSquared);
    if (!a.gt(new BN36(0)))
      throw Error("a need gt 0");
    if (!MaxU64.gt(a))
      throw Error("a need lt u64 max");
    return { a, b: new BN36(0), c: totalSellExpect };
  }
  static buyExactIn({ poolInfo, amount }) {
    const newQuote = poolInfo.realB.add(amount);
    const termInsideSqrt = new BN36(2).mul(newQuote).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN36(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
  ...
  static buyExactOut({ poolInfo, amount }) {
    ...
    const { div: _newQuoteDiv, mod: _newQuoteMod } = poolInfo.virtualA.mul(newBaseSquared).divmod(new BN36(2).mul(Q64));
    const newQuote = _newQuoteMod.isZero() ? _newQuoteDiv : _newQuoteDiv.add(new BN36(1));
  ...
  static sellExactOut({ poolInfo, amount }) {
    ...
    const termInsideSqrt = new BN36(2).mul(newB).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN36(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
...
var Curve = class {
  ...
  static getPoolCurvePointByInit({
    ...
    const itemStepBuy = totalFundRaising.div(new BN37(pointCount - 1));
    const zero = new BN37(0);
  ...
  static checkParam({
    ...
    if (supply.lt(config2.minSupplyA.mul(new BN37(10 ** decimals))))
    ...
    const liquidity = new BN37(new decimal_default(migrateAmountA.mul(totalFundRaising).toString()).sqrt().toFixed(0));
    if (migrateType === "amm") {
      const minLockLp = new BN37(10).pow(new BN37(decimals));
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else if (migrateType === "cpmm") {
      const minLockLp = new BN37(100);
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else {
      throw Error("migrate type error");
  ...
  static splitFee({
    ...
    const platformFee = totalFeeRate.isZero() ? new BN37(0) : totalFee.mul(platformFeeRate).div(totalFeeRate);
    const shareFee = totalFeeRate.isZero() ? new BN37(0) : totalFee.mul(shareFeeRate).div(totalFeeRate);
  ...
  static calculatePreFee({ postFeeAmount, feeRate }) {
    ...
    return numerator.add(denominator).sub(new BN37(1)).div(denominator);
  }
...
var LaunchpadPoolInitParam = {
  initPriceX64: new BN38("515752397214619"),
  supply: new BN38(1e15),
  totalSellA: new BN38(7931e11),
  totalFundRaisingB: new BN38(85e9),
  totalLockedAmount: new BN38("0"),
  cliffPeriod: new BN38("0"),
  unlockPeriod: new BN38("0"),
  decimals: 6,
  virtualA: new BN38("1073471847374405"),
  virtualB: new BN38("30050573465"),
  realA: new BN38(0),
  realB: new BN38(0),
  protocolFee: new BN38(0),
  platformId: new PublicKey38("4Bu96XjU84XjPDSpveTVf6LYGCkfW5FK7SNkREWcEfV4"),
  vestingSchedule: {
    totalLockedAmount: new BN38(0),
    cliffPeriod: new BN38(0),
    unlockPeriod: new BN38(0),
    startTime: new BN38(0),
    totalAllocatedShare: new BN38(0)
  }
};
var SLIPPAGE_UNIT = new BN38(1e4);
var LaunchpadModule = class extends ModuleBase {
  ...
  async createLaunchpad({
    ...
    const totalLockedAmount = extraConfigs?.totalLockedAmount ?? new BN38(0);
    ...
    const { c } = initCurve.getInitParam({
      ...
      totalFundRaising: poolInfo.totalFundRaisingB,
      totalLockedAmount,
      totalSell: configInfo.curveType === 0 ? poolInfo.totalSellA : new BN38(0),
      migrateFee: configInfo.migrateFee
    });
    try {
      Curve.checkParam({
    ...
    txBuilder.addInstruction({
      instructions: [
        initialize(programId, feePayer ?? this.scope.ownerPubKey, this.scope.ownerPubKey, configId, platformId, authProgramId, poolId, mintA, mintB, vaultA, vaultB, metaId, TOKEN_PROGRAM_ID25, TOKEN_PROGRAM_ID25, decimals, name, symbol, uri || "https://", {
          type: curType === 0 ? "ConstantCurve" : curType === 1 ? "FixedCurve" : curType === 2 ? "LinearCurve" : "ConstantCurve",
          totalSellA,
          migrateType,
          supply,
          totalFundRaisingB
        }, totalLockedAmount, extraConfigs?.cliffPeriod ?? new BN38(0), extraConfigs?.unlockPeriod ?? new BN38(0))
      ]
    });
    let outAmount = new BN38(0);
    let splitIns;
  ...
  async buyToken({
    ...
    const mintBUseSOLBalance = mintB.equals(NATIVE_MINT3);
    const { account: _ownerTokenAccountA, instructionParams: _tokenAccountAInstruction } = await this.scope.account.getOrCreateTokenAccount({
      mint: mintA,
      owner: this.scope.ownerPubKey,
      createInfo: {
    ...
    const minMintAAmount = propMinMintAAmount ?? (slippage ? new BN38(decimalAmountA.mul(multiplier).toFixed(0)) : calculatedAmount.amountA);
    ...
    const shareATA = shareFeeReceiver ? getATAAddress(shareFeeReceiver, mintB, TOKEN_PROGRAM_ID25).publicKey : void 0;
    if (shareATA) {
      txBuilder.addInstruction({
        instructions: [
          createAssociatedTokenAccountIdempotentInstruction2(this.scope.ownerPubKey, shareATA, shareFeeReceiver, mintB)
        ]
      });
    }
    txBuilder.addInstruction({
      instructions: [
        buyExactInInstruction(programId, this.scope.ownerPubKey, authProgramId, poolInfo.configId, poolInfo.platformId, poolId, userTokenAccountA, userTokenAccountB, poolInfo.vaultA, poolInfo.vaultB, mintA, mintB, TOKEN_PROGRAM_ID25, TOKEN_PROGRAM_ID25, calculatedAmount.amountB.lt(buyAmount) ? calculatedAmount.amountB : buyAmount, minMintAAmount, shareFeeRate, shareATA)
      ]
    });
    txBuilder.addCustomComputeBudget(computeBudgetConfig);
    txBuilder.addTipInstruction(txTipConfig);
  ...
  async sellToken({
    ...
    const multiplier = slippage ? new decimal_default(SLIPPAGE_UNIT.sub(slippage).toNumber() / SLIPPAGE_UNIT.toNumber()).clampedTo(0, 1) : new decimal_default(1);
    const minAmountB = propMinAmountB ?? (slippage ? new BN38(decimalAmountB.mul(multiplier).toFixed(0)) : calculatedAmount.amountB);
    if (minAmountB.lte(new BN38(0)))
  ...
  async claimPlatformFee({
    ...
    mintB: propsMintB,
    vaultB: propsVaultB,
    mintBProgram = TOKEN_PROGRAM_ID25,
    txVersion,
    computeBudgetConfig,
    txTipConfig,
    feePayer
    ...
    const userTokenAccountB = getATAAddress(this.scope.ownerPubKey, mintB, TOKEN_PROGRAM_ID25).publicKey;
    txBuilder.addInstruction({
      instructions: [
        createAssociatedTokenAccountIdempotentInstruction2(this.scope.ownerPubKey, userTokenAccountB, this.scope.ownerPubKey, mintB)
      ]
    });
    txBuilder.addInstruction({
      instructions: [
  ...
  async claimAllPlatformFee({
    ...
    allPlatformPool.forEach((data) => {
      ...
      const userTokenAccountB = getATAAddress(this.scope.ownerPubKey, pool.mintB, TOKEN_PROGRAM_ID25).publicKey;
      txBuilder.addInstruction({
        instructions: [
          createAssociatedTokenAccountIdempotentInstruction2(this.scope.ownerPubKey, userTokenAccountB, this.scope.ownerPubKey, pool.mintB)
        ]
      });
      txBuilder.addInstruction({
        instructions: [
          claimPlatformFee(programId, platformClaimFeeWallet, authProgramId, data.pubkey, platformId, pool.vaultB, userTokenAccountB, pool.mintB, TOKEN_PROGRAM_ID25)
        ]
      });
    });
    txBuilder.addTipInstruction(txTipConfig);
  ...
  async claimVesting({
    ...
    const userTokenAccountA = getATAAddress(this.scope.ownerPubKey, poolInfo.mintA, TOKEN_PROGRAM_ID25).publicKey;
    txBuilder.addInstruction({
      instructions: [
        createAssociatedTokenAccountIdempotentInstruction2(this.scope.ownerPubKey, userTokenAccountA, this.scope.ownerPubKey, poolInfo.mintA)
      ]
    });
    txBuilder.addInstruction({
      instructions: [
        claimVestedToken(programId, this.scope.ownerPubKey, authProgramId, poolId, vestingRecord, userTokenAccountA, poolInfo.vaultA, poolInfo.mintA, TOKEN_PROGRAM_ID25)
      ]
    });
    txBuilder.addCustomComputeBudget(computeBudgetConfig);
    txBuilder.addTipInstruction(txTipConfig);
...
import { PublicKey as PublicKey39 } from "@solana/web3.js";
import { MintLayout as MintLayout2, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID9, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID26 } from "@solana/spl-token";
var TokenModule = class extends ModuleBase {
  ...
  async load(params) {
    ...
    mintList.forEach((token) => {
      ...
      this._tokenMap.set(token.address, {
        ...
        type: "raydium",
        priority: 2,
        programId: token.programId ?? (token.tags.includes("token-2022") ? TOKEN_2022_PROGRAM_ID9.toBase58() : TOKEN_PROGRAM_ID26.toBase58())
      });
  ...
  async getTokenInfo(mint) {
    ...
    const onlineInfo = await this.scope.connection.getAccountInfo(new PublicKey39(mintStr));
    if (!onlineInfo)
      throw new Error(`mint address not found: ${mintStr}`);
    const data = MintLayout2.decode(onlineInfo.data);
...
export {
  ...
  addComputeBudget,
  addLiquidityLayout,
  anchorDataBuf4 as anchorDataBuf,
  array,
