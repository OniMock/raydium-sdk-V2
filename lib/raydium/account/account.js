...
var Account = class extends ModuleBase {
  ...
  async fetchWalletTokenAccounts(config2) {
    ...
    const [solAccountResp, ownerTokenAccountResp, ownerToken2022AccountResp] = await Promise.all([
      ...
      this.scope.connection.getTokenAccountsByOwner(this.scope.ownerPubKey, { programId: import_spl_token8.TOKEN_PROGRAM_ID }, customConfig.commitment),
      this.scope.connection.getTokenAccountsByOwner(this.scope.ownerPubKey, { programId: import_spl_token8.TOKEN_2022_PROGRAM_ID }, customConfig.commitment)
    ]);
  ...
  async getOrCreateTokenAccount(params) {
    ...
    const tokenProgram = new import_web314.PublicKey(params.tokenProgram || import_spl_token8.TOKEN_PROGRAM_ID);
    ...
    if (associatedOnly) {
      const _createATAIns = (0, import_spl_token8.createAssociatedTokenAccountInstruction)(owner, ata, owner, mint, tokenProgram);
      ...
      if (checkCreateATAOwner) {
        ...
        } else if (ataInfo.owner.equals(tokenProgram) && import_spl_token8.AccountLayout.decode(ataInfo.data).mint.equals(mint) && import_spl_token8.AccountLayout.decode(ataInfo.data).owner.equals(owner)) {
        } else {
      ...
      const balanceNeeded = await this.scope.connection.getMinimumBalanceForRentExemption(import_spl_token8.AccountLayout.span);
      const createAccountIns = import_web314.SystemProgram.createAccountWithSeed({
        ...
        space: import_spl_token8.AccountLayout.span,
        programId: tokenProgram
      });
  ...
  async checkOrCreateAta({
    ...
    if (!tokenAccountAddress) {
      ...
      const instruction = await (0, import_spl_token8.createAssociatedTokenAccountInstruction)(owner, ataAddress, owner, mint, programId);
  ...
  async handleTokenAccount(params) {
    ...
    if (new import_web314.PublicKey(WSOLMint).equals(mint)) {
      ...
      const instructions = [];
      const _createATAIns = (0, import_spl_token8.createAssociatedTokenAccountInstruction)(this.scope.ownerPubKey, ata, this.scope.ownerPubKey, mint, programId);
      if (checkCreateATAOwner) {
        const ataInfo = await this.scope.connection.getAccountInfo(ata);
        if (ataInfo === null) {
          instructions.push(_createATAIns);
        } else if (ataInfo.owner.equals(import_spl_token8.TOKEN_PROGRAM_ID) && import_spl_token8.AccountLayout.decode(ataInfo.data).mint.equals(mint) && import_spl_token8.AccountLayout.decode(ataInfo.data).owner.equals(this.scope.ownerPubKey)) {
        } else {
          throw Error(`create ata check error -> mint: ${mint.toString()}, ata: ${ata.toString()}`);
        }
      } else {
  ...
  async processTokenAccount(props) {
    const { mint, programId = import_spl_token8.TOKEN_PROGRAM_ID, amount, useSOLBalance, handleTokenAccount, feePayer } = props;
    let tokenAccount;
