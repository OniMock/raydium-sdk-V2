...
var Account = class extends ModuleBase {
  ...
  async fetchWalletTokenAccounts(config2) {
    ...
    const [solAccountResp, ownerTokenAccountResp, ownerToken2022AccountResp] = await Promise.all([
      ...
      this.scope.connection.getTokenAccountsByOwner(this.scope.ownerPubKey, { programId: TOKEN_PROGRAM_ID8 }, customConfig.commitment),
      this.scope.connection.getTokenAccountsByOwner(this.scope.ownerPubKey, { programId: TOKEN_2022_PROGRAM_ID }, customConfig.commitment)
    ]);
  ...
  async getOrCreateTokenAccount(params) {
    ...
    const tokenProgram = new PublicKey14(params.tokenProgram || TOKEN_PROGRAM_ID8);
  ...
  async handleTokenAccount(params) {
    ...
    if (new PublicKey14(WSOLMint).equals(mint)) {
      ...
      if (checkCreateATAOwner) {
        ...
        } else if (ataInfo.owner.equals(TOKEN_PROGRAM_ID8) && AccountLayout.decode(ataInfo.data).mint.equals(mint) && AccountLayout.decode(ataInfo.data).owner.equals(this.scope.ownerPubKey)) {
        } else {
          throw Error(`create ata check error -> mint: ${mint.toString()}, ata: ${ata.toString()}`);
        }
      } else {
  ...
  async processTokenAccount(props) {
    const { mint, programId = TOKEN_PROGRAM_ID8, amount, useSOLBalance, handleTokenAccount, feePayer } = props;
    let tokenAccount;
