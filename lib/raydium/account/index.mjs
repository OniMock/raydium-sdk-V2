...
function initTokenAccountInstruction(params) {
  const { mint, tokenAccount, owner, programId = TOKEN_PROGRAM_ID7 } = params;
  return createInitializeAccountInstruction(tokenAccount, mint, owner, programId);
}
function closeAccountInstruction(params) {
  const { tokenAccount, payer, multiSigners = [], owner, programId = TOKEN_PROGRAM_ID7 } = params;
...
async function createWSolAccountInstructions(params) {
  ...
  const newAccount = generatePubKey({ fromPublicKey: payer, programId: TOKEN_PROGRAM_ID7 });
  return {
