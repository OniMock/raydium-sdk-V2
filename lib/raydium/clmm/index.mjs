...
var Clmm = class extends ModuleBase {
  ...
  async createPool(props) {
    const {
      programId,
      owner = this.scope.owner?.publicKey || PublicKey15.default,
      mint1,
      mint2,
      ammConfig,
      initialPrice,
    ...
    const [mintA, mintB, initPrice] = new BN16(new PublicKey15(mint1.address).toBuffer()).gt(new BN16(new PublicKey15(mint2.address).toBuffer())) ? [mint2, mint1, new decimal_default(1).div(initialPrice)] : [mint1, mint2, initialPrice];
    const initialPriceX64 = SqrtPriceMath.priceToSqrtPriceX64(initPrice, mintA.decimals, mintB.decimals);
    const extendMintAccount = [];
    const fetchAccounts = [];
    if (mintA.programId === TOKEN_2022_PROGRAM_ID3.toBase58())
      fetchAccounts.push(getPdaMintExAccount(programId, new PublicKey15(mintA.address)).publicKey);
    if (mintB.programId === TOKEN_2022_PROGRAM_ID3.toBase58())
      fetchAccounts.push(getPdaMintExAccount(programId, new PublicKey15(mintB.address)).publicKey);
  ...
  async openPositionFromBase({
    ...
    const { account: _ownerTokenAccountA, instructionParams: _tokenAccountAInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey15(poolInfo.mintA.address),
    ...
    const { account: _ownerTokenAccountB, instructionParams: _tokenAccountBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey15(poolInfo.mintB.address),
      owner: this.scope.ownerPubKey,
  ...
  async increasePositionFromLiquidity(props) {
    ...
    const ins = ClmmInstrument.increasePositionFromLiquidityInstructions({
      ...
      amountMaxA,
      amountMaxB,
      nft2022: (await this.scope.connection.getAccountInfo(ownerPosition.nftMint))?.owner.equals(TOKEN_2022_PROGRAM_ID3)
    });
  ...
  async decreaseLiquidity(props) {
    ...
    for (const itemReward of poolInfo.rewardDefaultInfos) {
      ...
      if (itemReward.mint.address === poolInfo.mintA.address)
        ...
        const { account: _ownerRewardAccount, instructionParams: ownerRewardAccountInstructions } = await this.scope.account.getOrCreateTokenAccount({
          tokenProgram: new PublicKey15(itemReward.mint.programId),
          mint: new PublicKey15(itemReward.mint.address),
    ...
    const poolKeys = propPoolKeys ?? await this.getClmmPoolKeys(poolInfo.id);
    const nft2022 = (await this.scope.connection.getAccountInfo(ownerPosition.nftMint))?.owner.equals(TOKEN_2022_PROGRAM_ID3);
  ...
  async harvestLockPosition(props) {
    ...
    const { account: _ownerTokenAccountA, instructionParams: accountAInstructions } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey15(poolKeys.mintA.address),
      notUseTokenAccount: mintAUseSOLBalance,
      owner: this.scope.ownerPubKey,
      createInfo: {
        payer: this.scope.ownerPubKey,
    ...
    const { account: _ownerTokenAccountB, instructionParams: accountBInstructions } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey15(poolKeys.mintB.address),
      notUseTokenAccount: mintBUseSOLBalance,
      owner: this.scope.ownerPubKey,
      createInfo: {
        payer: this.scope.ownerPubKey,
    ...
    const lockNftAccount = getATAAddress(this.scope.ownerPubKey, lockData.lockNftMint, TOKEN_PROGRAM_ID8).publicKey;
    ...
    const { publicKey: tickArrayLower } = getPdaTickArrayAddress(new PublicKey15(poolKeys.programId), lockData.poolId, tickArrayLowerStartIndex);
    const { publicKey: tickArrayUpper } = getPdaTickArrayAddress(new PublicKey15(poolKeys.programId), lockData.poolId, tickArrayUpperStartIndex);
    const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(new PublicKey15(poolKeys.programId), lockData.poolId, position.tickLower, position.tickUpper);
    ...
    for (let i = 0; i < poolKeys.rewardInfos.length; i++) {
      rewardAccountsFullInfo.push({
        ...
        rewardMint: new PublicKey15(poolKeys.rewardInfos[i].mint.address)
      });
    }
    const harvestLockIns = await ClmmInstrument.harvestLockPositionInstructionV2({
      ...
      protocolPosition,
      vaultA: new PublicKey15(poolKeys.vault.A),
      vaultB: new PublicKey15(poolKeys.vault.B),
      tickArrayLower,
      tickArrayUpper,
      userVaultA: ownerTokenAccountA,
      userVaultB: ownerTokenAccountB,
      mintA: new PublicKey15(poolKeys.mintA.address),
      mintB: new PublicKey15(poolKeys.mintB.address),
  ...
  async initReward({
    ...
    const { account: ownerRewardAccount, instructionParams: ownerRewardAccountIns } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: new PublicKey15(rewardInfo.mint.address),
      mint: new PublicKey15(rewardInfo.mint.address),
      ...
      createInfo: rewardMintUseSOLBalance ? {
        ...
        amount: new BN16(new decimal_default(_baseRewardAmount.toFixed(0)).gte(_baseRewardAmount) ? _baseRewardAmount.toFixed(0) : _baseRewardAmount.add(1).toFixed(0))
      } : void 0,
    ...
    const insInfo = ClmmInstrument.initRewardInstructions({
      ...
      rewardInfo: {
        programId: new PublicKey15(rewardInfo.mint.programId),
        mint: new PublicKey15(rewardInfo.mint.address),
  ...
  async initRewards({
    ...
    for (const rewardInfo of rewardInfos) {
      ...
      const _baseRewardAmount = rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime);
      const { account: ownerRewardAccount, instructionParams: ownerRewardAccountIns } = await this.scope.account.getOrCreateTokenAccount({
        tokenProgram: new PublicKey15(rewardInfo.mint.programId),
        mint: new PublicKey15(rewardInfo.mint.address),
        notUseTokenAccount: !!rewardMintUseSOLBalance,
        skipCloseAccount: !rewardMintUseSOLBalance,
        owner: this.scope.ownerPubKey,
  ...
  async setReward({
    ...
    const { account: ownerRewardAccount, instructionParams: ownerRewardIns } = await this.scope.account.getOrCreateTokenAccount({
      ...
      createInfo: rewardMintUseSOLBalance ? {
        payer: ownerInfo.feePayer || this.scope.ownerPubKey,
        amount: new BN16(new decimal_default(rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime).toFixed(0)).gte(rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime)) ? rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime).toFixed(0) : rewardInfo.perSecond.mul(rewardInfo.endTime - rewardInfo.openTime).add(1).toFixed(0))
      } : void 0,
      associatedOnly: rewardMintUseSOLBalance ? false : associatedOnly,
      checkCreateATAOwner
    });
  ...
  async swap({
    ...
    if (!priceLimit || priceLimit.equals(new decimal_default(0))) {
      sqrtPriceLimitX64 = baseIn ? MIN_SQRT_PRICE_X64.add(new BN16(1)) : MAX_SQRT_PRICE_X64.sub(new BN16(1));
    } else {
      sqrtPriceLimitX64 = SqrtPriceMath.priceToSqrtPriceX64(priceLimit, poolInfo.mintA.decimals, poolInfo.mintB.decimals);
    }
    ...
    txBuilder.addInstruction(ClmmInstrument.makeSwapBaseInInstructions({
      ...
      inputMint: new PublicKey15(inputMint),
      amountIn,
      amountOutMin,
      sqrtPriceLimitX64,
      remainingAccounts
  ...
  async swapBaseOut({
    ...
    let sqrtPriceLimitX64;
    if (!priceLimit || priceLimit.equals(new decimal_default(0))) {
      sqrtPriceLimitX64 = outputMint.toString() === poolInfo.mintB.address ? MIN_SQRT_PRICE_X64.add(new BN16(1)) : MAX_SQRT_PRICE_X64.sub(new BN16(1));
    } else {
      sqrtPriceLimitX64 = SqrtPriceMath.priceToSqrtPriceX64(priceLimit, poolInfo.mintA.decimals, poolInfo.mintB.decimals);
    }
    let ownerTokenAccountA;
    ...
    txBuilder.addInstruction(ClmmInstrument.makeSwapBaseOutInstructions({
      ...
      outputMint: new PublicKey15(outputMint),
      amountOut,
      amountInMax,
      sqrtPriceLimitX64,
      remainingAccounts
    ...
    for (const itemInfo of Object.values(allPoolInfo)) {
      ...
      for (const itemPosition of allPositions[itemInfo.id]) {
        ...
        if (lockData) {
          ...
          const { publicKey: tickArrayLower } = getPdaTickArrayAddress(new PublicKey15(poolKeys.programId), lockData.poolId, tickArrayLowerStartIndex);
          const { publicKey: tickArrayUpper } = getPdaTickArrayAddress(new PublicKey15(poolKeys.programId), lockData.poolId, tickArrayUpperStartIndex);
          const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(new PublicKey15(poolKeys.programId), lockData.poolId, itemPosition.tickLower, itemPosition.tickUpper);
          ...
          const insData = ClmmInstrument.decreaseLiquidityInstructions({
            ...
            liquidity: new BN16(0),
            amountMinA: new BN16(0),
            amountMinB: new BN16(0),
            nft2022: record[itemPosition.nftMint.toBase58()]?.equals(TOKEN_2022_PROGRAM_ID3)
          });
  ...
  async getWhiteListMint({ programId }) {
    ...
    return whitelistMintsInfo.whitelistMints.filter((i) => !i.equals(PublicKey15.default));
  }
  async getOwnerPositionInfo({
    programId
  }) {
    await this.scope.account.fetchWalletTokenAccounts();
    const balanceMints = this.scope.account.tokenAccountRawInfos.filter((acc) => acc.accountInfo.amount.eq(new BN16(1)));
    const allPositionKey = balanceMints.map((acc) => getPdaPersonalPositionAddress(new PublicKey15(programId), acc.accountInfo.mint).publicKey);
  ...
  async getRpcClmmPoolInfos({
    ...
    const accounts = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, poolIds.map((i) => ({ pubkey: new PublicKey15(i) })), config2);
    const returnData = {};
  ...
  async getComputeClmmPoolInfos({
    ...
    const res = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, Array.from(configSet).map((s) => ({ pubkey: new PublicKey15(s) })));
    const clmmConfigs = {};
    res.forEach((acc) => {
      if (!acc.accountInfo)
        return;
      ...
      poolList: Object.keys(clmmPoolsRpcInfo).map((poolId) => {
        ...
        return {
          ...
          mintA: toApiV3Token({
            ...
            programId: mintInfos[mintA].programId.toBase58() || TOKEN_PROGRAM_ID8.toBase58(),
            extensions: {
          ...
          mintB: toApiV3Token({
            ...
            programId: mintInfos[mintB].programId.toBase58() || TOKEN_PROGRAM_ID8.toBase58(),
            extensions: {
  ...
  async getPoolInfoFromRpc(poolId) {
    ...
    const mintInfos = await fetchMultipleMintInfos({
      connection: this.scope.connection,
      mints: Array.from(mintSet).map((m) => new PublicKey15(m))
    });
    ...
    const poolKeys = {
      ...
      rewardInfos: computeClmmPoolInfo[poolId].rewardInfos.filter((r) => !r.tokenVault.equals(PublicKey15.default)).map((r) => ({
        mint: toApiV3Token({ address: r.tokenMint.toBase58(), programId: TOKEN_PROGRAM_ID8.toBase58(), decimals: 10 }),
