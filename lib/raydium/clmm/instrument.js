...
var PoolUtils = class {
  ...
  static async updatePoolRewardInfos({
    ...
    for (let i = 0; i < rewardInfos.length; i++) {
      ...
      const itemReward = {
        ...
        tokenProgramId: new import_web312.PublicKey(apiRewardProgram)
      };
      if (itemReward.tokenMint.equals(import_web312.PublicKey.default))
        continue;
  ...
  static computeAmountOutFormat({
    ...
    const [baseToken, outToken] = [
      new Token({
        ...
        isToken2022: inputMint.programId === import_spl_token6.TOKEN_2022_PROGRAM_ID.toBase58()
      }),
      new Token({
        ...outMint,
        mint: outMint.address,
        isToken2022: outMint.programId === import_spl_token6.TOKEN_2022_PROGRAM_ID.toBase58()
      })
    ];
    const {
      allTrade,
      ...
      poolInfo,
      tickArrayCache,
      baseMint: new import_web312.PublicKey(inputMint.address),
      amountIn,
      slippage,
      epochInfo,
  ...
  static async fetchComputeMultipleClmmInfo({
    ...
    const fetchRpcList = poolList.filter((p) => !rpcDataMap[p.id]).map((p) => new import_web312.PublicKey(p.id));
    ...
    const pdaList = poolList.map((poolInfo) => getPdaExBitmapAccount(new import_web312.PublicKey(poolInfo.programId), new import_web312.PublicKey(poolInfo.id)).publicKey);
    ...
    return poolList.reduce((acc, cur) => ({
      ...
      [cur.id]: {
        ...rpcDataMap[cur.id],
        id: new import_web312.PublicKey(cur.id),
        version: 6,
        programId: new import_web312.PublicKey(cur.programId),
        mintA: cur.mintA,
        mintB: cur.mintB,
        ammConfig: {
          ...cur.config,
          id: new import_web312.PublicKey(cur.config.id),
          fundOwner: ""
        },
        currentPrice: new decimal_default(cur.price),
        exBitmapAccount: getPdaExBitmapAccount(new import_web312.PublicKey(cur.programId), new import_web312.PublicKey(cur.id)).publicKey,
        exBitmapInfo: exBitData[getPdaExBitmapAccount(new import_web312.PublicKey(cur.programId), new import_web312.PublicKey(cur.id)).publicKey.toBase58()],
...
var ClmmInstrument = class {
  static createPoolInstruction(programId, poolId, poolCreator, ammConfigId, observationId, mintA, mintVaultA, mintProgramIdA, mintB, mintVaultB, mintProgramIdB, exTickArrayBitmap, sqrtPriceX64, extendMintAccount) {
    ...
    const keys = [
      ...
      { pubkey: import_web313.SystemProgram.programId, isSigner: false, isWritable: false },
    ...
    return new import_web313.TransactionInstruction({
      keys,
      programId,
      data: aData
    });
  ...
  static async createPoolInstructions(props) {
    ...
    const [mintAAddress, mintBAddress] = [new import_web313.PublicKey(mintA.address), new import_web313.PublicKey(mintB.address)];
    ...
    const ins = [
      this.createPoolInstruction(programId, poolId, owner, ammConfigId, observationId, mintAAddress, mintAVault, new import_web313.PublicKey(mintA.programId || import_spl_token7.TOKEN_PROGRAM_ID), mintBAddress, mintBVault, new import_web313.PublicKey(mintB.programId || import_spl_token7.TOKEN_PROGRAM_ID), exBitmapAccount, initialPriceX64, extendMintAccount)
    ];
    return {
      signers: [],
      instructions: ins,
  ...
  static async openPositionInstructions({
    ...
    const signers = [];
    const [programId, id] = [new import_web313.PublicKey(poolInfo.programId), new import_web313.PublicKey(poolInfo.id)];
    let nftMintAccount;
    if (getEphemeralSigners) {
      nftMintAccount = new import_web313.PublicKey((await getEphemeralSigners(1))[0]);
    } else {
      const _k = import_web313.Keypair.generate();
      signers.push(_k);
    ...
    const { publicKey: positionNftAccount } = nft2022 ? getATAAddress(ownerInfo.wallet, nftMintAccount, import_spl_token7.TOKEN_2022_PROGRAM_ID) : getATAAddress(ownerInfo.wallet, nftMintAccount, import_spl_token7.TOKEN_PROGRAM_ID);
    ...
    const ins = nft2022 ? this.openPositionFromLiquidityInstruction22(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0) : this.openPositionFromLiquidityInstruction(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
  ...
  static async openPositionFromBaseInstructions({
    ...
    const { publicKey: personalPosition } = getPdaPersonalPositionAddress(programId, nftMintAccount);
    const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(programId, id, tickLower, tickUpper);
    const ins = nft2022 ? this.openPositionFromBaseInstruction22(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, withMetadata, base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0) : this.openPositionFromBaseInstruction(programId, ownerInfo.feePayer, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, withMetadata, base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0);
    return {
  ...
  static async openPositionFromLiquidityInstructions({
    ...
    const { publicKey: personalPosition } = getPdaPersonalPositionAddress(programId, nftMintAccount);
    const { publicKey: protocolPosition } = getPdaProtocolPositionAddress(programId, id, tickLower, tickUpper);
    const ins = nft2022 ? this.openPositionFromLiquidityInstruction22(programId, ownerInfo.wallet, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolKeys.mintA.address), new import_web313.PublicKey(poolKeys.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0) : this.openPositionFromLiquidityInstruction(programId, ownerInfo.wallet, id, ownerInfo.wallet, nftMintAccount, positionNftAccount, metadataAccount, protocolPosition, tickArrayLower, tickArrayUpper, personalPosition, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolKeys.mintA.address), new import_web313.PublicKey(poolKeys.mintB.address), tickLower, tickUpper, tickArrayLowerStartIndex, tickArrayUpperStartIndex, liquidity, amountMaxA, amountMaxB, withMetadata, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0);
    return {
  ...
  static closePositionInstruction(programId, positionNftOwner, positionNftMint, positionNftAccount, personalPosition, nft2022) {
    ...
    const keys = [
      ...
      { pubkey: personalPosition, isSigner: false, isWritable: true },
      { pubkey: import_web313.SystemProgram.programId, isSigner: false, isWritable: false },
      { pubkey: nft2022 ? import_spl_token7.TOKEN_2022_PROGRAM_ID : import_spl_token7.TOKEN_PROGRAM_ID, isSigner: false, isWritable: false }
    ];
    const data = Buffer.alloc(dataLayout.span);
  ...
  static closePositionInstructions({
    ...
    const programId = new import_web313.PublicKey(poolInfo.programId);
    const positionNftAccount = nft2022 ? getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, import_spl_token7.TOKEN_2022_PROGRAM_ID).publicKey : getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, import_spl_token7.TOKEN_PROGRAM_ID).publicKey;
  ...
  static increasePositionFromLiquidityInstructions({
    ...
    const { publicKey: tickArrayLower } = getPdaTickArrayAddress(programId, id, tickArrayLowerStartIndex);
    const { publicKey: tickArrayUpper } = getPdaTickArrayAddress(programId, id, tickArrayUpperStartIndex);
    const { publicKey: positionNftAccount } = nft2022 ? getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, import_spl_token7.TOKEN_2022_PROGRAM_ID) : getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, import_spl_token7.TOKEN_PROGRAM_ID);
    const { publicKey: personalPosition } = getPdaPersonalPositionAddress(programId, ownerPosition.nftMint);
    ...
    const ins = this.increasePositionFromLiquidityInstruction(programId, ownerInfo.wallet, positionNftAccount, personalPosition, id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address), liquidity, amountMaxA, amountMaxB, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
    ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0);
    return {
  ...
  static increasePositionFromBaseInstructions({
    ...
    return {
      ...
      instructions: [
        this.increasePositionFromBaseInstruction(programId, ownerInfo.wallet, positionNftAccount, personalPosition, id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address), base, baseAmount, otherAmountMax, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
          tickArrayLowerStartIndex,
          tickArrayUpperStartIndex
        ]) ? getPdaExBitmapAccount(programId, id).publicKey : void 0)
      ],
  ...
  static decreaseLiquidityInstructions({
    ...
    const [poolProgramId, id] = [new import_web313.PublicKey(poolInfo.programId), new import_web313.PublicKey(poolInfo.id)];
    const tickArrayLowerStartIndex = TickUtils.getTickArrayStartIndexByTick(ownerPosition.tickLower, poolInfo.config.tickSpacing);
    const tickArrayUpperStartIndex = TickUtils.getTickArrayStartIndexByTick(ownerPosition.tickUpper, poolInfo.config.tickSpacing);
    ...
    const { publicKey: positionNftAccount } = nft2022 ? getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, import_spl_token7.TOKEN_2022_PROGRAM_ID) : getATAAddress(ownerInfo.wallet, ownerPosition.nftMint, programId);
    ...
    for (let i = 0; i < poolInfo.rewardDefaultInfos.length; i++) {
      rewardAccounts.push({
        poolRewardVault: new import_web313.PublicKey(poolKeys.rewardInfos[i].vault),
        ownerRewardVault: ownerInfo.rewardAccounts[i],
        rewardMint: new import_web313.PublicKey(poolInfo.rewardDefaultInfos[i].mint.address)
      });
    }
    const ins = [];
    const decreaseIns = this.decreaseLiquidityInstruction(poolProgramId, ownerInfo.wallet, positionNftAccount, personalPosition, id, protocolPosition, tickArrayLower, tickArrayUpper, ownerInfo.tokenAccountA, ownerInfo.tokenAccountB, new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B), new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address), rewardAccounts, liquidity, amountMinA, amountMinB, PoolUtils.isOverflowDefaultTickarrayBitmap(poolInfo.config.tickSpacing, [
      tickArrayLowerStartIndex,
      tickArrayUpperStartIndex
  ...
  static makeSwapBaseInInstructions({
    ...
    const [programId, id] = [new import_web313.PublicKey(poolInfo.programId), new import_web313.PublicKey(poolInfo.id)];
    const [mintAVault, mintBVault] = [new import_web313.PublicKey(poolKeys.vault.A), new import_web313.PublicKey(poolKeys.vault.B)];
    const [mintA, mintB] = [new import_web313.PublicKey(poolInfo.mintA.address), new import_web313.PublicKey(poolInfo.mintB.address)];
    const isInputMintA = poolInfo.mintA.address === inputMint.toString();
    const ins = [
      this.swapInstruction(programId, ownerInfo.wallet, id, new import_web313.PublicKey(poolInfo.config.id), isInputMintA ? ownerInfo.tokenAccountA : ownerInfo.tokenAccountB, isInputMintA ? ownerInfo.tokenAccountB : ownerInfo.tokenAccountA, isInputMintA ? mintAVault : mintBVault, isInputMintA ? mintBVault : mintAVault, isInputMintA ? mintA : mintB, isInputMintA ? mintB : mintA, remainingAccounts, observationId, amountIn, amountOutMin, sqrtPriceLimitX64, true, getPdaExBitmapAccount(programId, id).publicKey)
    ];
    return {
      signers: [],
      instructions: ins,
  ...
  static initRewardInstructions({
    ...
    const ins = [
      this.initRewardInstruction(programId, ownerInfo.wallet, id, operationId, new import_web313.PublicKey(poolInfo.config.id), ownerInfo.tokenAccount, rewardInfo.programId, rewardInfo.mint, poolRewardVault, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64)
    ];
    return {
  ...
  static setRewardInstructions({
    ...
    for (let index = 0; index < poolInfo.rewardDefaultInfos.length; index++)
      if (poolInfo.rewardDefaultInfos[index].mint.address === rewardInfo.mint.toString()) {
        rewardIndex = index;
        rewardVault = new import_web313.PublicKey(poolKeys.rewardInfos[index].vault);
        rewardMint = new import_web313.PublicKey(poolKeys.rewardInfos[index].mint.address);
      }
    ...
    const operationId = getPdaOperationAccount(programId).publicKey;
    const ins = [
      this.setRewardInstruction(programId, ownerInfo.wallet, id, operationId, new import_web313.PublicKey(poolInfo.config.id), ownerInfo.tokenAccount, rewardVault, rewardMint, rewardIndex, rewardInfo.openTime, rewardInfo.endTime, rewardInfo.emissionsPerSecondX64)
    ];
    return {
  ...
  static async makeLockPositions({
    ...
    let lockNftMint;
    if (getEphemeralSigners) {
      lockNftMint = new import_web313.PublicKey((await getEphemeralSigners(1))[0]);
    } else {
      const _k = import_web313.Keypair.generate();
      signers.push(_k);
      lockNftMint = _k.publicKey;
    }
    const positionNftAccount = nft2022 ? getATAAddress(wallet, nftMint, import_spl_token7.TOKEN_2022_PROGRAM_ID).publicKey : getATAAddress(wallet, nftMint, import_spl_token7.TOKEN_PROGRAM_ID).publicKey;
    ...
    const lockNftAccount = getATAAddress(wallet, lockNftMint, import_spl_token7.TOKEN_PROGRAM_ID).publicKey;
    ...
    const ins = ClmmInstrument.lockPositionInstructionV2({
      ...
      authPositionNftAccount: getATAAddress(authProgramId, nftMint, nft2022 ? import_spl_token7.TOKEN_2022_PROGRAM_ID : import_spl_token7.TOKEN_PROGRAM_ID).publicKey,
      positionNftProgram: nft2022 ? import_spl_token7.TOKEN_2022_PROGRAM_ID : import_spl_token7.TOKEN_PROGRAM_ID
    });
    return {
      address: {
        positionId,
  ...
  static lockPositionInstruction({
    ...
    const { publicKey: nftAccount } = getATAAddress(owner, positionNft, import_spl_token7.TOKEN_PROGRAM_ID);
  ...
  static harvestLockPositionInstruction(props) {
    const [poolProgramId, poolId] = [new import_web313.PublicKey(props.poolKeys.programId), new import_web313.PublicKey(props.poolKeys.id)];
    ...
    const { publicKey: nftAccount } = getATAAddress(props.owner, props.ownerPosition.nftMint, import_spl_token7.TOKEN_PROGRAM_ID);
    ...
    for (let i = 0; i < props.poolKeys.rewardInfos.length; i++) {
      rewardAccounts.push({
        poolRewardVault: new import_web313.PublicKey(props.poolKeys.rewardInfos[i].vault),
        ownerRewardVault: props.ownerRewardAccounts[i],
        rewardMint: new import_web313.PublicKey(props.poolKeys.rewardInfos[i].mint.address)
      });
    }
    const remainingAccounts = [
      ...rewardAccounts.map((i) => [
    ...
    const keys = [
      ...
      { pubkey: poolId, isSigner: false, isWritable: true },
      { pubkey: protocolPosition, isSigner: false, isWritable: true },
      { pubkey: new import_web313.PublicKey(props.poolKeys.vault.A), isSigner: false, isWritable: true },
      { pubkey: new import_web313.PublicKey(props.poolKeys.vault.B), isSigner: false, isWritable: true },
      { pubkey: tickArrayLower, isSigner: false, isWritable: true },
      { pubkey: tickArrayUpper, isSigner: false, isWritable: true },
      ...
      { pubkey: new import_web313.PublicKey(props.poolKeys.mintA.address), isSigner: false, isWritable: false },
      { pubkey: new import_web313.PublicKey(props.poolKeys.mintB.address), isSigner: false, isWritable: false },
      ...remainingAccounts
    ];
    return new import_web313.TransactionInstruction({
      keys,
