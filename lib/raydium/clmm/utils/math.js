...
function BNDivCeil(bn1, bn2) {
  ...
  if (mod2.gt(new import_bn.default(0))) {
    return div2.add(new import_bn.default(1));
  } else {
    return div2;
  }
}
...
var TokenAmount = class extends Fraction {
  ...
  constructor(token, amount, isRaw = true, name) {
    ...
    const multiplier = BN_TEN.pow(new import_bn5.default(token.decimals));
    if (isRaw) {
...
var _100_PERCENT = new Fraction(new import_bn6.default(100));
...
var BNLayout = class extends Layout {
  ...
  decode(b, offset2 = 0) {
    const num = new import_bn10.default(this.blob.decode(b, offset2), 10, "le");
    if (this.signed) {
  ...
  encode(src, b, offset2 = 0) {
    if (typeof src === "number")
      src = new import_bn10.default(src);
    if (this.signed) {
...
function seq2(elementLayout, count, property) {
  let parsedCount;
  const superCount = typeof count === "number" ? count : (0, import_bn10.isBN)(count) ? count.toNumber() : new Proxy(count, {
    get(target, property2) {
      if (!parsedCount) {
        const countProperty = Reflect.get(target, "count");
        parsedCount = (0, import_bn10.isBN)(countProperty) ? countProperty.toNumber() : countProperty;
...
var PositionUtils = class {
  static getfeeGrowthInside(poolState, tickLowerState, tickUpperState) {
    let feeGrowthBelowX64A = new import_bn12.default(0);
    let feeGrowthBelowX64B = new import_bn12.default(0);
    ...
    let feeGrowthAboveX64A = new import_bn12.default(0);
    let feeGrowthAboveX64B = new import_bn12.default(0);
  ...
  static getRewardGrowthInside(tickCurrentIndex, tickLowerState, tickUpperState, rewardInfos) {
    ...
    for (let i = 0; i < rewardInfos.length; i++) {
      let rewardGrowthsBelow = new import_bn12.default(0);
      ...
      let rewardGrowthsAbove = new import_bn12.default(0);
  ...
  static getAmountsFromLiquidity({
    ...
    const [amountSlippageA, amountSlippageB] = [
      getTransferAmountFeeV2(new import_bn12.default(new decimal_default(amounts.amountA.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintA.extensions?.feeConfig, epochInfo, true),
      getTransferAmountFeeV2(new import_bn12.default(new decimal_default(amounts.amountB.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintB.extensions?.feeConfig, epochInfo, true)
    ];
    return {
      liquidity,
      amountA,
...
var PoolUtils = class {
  ...
  static async updatePoolRewardInfos({
    ...
    for (let i = 0; i < rewardInfos.length; i++) {
      ...
      const latestUpdateTime = new import_bn13.default(Math.min(itemReward.endTime.toNumber(), chainTime));
        ...
        poolInfoA.positionAccount = [
          ...poolInfoA.positionAccount ?? [],
          {
            ...
            rewardInfos: position.rewardInfos.map((i) => ({
              ...i,
              pendingReward: new import_bn13.default(0)
            })),
            leverage,
            tokenFeeAmountA: new import_bn13.default(0),
            tokenFeeAmountB: new import_bn13.default(0)
          }
        ];
      ...
      if (updateOwnerRewardAndFee) {
        ...
        for (const { state, positionAccount } of pools) {
          ...
          for (const itemPA of positionAccount) {
            ...
            itemPA.tokenFeeAmountA = tokenFeeAmountA.gte(new import_bn13.default(0)) ? tokenFeeAmountA : new import_bn13.default(0);
            itemPA.tokenFeeAmountB = tokenFeeAmountB.gte(new import_bn13.default(0)) ? tokenFeeAmountB : new import_bn13.default(0);
            for (let i = 0; i < rewardInfos.length; i++) {
              itemPA.rewardInfos[i].pendingReward = rewardInfos[i].gte(new import_bn13.default(0)) ? rewardInfos[i] : new import_bn13.default(0);
            }
          }
        }
      }
  ...
  static computeAmountOut({
    ...
    if (priceLimit.equals(new decimal_default(0))) {
      sqrtPriceLimitX64 = isBaseIn ? MIN_SQRT_PRICE_X64.add(new import_bn13.default(1)) : MAX_SQRT_PRICE_X64.sub(new import_bn13.default(1));
    } else {
    ...
    const _minAmountOut = _expectedAmountOut.mul(new import_bn13.default(Math.floor((1 - slippage) * 1e10))).div(new import_bn13.default(1e10));
  ...
  static computeAmountOutFormat({
    ...
    const _currentPrice = new Price({
      baseToken,
      denominator: new import_bn13.default(10).pow(new import_bn13.default(20 + baseToken.decimals)),
      quoteToken: outToken,
  ...
  static computeAmountIn({
    ...
    const _executionPrice = SqrtPriceMath.sqrtPriceX64ToPrice(_executionPriceX64, poolInfo.mintA.decimals, poolInfo.mintB.decimals);
    const executionPrice = isBaseIn ? _executionPrice : new decimal_default(1).div(_executionPrice);
    const _maxAmountIn = _expectedAmountIn.mul(new import_bn13.default(Math.floor((1 + slippage) * 1e10))).div(new import_bn13.default(1e10));
  ...
  static async getLiquidityAmountOutFromAmountIn({
    ...
    const _amount = new import_bn13.default(new decimal_default(addFeeAmount.amount.sub(addFeeAmount.fee ?? ZERO).toString()).toFixed(0));
    let liquidity;
    if (sqrtPriceX64.lte(sqrtPriceX64A)) {
      liquidity = inputA ? LiquidityMath.getLiquidityFromTokenAmountA(sqrtPriceX64A, sqrtPriceX64B, _amount, !add2) : new import_bn13.default(0);
      ...
      liquidity = inputA ? new import_bn13.default(0) : LiquidityMath.getLiquidityFromTokenAmountB(sqrtPriceX64A, sqrtPriceX64B, _amount);
    }
...
var MathUtil = class {
  ...
  static decimalToX64(num) {
    return new import_bn14.default(num.mul(decimal_default.pow(2, 64)).floor().toFixed());
  }
...
var SqrtPriceMath = class {
  ...
  static getSqrtPriceX64FromTick(tick) {
    ...
    if ((tickAbs & 2) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18444899583751176192"));
    if ((tickAbs & 4) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18443055278223355904"));
    if ((tickAbs & 8) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18439367220385607680"));
    if ((tickAbs & 16) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18431993317065453568"));
    if ((tickAbs & 32) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18417254355718170624"));
    if ((tickAbs & 64) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18387811781193609216"));
    if ((tickAbs & 128) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18329067761203558400"));
    if ((tickAbs & 256) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("18212142134806163456"));
    if ((tickAbs & 512) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("17980523815641700352"));
    if ((tickAbs & 1024) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("17526086738831433728"));
    if ((tickAbs & 2048) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("16651378430235570176"));
    if ((tickAbs & 4096) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("15030750278694412288"));
    if ((tickAbs & 8192) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("12247334978884435968"));
    if ((tickAbs & 16384) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("8131365268886854656"));
    if ((tickAbs & 32768) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("3584323654725218816"));
    if ((tickAbs & 65536) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("696457651848324352"));
    if ((tickAbs & 131072) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("26294789957507116"));
    if ((tickAbs & 262144) != 0)
      ratio = mulRightShift(ratio, new import_bn14.default("37481735321082"));
    if (tick > 0)
...
var LiquidityMath = class {
  ...
  static getAmountsFromLiquidityWithSlippage(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, amountMax, roundUp, amountSlippage) {
    ...
    const amount0Slippage = new import_bn14.default(new decimal_default(amountA.toString()).mul(coefficient).toFixed(0));
    const amount1Slippage = new import_bn14.default(new decimal_default(amountB.toString()).mul(coefficient).toFixed(0));
    return {
  ...
  static getAmountsOutFromLiquidity({
    ...
    const [amountSlippageA, amountSlippageB] = [
      getTransferAmountFeeV2(new import_bn14.default(new decimal_default(amounts.amountA.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintA.extensions?.feeConfig, epochInfo, amountAddFee),
      getTransferAmountFeeV2(new import_bn14.default(new decimal_default(amounts.amountB.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintB.extensions?.feeConfig, epochInfo, amountAddFee)
    ];
    return {
      liquidity,
      amountA,
...
var SwapMath = class {
  ...
  static swapStepCompute(sqrtPriceX64Current, sqrtPriceX64Target, liquidity, amountRemaining, feeRate, zeroForOne) {
    ...
    if (baseInput) {
      const amountRemainingSubtractFee = MathUtil.mulDivFloor(amountRemaining, FEE_RATE_DENOMINATOR.sub(new import_bn14.default(feeRate.toString())), FEE_RATE_DENOMINATOR);
    ...
    if (baseInput && !swapStep.sqrtPriceX64Next.eq(sqrtPriceX64Target)) {
      ...
      swapStep.feeAmount = MathUtil.mulDivCeil(swapStep.amountIn, new import_bn14.default(feeRate), FEE_RATE_DENOMINATOR.sub(new import_bn14.default(feeRate)));
    }
