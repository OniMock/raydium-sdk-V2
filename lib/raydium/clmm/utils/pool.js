...
var MathUtil = class {
  ...
  static decimalToX64(num) {
    return new import_bn11.default(num.mul(decimal_default.pow(2, 64)).floor().toFixed());
  }
...
var SqrtPriceMath = class {
  ...
  static getSqrtPriceX64FromTick(tick) {
    ...
    let ratio = (tickAbs & 1) != 0 ? new import_bn11.default("18445821805675395072") : new import_bn11.default("18446744073709551616");
    if ((tickAbs & 2) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18444899583751176192"));
    if ((tickAbs & 4) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18443055278223355904"));
    if ((tickAbs & 8) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18439367220385607680"));
    if ((tickAbs & 16) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18431993317065453568"));
    if ((tickAbs & 32) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18417254355718170624"));
    if ((tickAbs & 64) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18387811781193609216"));
    if ((tickAbs & 128) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18329067761203558400"));
    if ((tickAbs & 256) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("18212142134806163456"));
    if ((tickAbs & 512) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("17980523815641700352"));
    if ((tickAbs & 1024) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("17526086738831433728"));
    if ((tickAbs & 2048) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("16651378430235570176"));
    if ((tickAbs & 4096) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("15030750278694412288"));
    if ((tickAbs & 8192) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("12247334978884435968"));
    if ((tickAbs & 16384) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("8131365268886854656"));
    if ((tickAbs & 32768) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("3584323654725218816"));
    if ((tickAbs & 65536) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("696457651848324352"));
    if ((tickAbs & 131072) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("26294789957507116"));
    if ((tickAbs & 262144) != 0)
      ratio = mulRightShift(ratio, new import_bn11.default("37481735321082"));
    if (tick > 0)
  ...
  static getTickFromSqrtPriceX64(sqrtPriceX64) {
    ...
    const adjustedMsb = new import_bn11.default(msb - 64);
    const log2pIntegerX32 = signedLeftShift(adjustedMsb, 32, 128);
    let bit = new import_bn11.default("8000000000000000", "hex");
    let precision = 0;
    let log2pFractionX64 = new import_bn11.default(0);
    let r = msb >= 64 ? sqrtPriceX64.shrn(msb - 63) : sqrtPriceX64.shln(63 - msb);
    while (bit.gt(new import_bn11.default(0)) && precision < BIT_PRECISION) {
      r = r.mul(r);
    ...
    const logbpX64 = log2pX32.mul(new import_bn11.default(LOG_B_2_X32));
    const tickLow = signedRightShift(logbpX64.sub(new import_bn11.default(LOG_B_P_ERR_MARGIN_LOWER_X64)), 64, 128).toNumber();
    const tickHigh = signedRightShift(logbpX64.add(new import_bn11.default(LOG_B_P_ERR_MARGIN_UPPER_X64)), 64, 128).toNumber();
    if (tickLow == tickHigh) {
      return tickLow;
    } else {
...
var LiquidityMath = class {
  ...
  static getAmountsFromLiquidity(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, roundUp) {
    ...
    if (sqrtPriceCurrentX64.lte(sqrtPriceX64A)) {
      return {
        ...
        amountB: new import_bn11.default(0)
      };
    } else if (sqrtPriceCurrentX64.lt(sqrtPriceX64B)) {
      ...
      return {
        amountA: new import_bn11.default(0),
  ...
  static getAmountsFromLiquidityWithSlippage(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, amountMax, roundUp, amountSlippage) {
    ...
    const amount0Slippage = new import_bn11.default(new decimal_default(amountA.toString()).mul(coefficient).toFixed(0));
    const amount1Slippage = new import_bn11.default(new decimal_default(amountB.toString()).mul(coefficient).toFixed(0));
    return {
  ...
  static getAmountsOutFromLiquidity({
    ...
    const [amountSlippageA, amountSlippageB] = [
      getTransferAmountFeeV2(new import_bn11.default(new decimal_default(amounts.amountA.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintA.extensions?.feeConfig, epochInfo, amountAddFee),
      getTransferAmountFeeV2(new import_bn11.default(new decimal_default(amounts.amountB.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintB.extensions?.feeConfig, epochInfo, amountAddFee)
    ];
    return {
      liquidity,
      amountA,
...
var SwapMath = class {
  static swapCompute(programId, poolId, tickArrayCache, tickArrayBitmap, tickarrayBitmapExtension, zeroForOne, fee, liquidity, currentTick, tickSpacing, currentSqrtPriceX64, amountSpecified, lastSavedTickArrayStartIndex, sqrtPriceLimitX64, catchLiquidityInsufficient = false) {
    ...
    const state = {
      ...
      accounts: [],
      liquidity,
      feeAmount: new import_bn11.default(0)
    };
  ...
  static swapStepCompute(sqrtPriceX64Current, sqrtPriceX64Target, liquidity, amountRemaining, feeRate, zeroForOne) {
    const swapStep = {
      sqrtPriceX64Next: new import_bn11.default(0),
      amountIn: new import_bn11.default(0),
      amountOut: new import_bn11.default(0),
      feeAmount: new import_bn11.default(0)
    };
    const baseInput = amountRemaining.gte(ZERO);
    if (baseInput) {
      const amountRemainingSubtractFee = MathUtil.mulDivFloor(amountRemaining, FEE_RATE_DENOMINATOR.sub(new import_bn11.default(feeRate.toString())), FEE_RATE_DENOMINATOR);
    ...
    if (baseInput && !swapStep.sqrtPriceX64Next.eq(sqrtPriceX64Target)) {
      ...
      swapStep.feeAmount = MathUtil.mulDivCeil(swapStep.amountIn, new import_bn11.default(feeRate), FEE_RATE_DENOMINATOR.sub(new import_bn11.default(feeRate)));
    }
