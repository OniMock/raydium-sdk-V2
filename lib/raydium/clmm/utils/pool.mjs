...
var MathUtil = class {
  ...
  static decimalToX64(num) {
    return new BN11(num.mul(decimal_default.pow(2, 64)).floor().toFixed());
  }
...
var SqrtPriceMath = class {
  ...
  static getSqrtPriceX64FromTick(tick) {
    ...
    let ratio = (tickAbs & 1) != 0 ? new BN11("18445821805675395072") : new BN11("18446744073709551616");
    if ((tickAbs & 2) != 0)
      ratio = mulRightShift(ratio, new BN11("18444899583751176192"));
    if ((tickAbs & 4) != 0)
      ratio = mulRightShift(ratio, new BN11("18443055278223355904"));
    if ((tickAbs & 8) != 0)
      ratio = mulRightShift(ratio, new BN11("18439367220385607680"));
    if ((tickAbs & 16) != 0)
      ratio = mulRightShift(ratio, new BN11("18431993317065453568"));
    if ((tickAbs & 32) != 0)
      ratio = mulRightShift(ratio, new BN11("18417254355718170624"));
    if ((tickAbs & 64) != 0)
      ratio = mulRightShift(ratio, new BN11("18387811781193609216"));
    if ((tickAbs & 128) != 0)
      ratio = mulRightShift(ratio, new BN11("18329067761203558400"));
    if ((tickAbs & 256) != 0)
      ratio = mulRightShift(ratio, new BN11("18212142134806163456"));
    if ((tickAbs & 512) != 0)
      ratio = mulRightShift(ratio, new BN11("17980523815641700352"));
    if ((tickAbs & 1024) != 0)
      ratio = mulRightShift(ratio, new BN11("17526086738831433728"));
    if ((tickAbs & 2048) != 0)
      ratio = mulRightShift(ratio, new BN11("16651378430235570176"));
    if ((tickAbs & 4096) != 0)
      ratio = mulRightShift(ratio, new BN11("15030750278694412288"));
    if ((tickAbs & 8192) != 0)
      ratio = mulRightShift(ratio, new BN11("12247334978884435968"));
    if ((tickAbs & 16384) != 0)
      ratio = mulRightShift(ratio, new BN11("8131365268886854656"));
    if ((tickAbs & 32768) != 0)
      ratio = mulRightShift(ratio, new BN11("3584323654725218816"));
    if ((tickAbs & 65536) != 0)
      ratio = mulRightShift(ratio, new BN11("696457651848324352"));
    if ((tickAbs & 131072) != 0)
      ratio = mulRightShift(ratio, new BN11("26294789957507116"));
    if ((tickAbs & 262144) != 0)
      ratio = mulRightShift(ratio, new BN11("37481735321082"));
    if (tick > 0)
  ...
  static getTickFromSqrtPriceX64(sqrtPriceX64) {
    ...
    const adjustedMsb = new BN11(msb - 64);
    const log2pIntegerX32 = signedLeftShift(adjustedMsb, 32, 128);
    let bit = new BN11("8000000000000000", "hex");
    let precision = 0;
    let log2pFractionX64 = new BN11(0);
    let r = msb >= 64 ? sqrtPriceX64.shrn(msb - 63) : sqrtPriceX64.shln(63 - msb);
    while (bit.gt(new BN11(0)) && precision < BIT_PRECISION) {
      r = r.mul(r);
    ...
    const logbpX64 = log2pX32.mul(new BN11(LOG_B_2_X32));
    const tickLow = signedRightShift(logbpX64.sub(new BN11(LOG_B_P_ERR_MARGIN_LOWER_X64)), 64, 128).toNumber();
    const tickHigh = signedRightShift(logbpX64.add(new BN11(LOG_B_P_ERR_MARGIN_UPPER_X64)), 64, 128).toNumber();
    if (tickLow == tickHigh) {
      return tickLow;
    } else {
...
var LiquidityMath = class {
  ...
  static getAmountsFromLiquidityWithSlippage(sqrtPriceCurrentX64, sqrtPriceX64A, sqrtPriceX64B, liquidity, amountMax, roundUp, amountSlippage) {
    ...
    const amount0Slippage = new BN11(new decimal_default(amountA.toString()).mul(coefficient).toFixed(0));
    const amount1Slippage = new BN11(new decimal_default(amountB.toString()).mul(coefficient).toFixed(0));
    return {
  ...
  static getAmountsOutFromLiquidity({
    ...
    const [amountSlippageA, amountSlippageB] = [
      getTransferAmountFeeV2(new BN11(new decimal_default(amounts.amountA.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintA.extensions?.feeConfig, epochInfo, amountAddFee),
      getTransferAmountFeeV2(new BN11(new decimal_default(amounts.amountB.toString()).mul(coefficientRe).toFixed(0)), poolInfo.mintB.extensions?.feeConfig, epochInfo, amountAddFee)
    ];
    return {
      liquidity,
      amountA,
...
var SwapMath = class {
  ...
  static swapStepCompute(sqrtPriceX64Current, sqrtPriceX64Target, liquidity, amountRemaining, feeRate, zeroForOne) {
    ...
    if (baseInput) {
      const amountRemainingSubtractFee = MathUtil.mulDivFloor(amountRemaining, FEE_RATE_DENOMINATOR.sub(new BN11(feeRate.toString())), FEE_RATE_DENOMINATOR);
    ...
    if (baseInput && !swapStep.sqrtPriceX64Next.eq(sqrtPriceX64Target)) {
      ...
      swapStep.feeAmount = MathUtil.mulDivCeil(swapStep.amountIn, new BN11(feeRate), FEE_RATE_DENOMINATOR.sub(new BN11(feeRate)));
    }
