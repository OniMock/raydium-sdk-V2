...
module.exports = __toCommonJS(cpmm_exports);
...
function createLogger(moduleName) {
  let logger8 = (0, import_lodash.get)(moduleLoggers, moduleName);
  if (!logger8) {
    const logLevel = (0, import_lodash.get)(moduleLevels, moduleName);
    logger8 = new Logger({ name: moduleName, logLevel });
    (0, import_lodash.set)(moduleLoggers, moduleName, logger8);
  }
  return logger8;
}
...
function parseBigNumberish(value) {
  const logger8 = createLogger("Raydium_parseBigNumberish");
  ...
  if (typeof value === "string") {
    ...
    logger8.logWithError(`invalid BigNumberish string: ${value}`);
  }
  if (typeof value === "number") {
    if (value % 1) {
      logger8.logWithError(`BigNumberish number underflow: ${value}`);
    }
    if (value >= MAX_SAFE || value <= -MAX_SAFE) {
      logger8.logWithError(`BigNumberish number overflow: ${value}`);
    }
  ...
  logger8.error(`invalid BigNumberish value: ${value}`);
...
function getATAAddress(owner, mint, programId) {
  return findProgramAddress([owner.toBuffer(), (programId ?? import_spl_token5.TOKEN_PROGRAM_ID).toBuffer(), mint.toBuffer()], new import_web36.PublicKey("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL"));
}
...
function getTransferAmountFeeV2(amount, _feeConfig, epochInfo, addFee) {
  ...
  const maxFee = new import_bn4.default(nowFeeConfig.maximumFee.toString());
  ...
  if (addFee) {
    if (nowFeeConfig.transferFeeBasisPoints === POINT) {
      const nowMaxFee = new import_bn4.default(nowFeeConfig.maximumFee.toString());
      return {
      ...
      const _TAmount = BNDivCeil(amount.mul(new import_bn4.default(POINT)), new import_bn4.default(POINT - nowFeeConfig.transferFeeBasisPoints));
      const nowMaxFee = new import_bn4.default(nowFeeConfig.maximumFee.toString());
      const TAmount = _TAmount.sub(amount).gt(nowMaxFee) ? amount.add(nowMaxFee) : _TAmount;
      const _fee = BNDivCeil(TAmount.mul(new import_bn4.default(nowFeeConfig.transferFeeBasisPoints)), new import_bn4.default(POINT));
    ...
    const _fee = BNDivCeil(amount.mul(new import_bn4.default(nowFeeConfig.transferFeeBasisPoints)), new import_bn4.default(POINT));
    const fee = _fee.gt(maxFee) ? maxFee : _fee;
    return {
      amount,
      fee,
...
function BNDivCeil(bn1, bn2) {
  ...
  if (mod2.gt(new import_bn4.default(0))) {
    return div2.add(new import_bn4.default(1));
  } else {
    return div2;
  }
}
...
var FEE_RATE_DENOMINATOR_VALUE = new import_bn6.default(1e6);

// src/module/amount.ts
var logger5 = createLogger("Raydium_amount");
var Big2 = formatter_default(import_big2.default);

// src/common/bignumber.ts
var BN_ZERO = new import_bn8.default(0);
var BN_ONE = new import_bn8.default(1);
var BN_TWO = new import_bn8.default(2);
var BN_THREE = new import_bn8.default(3);
var BN_FIVE = new import_bn8.default(5);
var BN_TEN = new import_bn8.default(10);
var BN_100 = new import_bn8.default(100);
var BN_1000 = new import_bn8.default(1e3);
var BN_10000 = new import_bn8.default(1e4);
function ceilDiv(tokenAmount, feeNumerator, feeDenominator) {
  return tokenAmount.mul(feeNumerator).add(feeDenominator).sub(new import_bn8.default(1)).div(feeDenominator);
}
...
var import_decimal3 = __toESM(require("decimal.js-light"));
...
function checkedCeilDiv(dividend, rhs) {
  ...
  if (remainder.gt(ZERO)) {
    quotient = quotient.add(new import_bn9.default(1));
    rhs = dividend.div(quotient);
    remainder = checkedRem(dividend, quotient);
    if (remainder.gt(ZERO)) {
      rhs = rhs.add(new import_bn9.default(1));
    }
  }
  return [quotient, rhs];
}
var ZERO = new import_bn9.default(0);
  ...
  static lpTokensToTradingTokens(lpTokenAmount, lpTokenSupply, swapTokenAmount0, swapTokenAmount1, roundDirection) {
    ...
    if (roundDirection === 0 /* Floor */) {
      ...
      if (tokenRemainder0.gt(ZERO) && tokenAmount0.gt(ZERO)) {
        tokenAmount0 = tokenAmount0.add(new import_bn9.default(1));
      }
      ...
      if (token1Remainder.gt(ZERO) && tokenAmount1.gt(ZERO)) {
        tokenAmount1 = tokenAmount1.add(new import_bn9.default(1));
      }
      return { tokenAmount0, tokenAmount1 };
    }
...
var CurveCalculator = class {
  ...
  static swapBaseOut({
    ...
    const currentPrice = new import_decimal3.default(reserveOutAmount.toString()).div(10 ** reserveOutDecimals).div(new import_decimal3.default(reserveInAmount.toString()).div(10 ** reserveInDecimals));
    const amountRealOut = outputAmount.gte(reserveOutAmount) ? reserveOutAmount.sub(new import_bn10.default(1)) : outputAmount;
    ...
    const amountIn = BNDivCeil(amountInWithoutFee.mul(new import_bn10.default(1e6)), new import_bn10.default(1e6).sub(tradeFeeRate));
    const fee = amountIn.sub(amountInWithoutFee);
    const executionPrice = new import_decimal3.default(amountRealOut.toString()).div(10 ** reserveOutDecimals).div(new import_decimal3.default(amountIn.toString()).div(10 ** reserveInDecimals));
...
var BNLayout = class extends Layout {
  ...
  decode(b, offset2 = 0) {
    const num = new import_bn11.default(this.blob.decode(b, offset2), 10, "le");
    if (this.signed) {
  ...
  encode(src, b, offset2 = 0) {
    if (typeof src === "number")
      src = new import_bn11.default(src);
    if (this.signed) {
...
function seq2(elementLayout, count, property) {
  let parsedCount;
  const superCount = typeof count === "number" ? count : (0, import_bn11.isBN)(count) ? count.toNumber() : new Proxy(count, {
    get(target, property2) {
      if (!parsedCount) {
        const countProperty = Reflect.get(target, "count");
        parsedCount = (0, import_bn11.isBN)(countProperty) ? countProperty.toNumber() : countProperty;
...
var ZERO2 = new import_bn12.default(0);
...
function makeDepositCpmmInInstruction(programId, owner, authority, poolId, userLpAccount, userVaultA, userVaultB, vaultA, vaultB, mintA, mintB, lpMint, lpAmount, amountMaxA, amountMaxB) {
  ...
  const keys = [
    ...
    { pubkey: vaultB, isSigner: false, isWritable: true },
    { pubkey: import_spl_token10.TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
    { pubkey: import_spl_token10.TOKEN_2022_PROGRAM_ID, isSigner: false, isWritable: false },
    { pubkey: mintA, isSigner: false, isWritable: false },
    { pubkey: mintB, isSigner: false, isWritable: false },
    { pubkey: lpMint, isSigner: false, isWritable: true }
  ];
  const data = Buffer.alloc(dataLayout.span);
  logger7.debug("cpmm deposit data", {
...
async function makeCpmmLockInstruction(props) {
  ...
  const signers = [];
  const [poolId, lpMint] = [new import_web316.PublicKey(poolInfo.id), new import_web316.PublicKey(poolInfo.lpMint.address)];
  let nftMintAccount;
  if (getEphemeralSigners) {
    nftMintAccount = new import_web316.PublicKey((await getEphemeralSigners(1))[0]);
  } else {
    const _k = import_web316.Keypair.generate();
    signers.push(_k);
    nftMintAccount = _k.publicKey;
  }
  const { publicKey: nftAccount } = getATAAddress(feeNftOwner, nftMintAccount, import_spl_token10.TOKEN_PROGRAM_ID);
  ...
  const { publicKey: userLpVault } = getATAAddress(ownerInfo.wallet, lpMint, import_spl_token10.TOKEN_PROGRAM_ID);
  const { publicKey: lockLpVault } = getATAAddress(props.lockAuthProgram, lpMint, import_spl_token10.TOKEN_PROGRAM_ID);
  const ins = cpmmLockPositionInstruction({
    ...
    userLpVault,
    lockLpVault,
    poolVaultA: new import_web316.PublicKey(poolKeys.vault.A),
    poolVaultB: new import_web316.PublicKey(poolKeys.vault.B),
    metadataAccount,
...
var CpmmModule = class extends ModuleBase {
  ...
  async getRpcPoolInfo(poolId, accountInfos, fetchConfigInfo) {
    ...
    const poolAccount = poolId ? (await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, [{ pubkey: new import_web317.PublicKey(poolId) }]))[0] : accountInfos.marketAccount;
    ...
    if (poolId) {
      accountsToFetch.push(...vaultKeys.map((key) => ({ pubkey: new import_web317.PublicKey(key) })));
      if (fetchConfigInfo) {
        accountsToFetch.push({ pubkey: new import_web317.PublicKey(poolInfo.configId) });
      }
    }
    let fetchedAccounts = [];
    ...
    const vaultAAmount = new import_bn19.default(import_spl_token11.AccountLayout.decode(vaultAInfo.data).amount.toString());
    const vaultBAmount = new import_bn19.default(import_spl_token11.AccountLayout.decode(vaultBInfo.data).amount.toString());
  ...
  async getRpcPoolInfos(poolIds, fetchConfigInfo) {
    const accounts = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, poolIds.map((i) => ({ pubkey: new import_web317.PublicKey(i) })));
    const poolInfos = {};
    ...
    if (fetchConfigInfo) {
      ...
      const configState = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, configIds.map((i) => ({ pubkey: new import_web317.PublicKey(i) })));
    ...
    const vaultInfo = {};
    const vaultAccountInfo = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, needFetchVaults.map((i) => ({ pubkey: new import_web317.PublicKey(i) })));
    for (let i = 0; i < needFetchVaults.length; i++) {
      ...
      vaultInfo[String(needFetchVaults[i])] = new import_bn19.default(import_spl_token11.AccountLayout.decode(vaultItemInfo.data).amount.toString());
    }
    const returnData = {};
  ...
  toComputePoolInfos({
    ...
    return Object.keys(pools).reduce((acc, cur) => {
      ...
      return {
        ...
        [cur]: {
          ...pool,
          id: new import_web317.PublicKey(cur),
  ...
  async getPoolInfoFromRpc(poolId, rpcData, mintInfos) {
    ...
    const lpMint = toApiV3Token({
      ...
      programId: import_spl_token11.TOKEN_PROGRAM_ID.toBase58()
    });
    const configInfo = {
    ...
    return {
      ...
      poolKeys: {
        ...
        mintLp: lpMint,
        config: configInfo,
        observationId: getPdaObservationId(rpcData.programId, new import_web317.PublicKey(poolId)).publicKey.toBase58()
      },
      rpcData
    };
  }
  async createPool({
    ...
    const isFront = new import_bn19.default(new import_web317.PublicKey(params.mintA.address).toBuffer()).lte(new import_bn19.default(new import_web317.PublicKey(params.mintB.address).toBuffer()));
    ...
    const mintAUseSOLBalance = ownerInfo.useSOLBalance && mintA.address === import_spl_token11.NATIVE_MINT.toBase58();
    const mintBUseSOLBalance = ownerInfo.useSOLBalance && mintB.address === import_spl_token11.NATIVE_MINT.toBase58();
    const [mintAPubkey, mintBPubkey] = [new import_web317.PublicKey(mintA.address), new import_web317.PublicKey(mintB.address)];
    ...
    const { account: userVaultB, instructionParams: userVaultBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      mint: new import_web317.PublicKey(mintB.address),
    ...
    const poolKeys = getCreatePoolKeys({
      poolId,
      programId,
      configId: new import_web317.PublicKey(feeConfig.id),
      mintA: mintAPubkey,
      mintB: mintBPubkey
    });
    txBuilder.addInstruction({
      instructions: [
        makeCreateCpmmPoolInInstruction(programId, this.scope.ownerPubKey, new import_web317.PublicKey(feeConfig.id), poolKeys.authority, poolKeys.poolId, mintAPubkey, mintBPubkey, poolKeys.lpMint, userVaultA, userVaultB, getATAAddress(this.scope.ownerPubKey, poolKeys.lpMint).publicKey, poolKeys.vaultA, poolKeys.vaultB, poolFeeAccount, new import_web317.PublicKey(mintA.programId ?? import_spl_token11.TOKEN_PROGRAM_ID), new import_web317.PublicKey(mintB.programId ?? import_spl_token11.TOKEN_PROGRAM_ID), poolKeys.observationId, mintAAmount, mintBAmount, startTime)
      ],
  ...
  async addLiquidity(params) {
    ...
    const mintAUseSOLBalance = poolInfo.mintA.address === import_spl_token11.NATIVE_MINT.toString();
    const mintBUseSOLBalance = poolInfo.mintB.address === import_spl_token11.NATIVE_MINT.toString();
    const txBuilder = this.createTxBuilder(feePayer);
    const [mintA, mintB] = [new import_web317.PublicKey(poolInfo.mintA.address), new import_web317.PublicKey(poolInfo.mintB.address)];
    const { account: tokenAccountA, instructionParams: _tokenAccountAInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new import_web317.PublicKey(poolInfo.mintA.address),
      owner: this.scope.ownerPubKey,
    ...
    const { account: tokenAccountB, instructionParams: _tokenAccountBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new import_web317.PublicKey(poolInfo.mintB.address),
      owner: this.scope.ownerPubKey,
    ...
    const lpTokenAccount = await account.getCreatedTokenAccount({
      mint: new import_web317.PublicKey(poolInfo.lpMint.address)
    });
    ...
    const _slippage = new Percent(new import_bn19.default(1)).sub(slippage);
    txBuilder.addInstruction({
      instructions: [
        makeDepositCpmmInInstruction(new import_web317.PublicKey(poolInfo.programId), this.scope.ownerPubKey, new import_web317.PublicKey(poolKeys.authority), new import_web317.PublicKey(poolInfo.id), _lpTokenAccount, tokenAccountA, tokenAccountB, new import_web317.PublicKey(poolKeys.vault.A), new import_web317.PublicKey(poolKeys.vault.B), mintA, mintB, new import_web317.PublicKey(poolInfo.lpMint.address), computeResult ? computeResult?.liquidity : _slippage.mul(liquidity).quotient, baseIn ? inputAmountFee.amount : anotherAmount, baseIn ? anotherAmount : inputAmountFee.amount)
      ],
  ...
  async withdrawLiquidity(params) {
    ...
    txBuilder.addInstruction({
      instructions: [
        makeWithdrawCpmmInInstruction(new import_web317.PublicKey(poolInfo.programId), this.scope.ownerPubKey, new import_web317.PublicKey(poolKeys.authority), new import_web317.PublicKey(poolInfo.id), lpTokenAccount, tokenAccountA, tokenAccountB, new import_web317.PublicKey(poolKeys.vault.A), new import_web317.PublicKey(poolKeys.vault.B), mintA, mintB, new import_web317.PublicKey(poolInfo.lpMint.address), lpAmount, amountMintA.sub(mintAAmountFee.fee ?? new import_bn19.default(0)), amountMintB.sub(mintBAmountFee.fee ?? new import_bn19.default(0)))
      ],
  ...
  async swap(params) {
    ...
    if (!fixedOut) {
      swapResult.destinationAmountSwapped = swapResult.destinationAmountSwapped.mul(new import_bn19.default((1 - slippage) * 1e4)).div(new import_bn19.default(1e4));
    } else {
      swapResult.sourceAmountSwapped = swapResult.sourceAmountSwapped.mul(new import_bn19.default((1 + slippage) * 1e4)).div(new import_bn19.default(1e4));
    }
    const [mintA, mintB] = [new import_web317.PublicKey(poolInfo.mintA.address), new import_web317.PublicKey(poolInfo.mintB.address)];
    let mintATokenAcc;
    let mintBTokenAcc;
    if (useIdempotent) {
      const tokenProgramA = new import_web317.PublicKey(poolInfo.mintA.programId ?? import_spl_token11.TOKEN_PROGRAM_ID);
      const tokenProgramB = new import_web317.PublicKey(poolInfo.mintB.programId ?? import_spl_token11.TOKEN_PROGRAM_ID);
      const mintAIsSOL = mintA.equals(import_spl_token11.NATIVE_MINT);
      const mintBIsSOL = mintB.equals(import_spl_token11.NATIVE_MINT);
      mintATokenAcc = (0, import_spl_token11.getAssociatedTokenAddressSync)(mintA, this.scope.ownerPubKey, false, tokenProgramA);
      mintBTokenAcc = (0, import_spl_token11.getAssociatedTokenAddressSync)(mintB, this.scope.ownerPubKey, false, tokenProgramB);
      if (!mintAIsSOL) {
        txBuilder.addInstruction({
          instructions: [
            (0, import_spl_token11.createAssociatedTokenAccountIdempotentInstruction)(this.scope.ownerPubKey, mintATokenAcc, this.scope.ownerPubKey, mintA, tokenProgramA)
          ],
      ...
      if (!mintBIsSOL) {
        txBuilder.addInstruction({
          instructions: [
            (0, import_spl_token11.createAssociatedTokenAccountIdempotentInstruction)(this.scope.ownerPubKey, mintBTokenAcc, this.scope.ownerPubKey, mintB, tokenProgramB)
          ],
          instructionTypes: [InstructionType.CreateATA]
        });
      }
      ...
      const { account: mintATokenAccResult, instructionParams: mintATokenAccInstruction } = await this.scope.account.getOrCreateTokenAccount({
        mint: mintA,
        tokenProgram: new import_web317.PublicKey(poolInfo.mintA.programId ?? import_spl_token11.TOKEN_PROGRAM_ID),
        owner: this.scope.ownerPubKey,
      ...
      const { account: mintBTokenAccResult, instructionParams: mintBTokenAccInstruction } = await this.scope.account.getOrCreateTokenAccount({
        mint: mintB,
        tokenProgram: new import_web317.PublicKey(poolInfo.mintB.programId ?? import_spl_token11.TOKEN_PROGRAM_ID),
        owner: this.scope.ownerPubKey,
    ...
    const sourceTokenProgram = baseIn ? new import_web317.PublicKey(poolInfo.mintA.programId ?? import_spl_token11.TOKEN_PROGRAM_ID) : new import_web317.PublicKey(poolInfo.mintB.programId ?? import_spl_token11.TOKEN_PROGRAM_ID);
    const destinationTokenProgram = baseIn ? new import_web317.PublicKey(poolInfo.mintB.programId ?? import_spl_token11.TOKEN_PROGRAM_ID) : new import_web317.PublicKey(poolInfo.mintA.programId ?? import_spl_token11.TOKEN_PROGRAM_ID);
    txBuilder.addInstruction({
      instructions: [
        !fixedOut ? makeSwapCpmmBaseInInstruction(new import_web317.PublicKey(poolInfo.programId), this.scope.ownerPubKey, new import_web317.PublicKey(poolKeys.authority), new import_web317.PublicKey(poolKeys.config.id), new import_web317.PublicKey(poolInfo.id), sourceTokenAccount, destinationTokenAccount, new import_web317.PublicKey(sourceVault), new import_web317.PublicKey(destinationVault), sourceTokenProgram, destinationTokenProgram, sourceMint, destinationMint, getPdaObservationId(new import_web317.PublicKey(poolInfo.programId), new import_web317.PublicKey(poolInfo.id)).publicKey, inputAmount, swapResult.destinationAmountSwapped) : makeSwapCpmmBaseOutInstruction(new import_web317.PublicKey(poolInfo.programId), this.scope.ownerPubKey, new import_web317.PublicKey(poolKeys.authority), new import_web317.PublicKey(poolKeys.config.id), new import_web317.PublicKey(poolInfo.id), sourceTokenAccount, destinationTokenAccount, new import_web317.PublicKey(sourceVault), new import_web317.PublicKey(destinationVault), sourceTokenProgram, destinationTokenProgram, sourceMint, destinationMint, getPdaObservationId(new import_web317.PublicKey(poolInfo.programId), new import_web317.PublicKey(poolInfo.id)).publicKey, swapResult.sourceAmountSwapped, swapResult.destinationAmountSwapped)
      ],
  ...
  async harvestLockLp(params) {
    ...
    const poolKeys = params.poolKeys ?? await this.getCpmmPoolKeys(poolInfo.id);
    const { publicKey: nftAccount } = getATAAddress(feePayer, nftMint, import_spl_token11.TOKEN_PROGRAM_ID);
    const { publicKey: lockPda } = getCpLockPda(programId, nftMint);
    const { publicKey: lockLpVault } = getATAAddress(authProgram, new import_web317.PublicKey(poolInfo.lpMint.address), import_spl_token11.TOKEN_PROGRAM_ID);
    txBuilder.addInstruction({
      instructions: [
        collectCpFeeInstruction({
          ...
          nftAccount,
          lockPda,
          poolId: new import_web317.PublicKey(poolInfo.id),
          mintLp: new import_web317.PublicKey(poolKeys.mintLp.address),
          userVaultA: tokenAccountA,
          userVaultB: tokenAccountB,
          poolVaultA: new import_web317.PublicKey(poolKeys.vault.A),
          poolVaultB: new import_web317.PublicKey(poolKeys.vault.B),
          mintA,
          mintB,
          lockLpVault,
          lpFeeAmount,
  ...
  computeSwapAmount({
    ...
    const minAmountOut = swapResult.destinationAmountSwapped.mul(new import_bn19.default((1 - slippage) * 1e4)).div(new import_bn19.default(1e4));
    return {
  ...
  computePairAmount({
    ...
    const inputAmount = new import_bn19.default(new decimal_default(amount).mul(10 ** poolInfo[baseIn ? "mintA" : "mintB"].decimals).mul(coefficient).toFixed(0));
    const inputAmountFee = getTransferAmountFeeV2(inputAmount, poolInfo[baseIn ? "mintA" : "mintB"].extensions.feeConfig, epochInfo, false);
    const _inputAmountWithoutFee = inputAmount.sub(inputAmountFee.fee ?? new import_bn19.default(0));
    const lpAmount = new import_bn19.default(new decimal_default(poolInfo.lpAmount).mul(10 ** poolInfo.lpMint.decimals).toFixed(0, decimal_default.ROUND_DOWN));
    ...
    const _slippage = new Percent(new import_bn19.default(1)).add(slippage);
    const _slippageMin = new Percent(new import_bn19.default(1)).sub(slippage);
    const slippageAdjustedAmount = getTransferAmountFeeV2(_slippage.mul(anotherAmountFee.amount.sub(anotherAmountFee.fee ?? new import_bn19.default(0))).quotient, poolInfo[baseIn ? "mintB" : "mintA"].extensions.feeConfig, epochInfo, true);
    const slippageAdjustedMinAmount = getTransferAmountFeeV2(_slippageMin.mul(anotherAmountFee.amount.sub(anotherAmountFee.fee ?? new import_bn19.default(0))).quotient, poolInfo[baseIn ? "mintB" : "mintA"].extensions.feeConfig, epochInfo, true);
