...
import {
  AccountLayout as AccountLayout2,
...
function createLogger(moduleName) {
  let logger8 = get(moduleLoggers, moduleName);
  if (!logger8) {
    const logLevel = get(moduleLevels, moduleName);
    logger8 = new Logger({ name: moduleName, logLevel });
    set(moduleLoggers, moduleName, logger8);
  }
  return logger8;
}
...
function getATAAddress(owner, mint, programId) {
  return findProgramAddress([owner.toBuffer(), (programId ?? TOKEN_PROGRAM_ID5).toBuffer(), mint.toBuffer()], new PublicKey6("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL"));
}
...
function getTransferAmountFeeV2(amount, _feeConfig, epochInfo, addFee) {
  ...
  const maxFee = new BN4(nowFeeConfig.maximumFee.toString());
  ...
  if (addFee) {
    if (nowFeeConfig.transferFeeBasisPoints === POINT) {
      const nowMaxFee = new BN4(nowFeeConfig.maximumFee.toString());
      return {
      ...
      const _TAmount = BNDivCeil(amount.mul(new BN4(POINT)), new BN4(POINT - nowFeeConfig.transferFeeBasisPoints));
      const nowMaxFee = new BN4(nowFeeConfig.maximumFee.toString());
      const TAmount = _TAmount.sub(amount).gt(nowMaxFee) ? amount.add(nowMaxFee) : _TAmount;
      const _fee = BNDivCeil(TAmount.mul(new BN4(nowFeeConfig.transferFeeBasisPoints)), new BN4(POINT));
    ...
    const _fee = BNDivCeil(amount.mul(new BN4(nowFeeConfig.transferFeeBasisPoints)), new BN4(POINT));
    const fee = _fee.gt(maxFee) ? maxFee : _fee;
    return {
      amount,
      fee,
...
function BNDivCeil(bn1, bn2) {
  ...
  if (mod2.gt(new BN4(0))) {
    return div2.add(new BN4(1));
  } else {
    return div2;
  }
}
...
var FEE_RATE_DENOMINATOR_VALUE = new BN6(1e6);
...
function ceilDiv(tokenAmount, feeNumerator, feeDenominator) {
  return tokenAmount.mul(feeNumerator).add(feeDenominator).sub(new BN8(1)).div(feeDenominator);
}
  ...
  static lpTokensToTradingTokens(lpTokenAmount, lpTokenSupply, swapTokenAmount0, swapTokenAmount1, roundDirection) {
    ...
    if (roundDirection === 0 /* Floor */) {
      ...
      if (tokenRemainder0.gt(ZERO) && tokenAmount0.gt(ZERO)) {
        tokenAmount0 = tokenAmount0.add(new BN9(1));
      }
      ...
      if (token1Remainder.gt(ZERO) && tokenAmount1.gt(ZERO)) {
        tokenAmount1 = tokenAmount1.add(new BN9(1));
      }
      return { tokenAmount0, tokenAmount1 };
    }
...
var CurveCalculator = class {
  ...
  static swapBaseOut({
    ...
    const amountRealOut = outputAmount.gte(reserveOutAmount) ? reserveOutAmount.sub(new BN10(1)) : outputAmount;
    ...
    const amountIn = BNDivCeil(amountInWithoutFee.mul(new BN10(1e6)), new BN10(1e6).sub(tradeFeeRate));
...
var BNLayout = class extends Layout {
  ...
  decode(b, offset2 = 0) {
    const num = new BN11(this.blob.decode(b, offset2), 10, "le");
    if (this.signed) {
...
async function makeCpmmLockInstruction(props) {
  ...
  const signers = [];
  const [poolId, lpMint] = [new PublicKey16(poolInfo.id), new PublicKey16(poolInfo.lpMint.address)];
  let nftMintAccount;
  if (getEphemeralSigners) {
    nftMintAccount = new PublicKey16((await getEphemeralSigners(1))[0]);
  } else {
    const _k = Keypair3.generate();
    signers.push(_k);
    nftMintAccount = _k.publicKey;
  }
  const { publicKey: nftAccount } = getATAAddress(feeNftOwner, nftMintAccount, TOKEN_PROGRAM_ID9);
  ...
  const { publicKey: userLpVault } = getATAAddress(ownerInfo.wallet, lpMint, TOKEN_PROGRAM_ID9);
  const { publicKey: lockLpVault } = getATAAddress(props.lockAuthProgram, lpMint, TOKEN_PROGRAM_ID9);
  const ins = cpmmLockPositionInstruction({
    ...
    userLpVault,
    lockLpVault,
    poolVaultA: new PublicKey16(poolKeys.vault.A),
    poolVaultB: new PublicKey16(poolKeys.vault.B),
    metadataAccount,
...
var CpmmModule = class extends ModuleBase {
  ...
  async getRpcPoolInfo(poolId, accountInfos, fetchConfigInfo) {
    ...
    const poolAccount = poolId ? (await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, [{ pubkey: new PublicKey17(poolId) }]))[0] : accountInfos.marketAccount;
    ...
    if (poolId) {
      accountsToFetch.push(...vaultKeys.map((key) => ({ pubkey: new PublicKey17(key) })));
      if (fetchConfigInfo) {
        accountsToFetch.push({ pubkey: new PublicKey17(poolInfo.configId) });
      }
    }
    let fetchedAccounts = [];
    ...
    const vaultAAmount = new BN19(AccountLayout2.decode(vaultAInfo.data).amount.toString());
    const vaultBAmount = new BN19(AccountLayout2.decode(vaultBInfo.data).amount.toString());
  ...
  async getRpcPoolInfos(poolIds, fetchConfigInfo) {
    const accounts = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, poolIds.map((i) => ({ pubkey: new PublicKey17(i) })));
    const poolInfos = {};
    ...
    if (fetchConfigInfo) {
      ...
      const configState = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, configIds.map((i) => ({ pubkey: new PublicKey17(i) })));
    ...
    const vaultInfo = {};
    const vaultAccountInfo = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, needFetchVaults.map((i) => ({ pubkey: new PublicKey17(i) })));
    for (let i = 0; i < needFetchVaults.length; i++) {
      ...
      vaultInfo[String(needFetchVaults[i])] = new BN19(AccountLayout2.decode(vaultItemInfo.data).amount.toString());
    }
    const returnData = {};
  ...
  async getPoolInfoFromRpc(poolId, rpcData, mintInfos) {
    ...
    const lpMint = toApiV3Token({
      ...
      programId: TOKEN_PROGRAM_ID10.toBase58()
    });
    const configInfo = {
    ...
    return {
      ...
      poolKeys: {
        ...
        mintLp: lpMint,
        config: configInfo,
        observationId: getPdaObservationId(rpcData.programId, new PublicKey17(poolId)).publicKey.toBase58()
      },
      rpcData
    };
  }
  async createPool({
    ...
    const isFront = new BN19(new PublicKey17(params.mintA.address).toBuffer()).lte(new BN19(new PublicKey17(params.mintB.address).toBuffer()));
    ...
    const [mintAPubkey, mintBPubkey] = [new PublicKey17(mintA.address), new PublicKey17(mintB.address)];
    ...
    const { account: userVaultB, instructionParams: userVaultBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      mint: new PublicKey17(mintB.address),
    ...
    const poolKeys = getCreatePoolKeys({
      poolId,
      programId,
      configId: new PublicKey17(feeConfig.id),
      mintA: mintAPubkey,
      mintB: mintBPubkey
    });
    txBuilder.addInstruction({
      instructions: [
        makeCreateCpmmPoolInInstruction(programId, this.scope.ownerPubKey, new PublicKey17(feeConfig.id), poolKeys.authority, poolKeys.poolId, mintAPubkey, mintBPubkey, poolKeys.lpMint, userVaultA, userVaultB, getATAAddress(this.scope.ownerPubKey, poolKeys.lpMint).publicKey, poolKeys.vaultA, poolKeys.vaultB, poolFeeAccount, new PublicKey17(mintA.programId ?? TOKEN_PROGRAM_ID10), new PublicKey17(mintB.programId ?? TOKEN_PROGRAM_ID10), poolKeys.observationId, mintAAmount, mintBAmount, startTime)
      ],
  ...
  async addLiquidity(params) {
    ...
    const txBuilder = this.createTxBuilder(feePayer);
    const [mintA, mintB] = [new PublicKey17(poolInfo.mintA.address), new PublicKey17(poolInfo.mintB.address)];
    const { account: tokenAccountA, instructionParams: _tokenAccountAInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey17(poolInfo.mintA.address),
      owner: this.scope.ownerPubKey,
    ...
    const { account: tokenAccountB, instructionParams: _tokenAccountBInstruction } = await this.scope.account.getOrCreateTokenAccount({
      ...
      mint: new PublicKey17(poolInfo.mintB.address),
      owner: this.scope.ownerPubKey,
    ...
    const lpTokenAccount = await account.getCreatedTokenAccount({
      mint: new PublicKey17(poolInfo.lpMint.address)
    });
    ...
    const _slippage = new Percent(new BN19(1)).sub(slippage);
    txBuilder.addInstruction({
      instructions: [
        makeDepositCpmmInInstruction(new PublicKey17(poolInfo.programId), this.scope.ownerPubKey, new PublicKey17(poolKeys.authority), new PublicKey17(poolInfo.id), _lpTokenAccount, tokenAccountA, tokenAccountB, new PublicKey17(poolKeys.vault.A), new PublicKey17(poolKeys.vault.B), mintA, mintB, new PublicKey17(poolInfo.lpMint.address), computeResult ? computeResult?.liquidity : _slippage.mul(liquidity).quotient, baseIn ? inputAmountFee.amount : anotherAmount, baseIn ? anotherAmount : inputAmountFee.amount)
      ],
  ...
  async withdrawLiquidity(params) {
    ...
    txBuilder.addInstruction({
      instructions: [
        makeWithdrawCpmmInInstruction(new PublicKey17(poolInfo.programId), this.scope.ownerPubKey, new PublicKey17(poolKeys.authority), new PublicKey17(poolInfo.id), lpTokenAccount, tokenAccountA, tokenAccountB, new PublicKey17(poolKeys.vault.A), new PublicKey17(poolKeys.vault.B), mintA, mintB, new PublicKey17(poolInfo.lpMint.address), lpAmount, amountMintA.sub(mintAAmountFee.fee ?? new BN19(0)), amountMintB.sub(mintBAmountFee.fee ?? new BN19(0)))
      ],
  ...
  async swap(params) {
    ...
    if (!fixedOut) {
      swapResult.destinationAmountSwapped = swapResult.destinationAmountSwapped.mul(new BN19((1 - slippage) * 1e4)).div(new BN19(1e4));
    } else {
      swapResult.sourceAmountSwapped = swapResult.sourceAmountSwapped.mul(new BN19((1 + slippage) * 1e4)).div(new BN19(1e4));
    }
    const [mintA, mintB] = [new PublicKey17(poolInfo.mintA.address), new PublicKey17(poolInfo.mintB.address)];
    let mintATokenAcc;
    let mintBTokenAcc;
    if (useIdempotent) {
      const tokenProgramA = new PublicKey17(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID10);
      const tokenProgramB = new PublicKey17(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID10);
      ...
      const { account: mintATokenAccResult, instructionParams: mintATokenAccInstruction } = await this.scope.account.getOrCreateTokenAccount({
        mint: mintA,
        tokenProgram: new PublicKey17(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID10),
        owner: this.scope.ownerPubKey,
      ...
      const { account: mintBTokenAccResult, instructionParams: mintBTokenAccInstruction } = await this.scope.account.getOrCreateTokenAccount({
        mint: mintB,
        tokenProgram: new PublicKey17(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID10),
        owner: this.scope.ownerPubKey,
    ...
    const sourceTokenProgram = baseIn ? new PublicKey17(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID10) : new PublicKey17(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID10);
    const destinationTokenProgram = baseIn ? new PublicKey17(poolInfo.mintB.programId ?? TOKEN_PROGRAM_ID10) : new PublicKey17(poolInfo.mintA.programId ?? TOKEN_PROGRAM_ID10);
    txBuilder.addInstruction({
      instructions: [
        !fixedOut ? makeSwapCpmmBaseInInstruction(new PublicKey17(poolInfo.programId), this.scope.ownerPubKey, new PublicKey17(poolKeys.authority), new PublicKey17(poolKeys.config.id), new PublicKey17(poolInfo.id), sourceTokenAccount, destinationTokenAccount, new PublicKey17(sourceVault), new PublicKey17(destinationVault), sourceTokenProgram, destinationTokenProgram, sourceMint, destinationMint, getPdaObservationId(new PublicKey17(poolInfo.programId), new PublicKey17(poolInfo.id)).publicKey, inputAmount, swapResult.destinationAmountSwapped) : makeSwapCpmmBaseOutInstruction(new PublicKey17(poolInfo.programId), this.scope.ownerPubKey, new PublicKey17(poolKeys.authority), new PublicKey17(poolKeys.config.id), new PublicKey17(poolInfo.id), sourceTokenAccount, destinationTokenAccount, new PublicKey17(sourceVault), new PublicKey17(destinationVault), sourceTokenProgram, destinationTokenProgram, sourceMint, destinationMint, getPdaObservationId(new PublicKey17(poolInfo.programId), new PublicKey17(poolInfo.id)).publicKey, swapResult.sourceAmountSwapped, swapResult.destinationAmountSwapped)
      ],
  ...
  async harvestLockLp(params) {
    ...
    const poolKeys = params.poolKeys ?? await this.getCpmmPoolKeys(poolInfo.id);
    const { publicKey: nftAccount } = getATAAddress(feePayer, nftMint, TOKEN_PROGRAM_ID10);
    const { publicKey: lockPda } = getCpLockPda(programId, nftMint);
    const { publicKey: lockLpVault } = getATAAddress(authProgram, new PublicKey17(poolInfo.lpMint.address), TOKEN_PROGRAM_ID10);
    txBuilder.addInstruction({
      instructions: [
        collectCpFeeInstruction({
          ...
          nftAccount,
          lockPda,
          poolId: new PublicKey17(poolInfo.id),
          mintLp: new PublicKey17(poolKeys.mintLp.address),
          userVaultA: tokenAccountA,
          userVaultB: tokenAccountB,
          poolVaultA: new PublicKey17(poolKeys.vault.A),
          poolVaultB: new PublicKey17(poolKeys.vault.B),
          mintA,
          mintB,
          lockLpVault,
          lpFeeAmount,
  ...
  computeSwapAmount({
    ...
    const minAmountOut = swapResult.destinationAmountSwapped.mul(new BN19((1 - slippage) * 1e4)).div(new BN19(1e4));
    return {
  ...
  computePairAmount({
    ...
    const inputAmount = new BN19(new decimal_default(amount).mul(10 ** poolInfo[baseIn ? "mintA" : "mintB"].decimals).mul(coefficient).toFixed(0));
    const inputAmountFee = getTransferAmountFeeV2(inputAmount, poolInfo[baseIn ? "mintA" : "mintB"].extensions.feeConfig, epochInfo, false);
    const _inputAmountWithoutFee = inputAmount.sub(inputAmountFee.fee ?? new BN19(0));
    const lpAmount = new BN19(new decimal_default(poolInfo.lpAmount).mul(10 ** poolInfo.lpMint.decimals).toFixed(0, decimal_default.ROUND_DOWN));
    ...
    const _slippage = new Percent(new BN19(1)).add(slippage);
    const _slippageMin = new Percent(new BN19(1)).sub(slippage);
    const slippageAdjustedAmount = getTransferAmountFeeV2(_slippage.mul(anotherAmountFee.amount.sub(anotherAmountFee.fee ?? new BN19(0))).quotient, poolInfo[baseIn ? "mintB" : "mintA"].extensions.feeConfig, epochInfo, true);
    const slippageAdjustedMinAmount = getTransferAmountFeeV2(_slippageMin.mul(anotherAmountFee.amount.sub(anotherAmountFee.fee ?? new BN19(0))).quotient, poolInfo[baseIn ? "mintB" : "mintA"].extensions.feeConfig, epochInfo, true);
