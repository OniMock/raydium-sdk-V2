...
function getATAAddress(owner, mint, programId) {
  return findProgramAddress([owner.toBuffer(), (programId ?? import_spl_token5.TOKEN_PROGRAM_ID).toBuffer(), mint.toBuffer()], new import_web38.PublicKey("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL"));
}
...
function BNDivCeil(bn1, bn2) {
  ...
  if (mod2.gt(new import_bn8.default(0))) {
    return div2.add(new import_bn8.default(1));
  } else {
    return div2;
  }
}
...
function checkedCeilDiv(dividend, rhs) {
  ...
  if (remainder.gt(ZERO3)) {
    quotient = quotient.add(new import_bn17.default(1));
    rhs = dividend.div(quotient);
    remainder = checkedRem(dividend, quotient);
    if (remainder.gt(ZERO3)) {
      rhs = rhs.add(new import_bn17.default(1));
    }
  }
  return [quotient, rhs];
}
  ...
  static lpTokensToTradingTokens(lpTokenAmount, lpTokenSupply, swapTokenAmount0, swapTokenAmount1, roundDirection) {
    ...
    if (roundDirection === 0 /* Floor */) {
      ...
      if (tokenRemainder0.gt(ZERO3) && tokenAmount0.gt(ZERO3)) {
        tokenAmount0 = tokenAmount0.add(new import_bn17.default(1));
      }
      ...
      if (token1Remainder.gt(ZERO3) && tokenAmount1.gt(ZERO3)) {
        tokenAmount1 = tokenAmount1.add(new import_bn17.default(1));
      }
      return { tokenAmount0, tokenAmount1 };
    }
...
var CurveCalculator = class {
  ...
  static swapBaseOut({
    ...
    const currentPrice = new import_decimal8.default(reserveOutAmount.toString()).div(10 ** reserveOutDecimals).div(new import_decimal8.default(reserveInAmount.toString()).div(10 ** reserveInDecimals));
    const amountRealOut = outputAmount.gte(reserveOutAmount) ? reserveOutAmount.sub(new import_bn18.default(1)) : outputAmount;
    ...
    const amountIn = BNDivCeil(amountInWithoutFee.mul(new import_bn18.default(1e6)), new import_bn18.default(1e6).sub(tradeFeeRate));
    const fee = amountIn.sub(amountInWithoutFee);
    const executionPrice = new import_decimal8.default(amountRealOut.toString()).div(10 ** reserveOutDecimals).div(new import_decimal8.default(amountIn.toString()).div(10 ** reserveInDecimals));
