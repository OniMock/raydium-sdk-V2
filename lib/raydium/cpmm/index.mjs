...
function getATAAddress(owner, mint, programId) {
  return findProgramAddress([owner.toBuffer(), (programId ?? TOKEN_PROGRAM_ID5).toBuffer(), mint.toBuffer()], new PublicKey8("ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL"));
}
...
function BNDivCeil(bn1, bn2) {
  ...
  if (mod2.gt(new BN8(0))) {
    return div2.add(new BN8(1));
  } else {
    return div2;
  }
}
  ...
  static lpTokensToTradingTokens(lpTokenAmount, lpTokenSupply, swapTokenAmount0, swapTokenAmount1, roundDirection) {
    ...
    if (roundDirection === 0 /* Floor */) {
      ...
      if (tokenRemainder0.gt(ZERO3) && tokenAmount0.gt(ZERO3)) {
        tokenAmount0 = tokenAmount0.add(new BN17(1));
      }
      ...
      if (token1Remainder.gt(ZERO3) && tokenAmount1.gt(ZERO3)) {
        tokenAmount1 = tokenAmount1.add(new BN17(1));
      }
      return { tokenAmount0, tokenAmount1 };
    }
...
var CurveCalculator = class {
  ...
  static swapBaseOut({
    ...
    const amountRealOut = outputAmount.gte(reserveOutAmount) ? reserveOutAmount.sub(new BN18(1)) : outputAmount;
    ...
    const amountIn = BNDivCeil(amountInWithoutFee.mul(new BN18(1e6)), new BN18(1e6).sub(tradeFeeRate));
