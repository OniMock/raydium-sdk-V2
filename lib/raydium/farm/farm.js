...
function createLogger(moduleName) {
  let logger10 = (0, import_lodash.get)(moduleLoggers, moduleName);
  if (!logger10) {
    const logLevel = (0, import_lodash.get)(moduleLevels, moduleName);
    logger10 = new Logger({ name: moduleName, logLevel });
    (0, import_lodash.set)(moduleLoggers, moduleName, logger10);
  }
  return logger10;
}
...
function parseBigNumberish(value) {
  const logger10 = createLogger("Raydium_parseBigNumberish");
  ...
  if (typeof value === "string") {
    ...
    logger10.logWithError(`invalid BigNumberish string: ${value}`);
  }
  if (typeof value === "number") {
    if (value % 1) {
      logger10.logWithError(`BigNumberish number underflow: ${value}`);
    }
    if (value >= MAX_SAFE || value <= -MAX_SAFE) {
      logger10.logWithError(`BigNumberish number overflow: ${value}`);
    }
  ...
  logger10.error(`invalid BigNumberish value: ${value}`);
...
// src/raydium/farm/config.ts
var logger7 = createLogger("Raydium_farm_config");
var FARM_LOCK_MINT = new import_web314.PublicKey("4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R");
var FARM_LOCK_VAULT = new import_web314.PublicKey("FrspKwj8i3pNmKwXreTveC4fu7KL5ZbGeXdZBe2XViu1");
...
var logger8 = createLogger("Raydium.farm.util");
...
function getFarmLedgerLayout(version) {
  ...
  if (!ledgerLayout)
    logger8.logWithError("invalid version", version);
  return ledgerLayout;
}

// src/raydium/farm/instruction.ts
var logger9 = createLogger("Raydium_farm_instruction");
var anchorDataBuf = {
...
function createAssociatedLedgerAccountInstruction(params) {
  ...
  if (!instruction)
    logger9.logWithError(`invalid farm pool version: ${version}`);
  ...
  const keys = [
    ...
    accountMeta({ pubkey: import_web315.SystemProgram.programId, isWritable: false }),
    accountMeta({ pubkey: import_web315.SYSVAR_RENT_PUBKEY, isWritable: false })
  ];
  return {
    instruction: new import_web315.TransactionInstruction({
      programId,
      keys,
      data
    }),
...
function makeCreateFarmInstruction(params) {
  ...
  return {
    instruction: new import_web315.TransactionInstruction({ programId: params.programId, keys, data }),
...
function makeCreatorWithdrawFarmRewardInstruction(params) {
  ...
  const keys = [
    accountMeta({ pubkey: import_spl_token8.TOKEN_PROGRAM_ID, isWritable: false }),
...
function makeRestartRewardInstruction({
  ...
  return new import_web315.TransactionInstruction({ programId: farmKeys.programId, keys, data });
}
...
function makeWithdrawInstructionV6(params) {
  ...
  const [programId, id] = [new import_web315.PublicKey(farmInfo.programId), new import_web315.PublicKey(farmInfo.id)];
  ...
  const keys = [
    accountMeta({ pubkey: import_spl_token8.TOKEN_PROGRAM_ID, isWritable: false }),
    accountMeta({ pubkey: id }),
    accountMeta({ pubkey: new import_web315.PublicKey(farmKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new import_web315.PublicKey(farmKeys.lpVault) }),
  ...
  for (let index = 0; index < farmKeys.rewardInfos.length; index++) {
    keys.push(accountMeta({ pubkey: new import_web315.PublicKey(farmKeys.rewardInfos[index].vault) }));
    keys.push(accountMeta({ pubkey: rewardAccounts[index] }));
  }
  return new import_web315.TransactionInstruction({ programId, keys, data });
}
function makeWithdrawInstructionV5(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: new import_web315.PublicKey(farmKeys.rewardInfos[0].vault) }),
    accountMeta({ pubkey: import_web315.SYSVAR_CLOCK_PUBKEY, isWritable: false }),
    accountMeta({ pubkey: import_spl_token8.TOKEN_PROGRAM_ID, isWritable: false })
  ];
...
function makeWithdrawInstructionV4(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: new import_web315.PublicKey(farmKeys.rewardInfos[1].vault) })
  ];
...
var Farm = class extends ModuleBase {
  ...
  async create({
    ...
    this.checkDisabled();
    this.scope.checkOwner();
    const lpMint = new import_web316.PublicKey(propPoolInfo.lpMint.address);
    const poolInfo = {
      lpMint,
    ...
    txBuilder.addInstruction({
      instructions: [
        import_web316.SystemProgram.createAccountWithSeed({
    ...
    const { publicKey: authority, nonce } = getAssociatedAuthority({
      programId: new import_web316.PublicKey(poolInfo.programId),
    ...
    for (const rewardInfo of poolInfo.rewardInfos) {
      ...
      const rewardMint = rewardInfo.mint.equals(SOLMint) ? new import_web316.PublicKey(TOKEN_WSOL.address) : rewardInfo.mint;
      rewardInfoKey.push({
        rewardMint,
    ...
    const { account: lockUserAccount, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
      mint: new import_web316.PublicKey(poolInfo.lockInfo.lockMint),
      owner: this.scope.ownerPubKey,
  ...
  async restartReward({
    ...
    const payerPubKey = payer || this.scope.ownerPubKey;
    const rewardMint = newRewardInfo.mint.equals(SOLMint) ? new import_web316.PublicKey(TOKEN_WSOL.address) : newRewardInfo.mint;
    const rewardInfoIndex = farmKeys.rewardInfos.findIndex((item) => new import_web316.PublicKey(item.mint.address).equals(rewardMint));
  ...
  async restartRewards({
    ...
    for (const itemReward of newRewardInfos) {
      const rewardMint = itemReward.mint.equals(SOLMint) ? new import_web316.PublicKey(TOKEN_WSOL.address) : itemReward.mint;
      const rewardInfoIndex = farmKeys.rewardInfos.findIndex((item) => new import_web316.PublicKey(item.mint.address).equals(rewardMint));
      const rewardInfo = farmInfoKeys.rewardInfos[rewardInfoIndex];
      if (!rewardInfo)
        this.logAndCreateError("configuration does not exist", "rewardMint", rewardMint);
  ...
  async addNewRewardToken(params) {
    ...
    const rewardVault = getAssociatedLedgerPoolAccount({
      programId: new import_web316.PublicKey(farmInfo.programId),
      poolId: new import_web316.PublicKey(farmInfo.id),
      mint: rewardMint,
      type: "rewardVault"
    });
    const { rewardPubKey: userRewardTokenPub, newInstruction } = await this._getUserRewardInfo({
  ...
  async deposit(params) {
    ...
    const [farmProgramId, farmId] = [new import_web316.PublicKey(farmInfo.programId), new import_web316.PublicKey(farmInfo.id)];
    ...
    for (const itemReward of rewardInfos) {
      ...
      if (!ownerRewardAccount) {
        const { account: _ownerRewardAccount, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
          ...
          mint: new import_web316.PublicKey(itemReward.mint.address),
    ...
    const insParams = {
      ...
      rewardAccounts,
      userAuxiliaryLedgers: userAuxiliaryLedgers?.map((key) => new import_web316.PublicKey(key))
    };
  ...
  async withdraw(params) {
    ...
    if (version !== 4) {
      ...
      if (!ledgerData) {
        if (version !== 6) {
          const { instruction, instructionType } = createAssociatedLedgerAccountInstruction({
            id: new import_web316.PublicKey(farmKeys.id),
            programId: new import_web316.PublicKey(farmKeys.programId),
            version,
            ledger,
            owner: this.scope.ownerPubKey
          });
    ...
    if (!ownerLpTokenAccount) {
      const { account: _ownerRewardAccount, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
        ...
        mint: new import_web316.PublicKey(lpMint),
  ...
  async withdrawFarmReward({
    ...
    const txBuilder = this.createTxBuilder(feePayer);
    let userRewardToken;
    if (withdrawMint.equals(SOLMint) || withdrawMint.equals(import_web316.PublicKey.default)) {
      ...
      if (selectUserRewardToken === null) {
        ...
        txBuilder.addInstruction({
          instructions: [
            (0, import_spl_token9.createAssociatedTokenAccountInstruction)(this.scope.ownerPubKey, userRewardToken, this.scope.ownerPubKey, withdrawMint)
          ],
