...
import { PublicKey as PublicKey16, SystemProgram as SystemProgram5 } from "@solana/web3.js";
import { createAssociatedTokenAccountInstruction as createAssociatedTokenAccountInstruction2 } from "@solana/spl-token";
...
function createLogger(moduleName) {
  let logger10 = get(moduleLoggers, moduleName);
  if (!logger10) {
    const logLevel = get(moduleLevels, moduleName);
    logger10 = new Logger({ name: moduleName, logLevel });
    set(moduleLoggers, moduleName, logger10);
  }
  return logger10;
}
...
var FARM_LOCK_MINT = new PublicKey14("4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R");
var FARM_LOCK_VAULT = new PublicKey14("FrspKwj8i3pNmKwXreTveC4fu7KL5ZbGeXdZBe2XViu1");
...
function createAssociatedLedgerAccountInstruction(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: SystemProgram4.programId, isWritable: false }),
    accountMeta({ pubkey: SYSVAR_RENT_PUBKEY2, isWritable: false })
  ];
  return {
    instruction: new TransactionInstruction4({
      programId,
      keys,
      data
    }),
...
function makeCreateFarmInstruction(params) {
  ...
  return {
    instruction: new TransactionInstruction4({ programId: params.programId, keys, data }),
...
function makeCreatorWithdrawFarmRewardInstruction(params) {
  ...
  const keys = [
    accountMeta({ pubkey: TOKEN_PROGRAM_ID8, isWritable: false }),
...
function makeRestartRewardInstruction({
  ...
  return new TransactionInstruction4({ programId: farmKeys.programId, keys, data });
}
...
function makeWithdrawInstructionV6(params) {
  ...
  const [programId, id] = [new PublicKey15(farmInfo.programId), new PublicKey15(farmInfo.id)];
  ...
  const keys = [
    accountMeta({ pubkey: TOKEN_PROGRAM_ID8, isWritable: false }),
    accountMeta({ pubkey: id }),
    accountMeta({ pubkey: new PublicKey15(farmKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new PublicKey15(farmKeys.lpVault) }),
  ...
  for (let index = 0; index < farmKeys.rewardInfos.length; index++) {
    keys.push(accountMeta({ pubkey: new PublicKey15(farmKeys.rewardInfos[index].vault) }));
    keys.push(accountMeta({ pubkey: rewardAccounts[index] }));
  }
  return new TransactionInstruction4({ programId, keys, data });
}
function makeWithdrawInstructionV5(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: new PublicKey15(farmKeys.rewardInfos[0].vault) }),
...
function makeWithdrawInstructionV4(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: new PublicKey15(farmKeys.rewardInfos[1].vault) })
  ];
...
var Farm = class extends ModuleBase {
  ...
  async create({
    ...
    this.checkDisabled();
    this.scope.checkOwner();
    const lpMint = new PublicKey16(propPoolInfo.lpMint.address);
    const poolInfo = {
      lpMint,
    ...
    const { publicKey: authority, nonce } = getAssociatedAuthority({
      programId: new PublicKey16(poolInfo.programId),
    ...
    for (const rewardInfo of poolInfo.rewardInfos) {
      ...
      const rewardMint = rewardInfo.mint.equals(SOLMint) ? new PublicKey16(TOKEN_WSOL.address) : rewardInfo.mint;
      rewardInfoKey.push({
        rewardMint,
    ...
    const { account: lockUserAccount, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
      mint: new PublicKey16(poolInfo.lockInfo.lockMint),
      owner: this.scope.ownerPubKey,
  ...
  async restartReward({
    ...
    const payerPubKey = payer || this.scope.ownerPubKey;
    const rewardMint = newRewardInfo.mint.equals(SOLMint) ? new PublicKey16(TOKEN_WSOL.address) : newRewardInfo.mint;
    const rewardInfoIndex = farmKeys.rewardInfos.findIndex((item) => new PublicKey16(item.mint.address).equals(rewardMint));
  ...
  async restartRewards({
    ...
    for (const itemReward of newRewardInfos) {
      const rewardMint = itemReward.mint.equals(SOLMint) ? new PublicKey16(TOKEN_WSOL.address) : itemReward.mint;
      const rewardInfoIndex = farmKeys.rewardInfos.findIndex((item) => new PublicKey16(item.mint.address).equals(rewardMint));
      const rewardInfo = farmInfoKeys.rewardInfos[rewardInfoIndex];
      if (!rewardInfo)
        this.logAndCreateError("configuration does not exist", "rewardMint", rewardMint);
  ...
  async addNewRewardToken(params) {
    ...
    const rewardVault = getAssociatedLedgerPoolAccount({
      programId: new PublicKey16(farmInfo.programId),
      poolId: new PublicKey16(farmInfo.id),
      mint: rewardMint,
      type: "rewardVault"
    });
    const { rewardPubKey: userRewardTokenPub, newInstruction } = await this._getUserRewardInfo({
  ...
  async deposit(params) {
    ...
    const [farmProgramId, farmId] = [new PublicKey16(farmInfo.programId), new PublicKey16(farmInfo.id)];
    ...
    for (const itemReward of rewardInfos) {
      ...
      if (!ownerRewardAccount) {
        const { account: _ownerRewardAccount, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
          ...
          mint: new PublicKey16(itemReward.mint.address),
    ...
    const insParams = {
      ...
      rewardAccounts,
      userAuxiliaryLedgers: userAuxiliaryLedgers?.map((key) => new PublicKey16(key))
    };
  ...
  async withdraw(params) {
    ...
    if (version !== 4) {
      ...
      if (!ledgerData) {
        if (version !== 6) {
          const { instruction, instructionType } = createAssociatedLedgerAccountInstruction({
            id: new PublicKey16(farmKeys.id),
            programId: new PublicKey16(farmKeys.programId),
            version,
            ledger,
            owner: this.scope.ownerPubKey
          });
  ...
  async withdrawFarmReward({
    ...
    const txBuilder = this.createTxBuilder(feePayer);
    let userRewardToken;
    if (withdrawMint.equals(SOLMint) || withdrawMint.equals(PublicKey16.default)) {
      ...
      if (selectUserRewardToken === null) {
        ...
        txBuilder.addInstruction({
          instructions: [
            createAssociatedTokenAccountInstruction2(this.scope.ownerPubKey, userRewardToken, this.scope.ownerPubKey, withdrawMint)
          ],
