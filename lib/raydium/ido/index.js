...
// src/raydium/ido/index.ts
var ido_exports = {};
__export(ido_exports, {
...
module.exports = __toCommonJS(ido_exports);
...
async function getMultipleAccountsInfo(connection, publicKeys, config2) {
  ...
  if (batchRequest) {
    ...
    results = unsafeResponse.map((unsafeRes) => {
      if (unsafeRes.error)
        logger3.logWithError(`failed to get info for multiple accounts, RPC_ERROR, ${unsafeRes.error.message}`);
      return unsafeRes.result.value.map((accountInfo) => {
        if (accountInfo) {
          const { data, executable, lamports, owner, rentEpoch } = accountInfo;
          if (data.length !== 2 && data[1] !== "base64")
            logger3.logWithError(`info must be base64 encoded, RPC_ERROR`);
          return {
            data: Buffer.from(data[0], "base64"),
            executable,
            lamports,
            owner: new import_web35.PublicKey(owner),
            rentEpoch
          };
        }
        return null;
    ...
    try {
      ...
      if (error instanceof Error) {
        logger3.logWithError(`failed to get info for multiple accounts, RPC_ERROR, ${error.message}`);
      }
    }
  }
  return results.flat();
...
function makePurchaseInstruction({
  ...
  return new import_web312.TransactionInstruction({ keys, programId, data });
}
...
function makeClaimInstructionV4(params) {
  ...
  const keys = [
    {
      pubkey: import_spl_token6.TOKEN_PROGRAM_ID,
      isWritable: false,
      isSigner: false
    },
    {
      pubkey: import_web312.SYSVAR_CLOCK_PUBKEY,
      isWritable: false,
      isSigner: false
    },
    {
...
var MarketV2 = class extends ModuleBase {
  async claim({
    ...
    const [hasUnClaimedProject, hasUnClaimedBuy] = [!new import_bn10.default(ownerInfo.coin).isZero(), !new import_bn10.default(ownerInfo.pc).isZero()];
        ...
        instructions: [
          ...hasUnClaimedProject ? [
            makeClaimInstruction({ programId: poolConfigKey.programId }, {
              ...
              userIdoInfo: new import_web313.PublicKey(ownerInfo.userIdoInfo),
              userOwner: this.scope.ownerPubKey
            })
          ] : [],
          ...hasUnClaimedBuy ? [
            makeClaimInstruction({ programId: new import_web313.PublicKey(idoKeys.programId) }, {
              idoId: poolConfigKey.id,
              authority: poolConfigKey.authority,
    ...
    const keys = {
      ...
      userKeys: {
        ...
        ledgerAccount: new import_web313.PublicKey(ownerInfo.userIdoInfo),
        owner: this.scope.ownerPubKey
      }
    };
    return txBuilder.addInstruction({
