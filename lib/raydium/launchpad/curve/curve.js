...
var LaunchPadConstantProductCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (supplyMinusSellLocked.lte(new import_bn17.default(0)))
      throw Error("supplyMinusSellLocked <= 0");
    const tfMinusMf = totalFundRaising.sub(migrateFee);
    if (tfMinusMf.lte(new import_bn17.default(0)))
    ...
    if (denominator.lt(new import_bn17.default(0)))
    ...
    if (x0.lt(new import_bn17.default(0)) || y0.lt(new import_bn17.default(0)))
...
var FixedPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (supplyMinusLocked.lte(new import_bn18.default(0)))
      throw Error("invalid input 1");
    const denominator = new import_bn18.default(2).mul(totalFundRaising).sub(migrateFee);
...
var MathLaunch = class {
  ...
  static getPriceX64({ price, decimalA, decimalB }) {
    ...
    const priceX64 = new import_bn19.default(priceWithDecimals.mul(this._Q64).toFixed(0));
    return priceX64;
  }
};
__publicField(MathLaunch, "_Q64", new decimal_default(new import_bn19.default(1).shln(64).toString()));
...
var LinearPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    const supplyMinusLocked = supply.sub(totalLockedAmount);
    if (supplyMinusLocked.lte(new import_bn20.default(0)))
      throw Error("supplyMinusLocked need gt 0");
    const denominator = totalFundRaising.mul(new import_bn20.default(3)).sub(migrateFee);
    const numerator = totalFundRaising.mul(new import_bn20.default(2)).mul(supplyMinusLocked);
    const totalSellExpect = numerator.div(denominator);
    const totalSellSquared = totalSellExpect.mul(totalSellExpect);
    const a = totalFundRaising.mul(new import_bn20.default(2)).mul(Q64).div(totalSellSquared);
    if (!a.gt(new import_bn20.default(0)))
      throw Error("a need gt 0");
    if (!MaxU64.gt(a))
      throw Error("a need lt u64 max");
    return { a, b: new import_bn20.default(0), c: totalSellExpect };
  }
  static buyExactIn({ poolInfo, amount }) {
    const newQuote = poolInfo.realB.add(amount);
    const termInsideSqrt = new import_bn20.default(2).mul(newQuote).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new import_bn20.default(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
  ...
  static buyExactOut({ poolInfo, amount }) {
    ...
    const { div: _newQuoteDiv, mod: _newQuoteMod } = poolInfo.virtualA.mul(newBaseSquared).divmod(new import_bn20.default(2).mul(Q64));
    const newQuote = _newQuoteMod.isZero() ? _newQuoteDiv : _newQuoteDiv.add(new import_bn20.default(1));
  ...
  static sellExactOut({ poolInfo, amount }) {
    ...
    const termInsideSqrt = new import_bn20.default(2).mul(newB).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new import_bn20.default(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
...
var Curve = class {
  ...
  static getPoolCurvePointByInit({
    ...
    const itemStepBuy = totalFundRaising.div(new import_bn21.default(pointCount - 1));
    const zero = new import_bn21.default(0);
  ...
  static checkParam({
    ...
    if (supply.lt(config2.minSupplyA.mul(new import_bn21.default(10 ** decimals))))
    ...
    const liquidity = new import_bn21.default(new decimal_default(migrateAmountA.mul(totalFundRaising).toString()).sqrt().toFixed(0));
    if (migrateType === "amm") {
      const minLockLp = new import_bn21.default(10).pow(new import_bn21.default(decimals));
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else if (migrateType === "cpmm") {
      const minLockLp = new import_bn21.default(100);
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else {
  ...
  static splitFee({
    ...
    const platformFee = totalFeeRate.isZero() ? new import_bn21.default(0) : totalFee.mul(platformFeeRate).div(totalFeeRate);
    const shareFee = totalFeeRate.isZero() ? new import_bn21.default(0) : totalFee.mul(shareFeeRate).div(totalFeeRate);
  ...
  static calculatePreFee({ postFeeAmount, feeRate }) {
    ...
    return numerator.add(denominator).sub(new import_bn21.default(1)).div(denominator);
  }
