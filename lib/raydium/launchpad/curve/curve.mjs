...
var FixedPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (supplyMinusLocked.lte(new BN18(0)))
      throw Error("invalid input 1");
    const denominator = new BN18(2).mul(totalFundRaising).sub(migrateFee);
...
var MathLaunch = class {
  ...
  static getPriceX64({ price, decimalA, decimalB }) {
    ...
    const priceX64 = new BN19(priceWithDecimals.mul(this._Q64).toFixed(0));
    return priceX64;
  }
};
__publicField(MathLaunch, "_Q64", new decimal_default(new BN19(1).shln(64).toString()));
...
var LinearPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    const supplyMinusLocked = supply.sub(totalLockedAmount);
    if (supplyMinusLocked.lte(new BN20(0)))
      throw Error("supplyMinusLocked need gt 0");
    const denominator = totalFundRaising.mul(new BN20(3)).sub(migrateFee);
    const numerator = totalFundRaising.mul(new BN20(2)).mul(supplyMinusLocked);
    const totalSellExpect = numerator.div(denominator);
    const totalSellSquared = totalSellExpect.mul(totalSellExpect);
    const a = totalFundRaising.mul(new BN20(2)).mul(Q64).div(totalSellSquared);
    if (!a.gt(new BN20(0)))
      throw Error("a need gt 0");
    if (!MaxU64.gt(a))
      throw Error("a need lt u64 max");
    return { a, b: new BN20(0), c: totalSellExpect };
  }
  static buyExactIn({ poolInfo, amount }) {
    const newQuote = poolInfo.realB.add(amount);
    const termInsideSqrt = new BN20(2).mul(newQuote).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN20(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
  ...
  static buyExactOut({ poolInfo, amount }) {
    ...
    const { div: _newQuoteDiv, mod: _newQuoteMod } = poolInfo.virtualA.mul(newBaseSquared).divmod(new BN20(2).mul(Q64));
    const newQuote = _newQuoteMod.isZero() ? _newQuoteDiv : _newQuoteDiv.add(new BN20(1));
  ...
  static sellExactOut({ poolInfo, amount }) {
    ...
    const termInsideSqrt = new BN20(2).mul(newB).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN20(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
...
var Curve = class {
  ...
  static getPoolCurvePointByInit({
    ...
    const itemStepBuy = totalFundRaising.div(new BN21(pointCount - 1));
  ...
  static checkParam({
    ...
    if (supply.lt(config2.minSupplyA.mul(new BN21(10 ** decimals))))
    ...
    const liquidity = new BN21(new decimal_default(migrateAmountA.mul(totalFundRaising).toString()).sqrt().toFixed(0));
    if (migrateType === "amm") {
      const minLockLp = new BN21(10).pow(new BN21(decimals));
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else if (migrateType === "cpmm") {
      const minLockLp = new BN21(100);
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else {
  ...
  static splitFee({
    ...
    const platformFee = totalFeeRate.isZero() ? new BN21(0) : totalFee.mul(platformFeeRate).div(totalFeeRate);
    const shareFee = totalFeeRate.isZero() ? new BN21(0) : totalFee.mul(shareFeeRate).div(totalFeeRate);
  ...
  static calculatePreFee({ postFeeAmount, feeRate }) {
    ...
    return numerator.add(denominator).sub(new BN21(1)).div(denominator);
  }
