...
var MathLaunch = class {
  ...
  static getPriceX64({ price, decimalA, decimalB }) {
    ...
    const priceX64 = new import_bn.default(priceWithDecimals.mul(this._Q64).toFixed(0));
    return priceX64;
  }
};
__publicField(MathLaunch, "_Q64", new decimal_default(new import_bn.default(1).shln(64).toString()));
function checkPoolToAmm({
  ...
  const liquidity = new import_bn.default(new decimal_default(migrateAmountA.mul(totalFundRaisingB).toString()).sqrt().toFixed(0));
  if (migrateType === "amm") {
    if (liquidity.gt(new import_bn.default(10).pow(new import_bn.default(decimalsA))))
      return true;
  } else if (migrateType === "cpmm") {
    if (liquidity.gt(new import_bn.default(100)))
      return true;
  } else {
