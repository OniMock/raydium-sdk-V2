...
__publicField(MathLaunch, "_Q64", new decimal_default(new BN(1).shln(64).toString()));
function checkPoolToAmm({
  ...
  const liquidity = new BN(new decimal_default(migrateAmountA.mul(totalFundRaisingB).toString()).sqrt().toFixed(0));
  if (migrateType === "amm") {
    if (liquidity.gt(new BN(10).pow(new BN(decimalsA))))
      return true;
  } else if (migrateType === "cpmm") {
    if (liquidity.gt(new BN(100)))
      return true;
  } else {
