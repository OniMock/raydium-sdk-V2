...
var MathLaunch = class {
  ...
  static getPriceX64({ price, decimalA, decimalB }) {
    ...
    const priceX64 = new BN18(priceWithDecimals.mul(this._Q64).toFixed(0));
    return priceX64;
  }
};
__publicField(MathLaunch, "_Q64", new decimal_default(new BN18(1).shln(64).toString()));
...
var LinearPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    const denominator = totalFundRaising.mul(new BN19(3)).sub(migrateFee);
    const numerator = totalFundRaising.mul(new BN19(2)).mul(supplyMinusLocked);
    const totalSellExpect = numerator.div(denominator);
    const totalSellSquared = totalSellExpect.mul(totalSellExpect);
    const a = totalFundRaising.mul(new BN19(2)).mul(Q64).div(totalSellSquared);
    if (!a.gt(new BN19(0)))
      throw Error("a need gt 0");
    if (!MaxU64.gt(a))
      throw Error("a need lt u64 max");
    return { a, b: new BN19(0), c: totalSellExpect };
  }
  static buyExactIn({ poolInfo, amount }) {
    const newQuote = poolInfo.realB.add(amount);
    const termInsideSqrt = new BN19(2).mul(newQuote).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN19(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
  ...
  static buyExactOut({ poolInfo, amount }) {
    ...
    const { div: _newQuoteDiv, mod: _newQuoteMod } = poolInfo.virtualA.mul(newBaseSquared).divmod(new BN19(2).mul(Q64));
    const newQuote = _newQuoteMod.isZero() ? _newQuoteDiv : _newQuoteDiv.add(new BN19(1));
  ...
  static sellExactOut({ poolInfo, amount }) {
    ...
    const termInsideSqrt = new BN19(2).mul(newB).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN19(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
