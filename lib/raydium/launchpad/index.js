...
// src/raydium/launchpad/index.ts
...
function parseBigNumberish(value) {
  ...
  if (value instanceof import_bn2.default) {
    return value;
  }
  if (typeof value === "string") {
    if (value.match(/^-?[0-9]+$/)) {
      return new import_bn2.default(value);
    }
  ...
  if (typeof value === "number") {
    ...
    return new import_bn2.default(String(value));
  }
  if (typeof value === "bigint") {
    return new import_bn2.default(value.toString());
  }
...
var Fraction = class {
  numerator;
  denominator;
  constructor(numerator, denominator = new import_bn3.default(1)) {
...
function ceilDiv(tokenAmount, feeNumerator, feeDenominator) {
  return tokenAmount.mul(feeNumerator).add(feeDenominator).sub(new import_bn7.default(1)).div(feeDenominator);
}
...
function findProgramAddress(seeds, programId) {
  const [publicKey2, nonce] = import_web37.PublicKey.findProgramAddressSync(seeds, programId);
...
function ceilDivBN(amountA, amountB) {
  ...
  if (quotient.isZero())
    return new import_bn8.default(1);
  const remainder = amountA.mod(amountB);
  if (remainder.gt(new import_bn8.default(0))) {
    return quotient.add(new import_bn8.default(1));
  }
  return quotient;
}
...
function checkPoolToAmm({
  ...
  const liquidity = new import_bn20.default(new decimal_default(migrateAmountA.mul(totalFundRaisingB).toString()).sqrt().toFixed(0));
  if (migrateType === "amm") {
    if (liquidity.gt(new import_bn20.default(10).pow(new import_bn20.default(decimalsA))))
      return true;
  } else if (migrateType === "cpmm") {
    if (liquidity.gt(new import_bn20.default(100)))
      return true;
  } else {
