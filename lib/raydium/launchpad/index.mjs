...
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID2 } from "@solana/spl-token";
...
function ceilDiv(tokenAmount, feeNumerator, feeDenominator) {
  return tokenAmount.mul(feeNumerator).add(feeDenominator).sub(new BN7(1)).div(feeDenominator);
}
...
function findProgramAddress(seeds, programId) {
  const [publicKey2, nonce] = PublicKey7.findProgramAddressSync(seeds, programId);
...
function ceilDivBN(amountA, amountB) {
  ...
  if (remainder.gt(new BN8(0))) {
    return quotient.add(new BN8(1));
  }
  return quotient;
}
...
function buyExactInInstruction(programId, owner, auth, configId, platformId, poolId, userTokenAccountA, userTokenAccountB, vaultA, vaultB, mintA, mintB, tokenProgramA, tokenProgramB, amountB, minAmountA, shareFeeRate, shareFeeReceiver) {
  ...
  dataLayout.encode({
    amountB,
    minAmountA,
    shareFeeRate: shareFeeRate ?? new BN17(0)
  }, data);
  return new TransactionInstruction4({
    keys,
    programId,
...
function claimPlatformFee(programId, platformClaimFeeWallet, auth, poolId, platformId, vaultB, userTokenAccountB, mintB, tokenProgramB) {
  const keys = [
    ...
    { pubkey: SystemProgram4.programId, isSigner: false, isWritable: true },
    { pubkey: ASSOCIATED_TOKEN_PROGRAM_ID2, isSigner: false, isWritable: true }
  ];
  return new TransactionInstruction4({
    keys,
    programId,
...
var LaunchPadConstantProductCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (x0.lt(new BN18(0)) || y0.lt(new BN18(0)))
...
var FixedPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (supplyMinusLocked.lte(new BN19(0)))
      throw Error("invalid input 1");
    const denominator = new BN19(2).mul(totalFundRaising).sub(migrateFee);
...
var MathLaunch = class {
  ...
  static getPriceX64({ price, decimalA, decimalB }) {
    ...
    const priceX64 = new BN20(priceWithDecimals.mul(this._Q64).toFixed(0));
    return priceX64;
  }
};
__publicField(MathLaunch, "_Q64", new decimal_default(new BN20(1).shln(64).toString()));
function checkPoolToAmm({
  ...
  const liquidity = new BN20(new decimal_default(migrateAmountA.mul(totalFundRaisingB).toString()).sqrt().toFixed(0));
  if (migrateType === "amm") {
    if (liquidity.gt(new BN20(10).pow(new BN20(decimalsA))))
      return true;
  } else if (migrateType === "cpmm") {
    if (liquidity.gt(new BN20(100)))
      return true;
  } else {
...
var LinearPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    const supplyMinusLocked = supply.sub(totalLockedAmount);
    if (supplyMinusLocked.lte(new BN21(0)))
      throw Error("supplyMinusLocked need gt 0");
    const denominator = totalFundRaising.mul(new BN21(3)).sub(migrateFee);
    const numerator = totalFundRaising.mul(new BN21(2)).mul(supplyMinusLocked);
    const totalSellExpect = numerator.div(denominator);
    const totalSellSquared = totalSellExpect.mul(totalSellExpect);
    const a = totalFundRaising.mul(new BN21(2)).mul(Q64).div(totalSellSquared);
    if (!a.gt(new BN21(0)))
      throw Error("a need gt 0");
    if (!MaxU64.gt(a))
      throw Error("a need lt u64 max");
    return { a, b: new BN21(0), c: totalSellExpect };
  }
  static buyExactIn({ poolInfo, amount }) {
    const newQuote = poolInfo.realB.add(amount);
    const termInsideSqrt = new BN21(2).mul(newQuote).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN21(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
  ...
  static buyExactOut({ poolInfo, amount }) {
    ...
    const { div: _newQuoteDiv, mod: _newQuoteMod } = poolInfo.virtualA.mul(newBaseSquared).divmod(new BN21(2).mul(Q64));
    const newQuote = _newQuoteMod.isZero() ? _newQuoteDiv : _newQuoteDiv.add(new BN21(1));
  ...
  static sellExactOut({ poolInfo, amount }) {
    ...
    const termInsideSqrt = new BN21(2).mul(newB).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new BN21(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
...
var Curve = class {
  ...
  static getPoolCurvePointByInit({
    ...
    const itemStepBuy = totalFundRaising.div(new BN22(pointCount - 1));
    const zero = new BN22(0);
  ...
  static checkParam({
    ...
    if (supply.lt(config2.minSupplyA.mul(new BN22(10 ** decimals))))
    ...
    const liquidity = new BN22(new decimal_default(migrateAmountA.mul(totalFundRaising).toString()).sqrt().toFixed(0));
    if (migrateType === "amm") {
      const minLockLp = new BN22(10).pow(new BN22(decimals));
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else if (migrateType === "cpmm") {
      const minLockLp = new BN22(100);
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else {
      throw Error("migrate type error");
  ...
  static splitFee({
    ...
    const platformFee = totalFeeRate.isZero() ? new BN22(0) : totalFee.mul(platformFeeRate).div(totalFeeRate);
    const shareFee = totalFeeRate.isZero() ? new BN22(0) : totalFee.mul(shareFeeRate).div(totalFeeRate);
  ...
  static calculatePreFee({ postFeeAmount, feeRate }) {
    ...
    return numerator.add(denominator).sub(new BN22(1)).div(denominator);
  }
...
import { NATIVE_MINT, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID9, createAssociatedTokenAccountIdempotentInstruction } from "@solana/spl-token";
import BN23 from "bn.js";
import { PublicKey as PublicKey17 } from "@solana/web3.js";
var LaunchpadPoolInitParam = {
  initPriceX64: new BN23("515752397214619"),
  supply: new BN23(1e15),
  totalSellA: new BN23(7931e11),
  totalFundRaisingB: new BN23(85e9),
  totalLockedAmount: new BN23("0"),
  cliffPeriod: new BN23("0"),
  unlockPeriod: new BN23("0"),
  decimals: 6,
  virtualA: new BN23("1073471847374405"),
  virtualB: new BN23("30050573465"),
  realA: new BN23(0),
  realB: new BN23(0),
  protocolFee: new BN23(0),
  platformId: new PublicKey17("4Bu96XjU84XjPDSpveTVf6LYGCkfW5FK7SNkREWcEfV4"),
  vestingSchedule: {
    totalLockedAmount: new BN23(0),
    cliffPeriod: new BN23(0),
    unlockPeriod: new BN23(0),
    startTime: new BN23(0),
    totalAllocatedShare: new BN23(0)
  }
};
var SLIPPAGE_UNIT = new BN23(1e4);
export {
  Curve,
  CurveBase,
  FixedPriceCurve,
