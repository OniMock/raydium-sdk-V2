...
var launchpad_exports = {};
__export(launchpad_exports, {
  LaunchpadPoolInitParam: () => LaunchpadPoolInitParam,
  default: () => LaunchpadModule
});
module.exports = __toCommonJS(launchpad_exports);
...
function createLogger(moduleName) {
  let logger7 = (0, import_lodash.get)(moduleLoggers, moduleName);
  if (!logger7) {
    const logLevel = (0, import_lodash.get)(moduleLevels, moduleName);
    logger7 = new Logger({ name: moduleName, logLevel });
    (0, import_lodash.set)(moduleLoggers, moduleName, logger7);
  }
  return logger7;
}
      ...
      return unsafeRes.result.value.map((accountInfo) => {
        if (accountInfo) {
          ...
          return {
            ...
            executable,
            lamports,
            owner: new import_web38.PublicKey(owner),
            rentEpoch
          };
        }
        return null;
...
function publicKey(property) {
  return new WrappedLayout(blob(32), (b) => new import_web312.PublicKey(b), (key) => key.toBuffer(), property);
}
...
function buyExactInInstruction(programId, owner, auth, configId, platformId, poolId, userTokenAccountA, userTokenAccountB, vaultA, vaultB, mintA, mintB, tokenProgramA, tokenProgramB, amountB, minAmountA, shareFeeRate, shareFeeReceiver) {
  ...
  dataLayout.encode({
    amountB,
    minAmountA,
    shareFeeRate: shareFeeRate ?? new import_bn17.default(0)
  }, data);
  return new import_web316.TransactionInstruction({
    keys,
    programId,
...
function claimPlatformFee(programId, platformClaimFeeWallet, auth, poolId, platformId, vaultB, userTokenAccountB, mintB, tokenProgramB) {
  const keys = [
    ...
    { pubkey: import_web316.SystemProgram.programId, isSigner: false, isWritable: true },
    { pubkey: import_spl_token10.ASSOCIATED_TOKEN_PROGRAM_ID, isSigner: false, isWritable: true }
  ];
  return new import_web316.TransactionInstruction({
    keys,
    programId,
...
var LaunchPadConstantProductCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (supplyMinusSellLocked.lte(new import_bn18.default(0)))
      throw Error("supplyMinusSellLocked <= 0");
    const tfMinusMf = totalFundRaising.sub(migrateFee);
    if (tfMinusMf.lte(new import_bn18.default(0)))
    ...
    if (denominator.lt(new import_bn18.default(0)))
    ...
    if (x0.lt(new import_bn18.default(0)) || y0.lt(new import_bn18.default(0)))
...
var FixedPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    if (supplyMinusLocked.lte(new import_bn19.default(0)))
      throw Error("invalid input 1");
    const denominator = new import_bn19.default(2).mul(totalFundRaising).sub(migrateFee);
...
var MathLaunch = class {
  ...
  static getPriceX64({ price, decimalA, decimalB }) {
    ...
    const priceX64 = new import_bn20.default(priceWithDecimals.mul(this._Q64).toFixed(0));
    return priceX64;
  }
};
__publicField(MathLaunch, "_Q64", new decimal_default(new import_bn20.default(1).shln(64).toString()));
...
var LinearPriceCurve = class extends CurveBase {
  ...
  static getInitParam({
    ...
    const supplyMinusLocked = supply.sub(totalLockedAmount);
    if (supplyMinusLocked.lte(new import_bn21.default(0)))
      throw Error("supplyMinusLocked need gt 0");
    const denominator = totalFundRaising.mul(new import_bn21.default(3)).sub(migrateFee);
    const numerator = totalFundRaising.mul(new import_bn21.default(2)).mul(supplyMinusLocked);
    const totalSellExpect = numerator.div(denominator);
    const totalSellSquared = totalSellExpect.mul(totalSellExpect);
    const a = totalFundRaising.mul(new import_bn21.default(2)).mul(Q64).div(totalSellSquared);
    if (!a.gt(new import_bn21.default(0)))
      throw Error("a need gt 0");
    if (!MaxU64.gt(a))
      throw Error("a need lt u64 max");
    return { a, b: new import_bn21.default(0), c: totalSellExpect };
  }
  static buyExactIn({ poolInfo, amount }) {
    const newQuote = poolInfo.realB.add(amount);
    const termInsideSqrt = new import_bn21.default(2).mul(newQuote).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new import_bn21.default(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
  ...
  static buyExactOut({ poolInfo, amount }) {
    ...
    const { div: _newQuoteDiv, mod: _newQuoteMod } = poolInfo.virtualA.mul(newBaseSquared).divmod(new import_bn21.default(2).mul(Q64));
    const newQuote = _newQuoteMod.isZero() ? _newQuoteDiv : _newQuoteDiv.add(new import_bn21.default(1));
  ...
  static sellExactOut({ poolInfo, amount }) {
    ...
    const termInsideSqrt = new import_bn21.default(2).mul(newB).mul(Q64).div(poolInfo.virtualA);
    const sqrtTerm = new import_bn21.default(new decimal_default(termInsideSqrt.toString()).sqrt().toFixed(0));
...
var Curve = class {
  ...
  static getPoolCurvePointByInit({
    ...
    const itemStepBuy = totalFundRaising.div(new import_bn22.default(pointCount - 1));
    const zero = new import_bn22.default(0);
  ...
  static checkParam({
    ...
    if (supply.lt(config2.minSupplyA.mul(new import_bn22.default(10 ** decimals))))
    ...
    const liquidity = new import_bn22.default(new decimal_default(migrateAmountA.mul(totalFundRaising).toString()).sqrt().toFixed(0));
    if (migrateType === "amm") {
      const minLockLp = new import_bn22.default(10).pow(new import_bn22.default(decimals));
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else if (migrateType === "cpmm") {
      const minLockLp = new import_bn22.default(100);
      if (liquidity.lte(minLockLp))
        throw Error("check migrate lp error");
    } else {
  ...
  static splitFee({
    ...
    const platformFee = totalFeeRate.isZero() ? new import_bn22.default(0) : totalFee.mul(platformFeeRate).div(totalFeeRate);
    const shareFee = totalFeeRate.isZero() ? new import_bn22.default(0) : totalFee.mul(shareFeeRate).div(totalFeeRate);
  ...
  static calculatePreFee({ postFeeAmount, feeRate }) {
    ...
    return numerator.add(denominator).sub(new import_bn22.default(1)).div(denominator);
  }
...
var LaunchpadPoolInitParam = {
  initPriceX64: new import_bn23.default("515752397214619"),
  supply: new import_bn23.default(1e15),
  totalSellA: new import_bn23.default(7931e11),
  totalFundRaisingB: new import_bn23.default(85e9),
  totalLockedAmount: new import_bn23.default("0"),
  cliffPeriod: new import_bn23.default("0"),
  unlockPeriod: new import_bn23.default("0"),
  decimals: 6,
  virtualA: new import_bn23.default("1073471847374405"),
  virtualB: new import_bn23.default("30050573465"),
  realA: new import_bn23.default(0),
  realB: new import_bn23.default(0),
  protocolFee: new import_bn23.default(0),
  platformId: new import_web317.PublicKey("4Bu96XjU84XjPDSpveTVf6LYGCkfW5FK7SNkREWcEfV4"),
  vestingSchedule: {
    totalLockedAmount: new import_bn23.default(0),
    cliffPeriod: new import_bn23.default(0),
    unlockPeriod: new import_bn23.default(0),
    startTime: new import_bn23.default(0),
    totalAllocatedShare: new import_bn23.default(0)
  }
};
var SLIPPAGE_UNIT = new import_bn23.default(1e4);
var LaunchpadModule = class extends ModuleBase {
  ...
  async createLaunchpad({
    ...
    if (buyAmount.lte(new import_bn23.default(0)))
    ...
    const totalLockedAmount = extraConfigs?.totalLockedAmount ?? new import_bn23.default(0);
    ...
    const poolInfo = {
      epoch: new import_bn23.default(896),
      bump: 254,
      status: 0,
      ...
      supply,
      totalSellA,
      mintA: new import_web317.PublicKey(mintA),
      mintB,
    ...
    const { c } = initCurve.getInitParam({
      ...
      totalFundRaising: poolInfo.totalFundRaisingB,
      totalLockedAmount,
      totalSell: configInfo.curveType === 0 ? poolInfo.totalSellA : new import_bn23.default(0),
      migrateFee: configInfo.migrateFee
    });
    try {
      Curve.checkParam({
    ...
    txBuilder.addInstruction({
      instructions: [
        initialize(programId, feePayer ?? this.scope.ownerPubKey, this.scope.ownerPubKey, configId, platformId, authProgramId, poolId, mintA, mintB, vaultA, vaultB, metaId, import_spl_token11.TOKEN_PROGRAM_ID, import_spl_token11.TOKEN_PROGRAM_ID, decimals, name, symbol, uri || "https://", {
          type: curType === 0 ? "ConstantCurve" : curType === 1 ? "FixedCurve" : curType === 2 ? "LinearCurve" : "ConstantCurve",
          totalSellA,
          migrateType,
          supply,
          totalFundRaisingB
        }, totalLockedAmount, extraConfigs?.cliffPeriod ?? new import_bn23.default(0), extraConfigs?.unlockPeriod ?? new import_bn23.default(0))
      ]
    });
    let outAmount = new import_bn23.default(0);
    let splitIns;
  ...
  async buyToken({
    ...
    authProgramId,
    mintA,
    mintB = import_spl_token11.NATIVE_MINT,
    ...
    slippage,
    shareFeeRate = new import_bn23.default(0),
    shareFeeReceiver,
    associatedOnly = true,
    checkCreateATAOwner = false
  }) {
    ...
    const mintBUseSOLBalance = mintB.equals(import_spl_token11.NATIVE_MINT);
    ...
    const minMintAAmount = propMinMintAAmount ?? (slippage ? new import_bn23.default(decimalAmountA.mul(multiplier).toFixed(0)) : calculatedAmount.amountA);
    ...
    const shareATA = shareFeeReceiver ? getATAAddress(shareFeeReceiver, mintB, import_spl_token11.TOKEN_PROGRAM_ID).publicKey : void 0;
    if (shareATA) {
      txBuilder.addInstruction({
        instructions: [
          (0, import_spl_token11.createAssociatedTokenAccountIdempotentInstruction)(this.scope.ownerPubKey, shareATA, shareFeeReceiver, mintB)
        ]
      });
    }
    txBuilder.addInstruction({
      instructions: [
        buyExactInInstruction(programId, this.scope.ownerPubKey, authProgramId, poolInfo.configId, poolInfo.platformId, poolId, userTokenAccountA, userTokenAccountB, poolInfo.vaultA, poolInfo.vaultB, mintA, mintB, import_spl_token11.TOKEN_PROGRAM_ID, import_spl_token11.TOKEN_PROGRAM_ID, calculatedAmount.amountB.lt(buyAmount) ? calculatedAmount.amountB : buyAmount, minMintAAmount, shareFeeRate, shareATA)
      ]
    });
  ...
  async sellToken({
    ...
    authProgramId = authProgramId ?? getPdaLaunchpadAuth(programId).publicKey;
    const txBuilder = this.createTxBuilder(feePayer);
    if (sellAmount.lte(new import_bn23.default(0)))
    ...
    const multiplier = slippage ? new decimal_default(SLIPPAGE_UNIT.sub(slippage).toNumber() / SLIPPAGE_UNIT.toNumber()).clampedTo(0, 1) : new decimal_default(1);
    const minAmountB = propMinAmountB ?? (slippage ? new import_bn23.default(decimalAmountB.mul(multiplier).toFixed(0)) : calculatedAmount.amountB);
    if (minAmountB.lte(new import_bn23.default(0)))
  ...
  async claimPlatformFee({
    ...
    mintB: propsMintB,
    vaultB: propsVaultB,
    mintBProgram = import_spl_token11.TOKEN_PROGRAM_ID,
    txVersion,
    computeBudgetConfig,
    txTipConfig,
    feePayer
    ...
    const userTokenAccountB = getATAAddress(this.scope.ownerPubKey, mintB, import_spl_token11.TOKEN_PROGRAM_ID).publicKey;
    txBuilder.addInstruction({
      instructions: [
        (0, import_spl_token11.createAssociatedTokenAccountIdempotentInstruction)(this.scope.ownerPubKey, userTokenAccountB, this.scope.ownerPubKey, mintB)
      ]
    });
    txBuilder.addInstruction({
      instructions: [
  ...
  async claimAllPlatformFee({
    ...
    allPlatformPool.forEach((data) => {
      ...
      const userTokenAccountB = getATAAddress(this.scope.ownerPubKey, pool.mintB, import_spl_token11.TOKEN_PROGRAM_ID).publicKey;
      txBuilder.addInstruction({
        instructions: [
          (0, import_spl_token11.createAssociatedTokenAccountIdempotentInstruction)(this.scope.ownerPubKey, userTokenAccountB, this.scope.ownerPubKey, pool.mintB)
        ]
      });
      txBuilder.addInstruction({
        instructions: [
          claimPlatformFee(programId, platformClaimFeeWallet, authProgramId, data.pubkey, platformId, pool.vaultB, userTokenAccountB, pool.mintB, import_spl_token11.TOKEN_PROGRAM_ID)
        ]
      });
    });
    txBuilder.addTipInstruction(txTipConfig);
  ...
  async claimVesting({
    ...
    const userTokenAccountA = getATAAddress(this.scope.ownerPubKey, poolInfo.mintA, import_spl_token11.TOKEN_PROGRAM_ID).publicKey;
    txBuilder.addInstruction({
      instructions: [
        (0, import_spl_token11.createAssociatedTokenAccountIdempotentInstruction)(this.scope.ownerPubKey, userTokenAccountA, this.scope.ownerPubKey, poolInfo.mintA)
      ]
    });
    txBuilder.addInstruction({
      instructions: [
        claimVestedToken(programId, this.scope.ownerPubKey, authProgramId, poolId, vestingRecord, userTokenAccountA, poolInfo.vaultA, poolInfo.mintA, import_spl_token11.TOKEN_PROGRAM_ID)
      ]
    });
    txBuilder.addCustomComputeBudget(computeBudgetConfig);
    txBuilder.addTipInstruction(txTipConfig);
