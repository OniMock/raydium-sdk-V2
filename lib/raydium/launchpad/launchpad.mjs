...
var LaunchpadModule = class extends ModuleBase {
  ...
  async createLaunchpad({
    ...
    const totalLockedAmount = extraConfigs?.totalLockedAmount ?? new BN23(0);
    ...
    const { c } = initCurve.getInitParam({
      ...
      totalFundRaising: poolInfo.totalFundRaisingB,
      totalLockedAmount,
      totalSell: configInfo.curveType === 0 ? poolInfo.totalSellA : new BN23(0),
      migrateFee: configInfo.migrateFee
    });
    try {
      Curve.checkParam({
    ...
    txBuilder.addInstruction({
      instructions: [
        initialize(programId, feePayer ?? this.scope.ownerPubKey, this.scope.ownerPubKey, configId, platformId, authProgramId, poolId, mintA, mintB, vaultA, vaultB, metaId, TOKEN_PROGRAM_ID9, TOKEN_PROGRAM_ID9, decimals, name, symbol, uri || "https://", {
          type: curType === 0 ? "ConstantCurve" : curType === 1 ? "FixedCurve" : curType === 2 ? "LinearCurve" : "ConstantCurve",
          totalSellA,
          migrateType,
          supply,
          totalFundRaisingB
        }, totalLockedAmount, extraConfigs?.cliffPeriod ?? new BN23(0), extraConfigs?.unlockPeriod ?? new BN23(0))
      ]
    });
    let outAmount = new BN23(0);
    let splitIns;
  ...
  async buyToken({
    ...
    const minMintAAmount = propMinMintAAmount ?? (slippage ? new BN23(decimalAmountA.mul(multiplier).toFixed(0)) : calculatedAmount.amountA);
    ...
    const shareATA = shareFeeReceiver ? getATAAddress(shareFeeReceiver, mintB, TOKEN_PROGRAM_ID9).publicKey : void 0;
    if (shareATA) {
      txBuilder.addInstruction({
        instructions: [
  ...
  async sellToken({
    ...
    const multiplier = slippage ? new decimal_default(SLIPPAGE_UNIT.sub(slippage).toNumber() / SLIPPAGE_UNIT.toNumber()).clampedTo(0, 1) : new decimal_default(1);
    const minAmountB = propMinAmountB ?? (slippage ? new BN23(decimalAmountB.mul(multiplier).toFixed(0)) : calculatedAmount.amountB);
    if (minAmountB.lte(new BN23(0)))
  ...
  async claimPlatformFee({
    ...
    mintB: propsMintB,
    vaultB: propsVaultB,
    mintBProgram = TOKEN_PROGRAM_ID9,
    txVersion,
    computeBudgetConfig,
    txTipConfig,
    feePayer
    ...
    const userTokenAccountB = getATAAddress(this.scope.ownerPubKey, mintB, TOKEN_PROGRAM_ID9).publicKey;
    txBuilder.addInstruction({
      instructions: [
        createAssociatedTokenAccountIdempotentInstruction(this.scope.ownerPubKey, userTokenAccountB, this.scope.ownerPubKey, mintB)
      ]
    });
    txBuilder.addInstruction({
      instructions: [
  ...
  async claimAllPlatformFee({
    ...
    allPlatformPool.forEach((data) => {
      ...
      const userTokenAccountB = getATAAddress(this.scope.ownerPubKey, pool.mintB, TOKEN_PROGRAM_ID9).publicKey;
      txBuilder.addInstruction({
        instructions: [
          createAssociatedTokenAccountIdempotentInstruction(this.scope.ownerPubKey, userTokenAccountB, this.scope.ownerPubKey, pool.mintB)
        ]
      });
      txBuilder.addInstruction({
        instructions: [
          claimPlatformFee(programId, platformClaimFeeWallet, authProgramId, data.pubkey, platformId, pool.vaultB, userTokenAccountB, pool.mintB, TOKEN_PROGRAM_ID9)
        ]
      });
    });
    txBuilder.addTipInstruction(txTipConfig);
  ...
  async claimVesting({
    ...
    const userTokenAccountA = getATAAddress(this.scope.ownerPubKey, poolInfo.mintA, TOKEN_PROGRAM_ID9).publicKey;
    txBuilder.addInstruction({
      instructions: [
        createAssociatedTokenAccountIdempotentInstruction(this.scope.ownerPubKey, userTokenAccountA, this.scope.ownerPubKey, poolInfo.mintA)
      ]
    });
    txBuilder.addInstruction({
      instructions: [
        claimVestedToken(programId, this.scope.ownerPubKey, authProgramId, poolId, vestingRecord, userTokenAccountA, poolInfo.vaultA, poolInfo.mintA, TOKEN_PROGRAM_ID9)
      ]
    });
    txBuilder.addCustomComputeBudget(computeBudgetConfig);
    txBuilder.addTipInstruction(txTipConfig);
