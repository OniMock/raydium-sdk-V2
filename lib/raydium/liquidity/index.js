...
// src/raydium/liquidity/index.ts
...
var BNLayout = class extends Layout {
  ...
  decode(b, offset2 = 0) {
    const num = new import_bn2.default(this.blob.decode(b, offset2), 10, "le");
    if (this.signed) {
  ...
  encode(src, b, offset2 = 0) {
    if (typeof src === "number")
      src = new import_bn2.default(src);
    if (this.signed) {
...
function seq2(elementLayout, count, property) {
  let parsedCount;
  const superCount = typeof count === "number" ? count : (0, import_bn2.isBN)(count) ? count.toNumber() : new Proxy(count, {
    get(target, property2) {
      if (!parsedCount) {
        const countProperty = Reflect.get(target, "count");
        parsedCount = (0, import_bn2.isBN)(countProperty) ? countProperty.toNumber() : countProperty;
...
function createLogger(moduleName) {
  let logger9 = (0, import_lodash.get)(moduleLoggers, moduleName);
  if (!logger9) {
    const logLevel = (0, import_lodash.get)(moduleLevels, moduleName);
    logger9 = new Logger({ name: moduleName, logLevel });
    (0, import_lodash.set)(moduleLoggers, moduleName, logger9);
  }
  return logger9;
}
...
async function simulateMultipleInstruction(connection, instructions, keyword, batchRequest = true) {
  const feePayer = new import_web32.PublicKey("RaydiumSimuLateTransaction11111111111111111");
  const transactions = [];
  let transaction = new import_web32.Transaction();
  transaction.feePayer = feePayer;
  for (const instruction of instructions) {
    if (!forecastTransactionSize([...transaction.instructions, instruction], [feePayer])) {
      transactions.push(transaction);
      transaction = new import_web32.Transaction();
      transaction.feePayer = feePayer;
    }
...
function findProgramAddress(seeds, programId) {
  const [publicKey2, nonce] = import_web32.PublicKey.findProgramAddressSync(seeds, programId);
...
var SOL_INFO = {
  chainId: 101,
  address: import_web33.PublicKey.default.toBase58(),
...
var commonSystemAccountMeta = [
  ...
  accountMeta({ pubkey: import_web34.SystemProgram.programId, isWritable: false }),
  accountMeta({ pubkey: import_web34.SYSVAR_RENT_PUBKEY, isWritable: false })
];
function validateAndParsePublicKey({
  ...
  if (publicKey2 instanceof import_web34.PublicKey) {
  ...
  if (typeof publicKey2 === "string") {
    if (publicKey2 === import_web34.PublicKey.default.toBase58())
      return import_web34.PublicKey.default;
    try {
      const key = new import_web34.PublicKey(publicKey2);
      return key;
    } catch {
...
function tryParsePublicKey(v) {
  try {
    return new import_web34.PublicKey(v);
  } catch (e) {
    return v;
  }
}
var MEMO_PROGRAM_ID = new import_web34.PublicKey("MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr");
var MEMO_PROGRAM_ID2 = new import_web34.PublicKey("MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr");
var RENT_PROGRAM_ID = new import_web34.PublicKey("SysvarRent111111111111111111111111111111111");
var CLOCK_PROGRAM_ID = new import_web34.PublicKey("SysvarC1ock11111111111111111111111111111111");
var METADATA_PROGRAM_ID = new import_web34.PublicKey("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s");
var INSTRUCTION_PROGRAM_ID = new import_web34.PublicKey("Sysvar1nstructions1111111111111111111111111");
var SYSTEM_PROGRAM_ID = import_web34.SystemProgram.programId;
var RAYMint = new import_web34.PublicKey("4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R");
var PAIMint = new import_web34.PublicKey("Ea5SjE2Y6yvCeW5dYTn7PYMuW5ikXkvbGdcmSnXeaLjS");
var SRMMint = new import_web34.PublicKey("SRMuApVNdxXokk5GT7XD5cUUgXMBCoAz2LHeuAoKWRt");
var USDCMint = new import_web34.PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
var USDTMint = new import_web34.PublicKey("Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB");
var mSOLMint = new import_web34.PublicKey("mSoLzYCxHdYgdzU16g5QSh3i5K3z3KZK7ytfqcJm7So");
var stSOLMint = new import_web34.PublicKey("7dHbWXmci3dT8UFYWYZweBLXgycu7Y3iL6trKn1Y7ARj");
var USDHMint = new import_web34.PublicKey("USDH1SM1ojwWUga67PGrgFWUHibbjqMvuMaDkRJTgkX");
var NRVMint = new import_web34.PublicKey("NRVwhjBQiUPYtfDT5zRBVJajzFQHaBUNtC7SNVvqRFa");
var ANAMint = new import_web34.PublicKey("ANAxByE6G2WjFp7A4NqtWYXb3mgruyzZYg3spfxe6Lbo");
var ETHMint = new import_web34.PublicKey("7vfCXTUXx5WJV5JADk17DUJ4ksgau7utNKj4b963voxs");
var WSOLMint = new import_web34.PublicKey("So11111111111111111111111111111111111111112");
var SOLMint = import_web34.PublicKey.default;
...
var _Token = class {
  ...
  constructor({ mint, decimals, symbol, name, skipMint = false, isToken2022 = false }) {
    if (mint === SOLMint.toBase58() || mint instanceof import_web35.PublicKey && SOLMint.equals(mint)) {
      this.decimals = TOKEN_WSOL.decimals;
      this.symbol = TOKEN_WSOL.symbol;
      this.name = TOKEN_WSOL.name;
      this.mint = new import_web35.PublicKey(TOKEN_WSOL.address);
    ...
    this.mint = skipMint ? import_web35.PublicKey.default : validateAndParsePublicKey({ publicKey: mint });
...
function parseBigNumberish(value) {
  const logger9 = createLogger("Raydium_parseBigNumberish");
  if (value instanceof import_bn3.default) {
    return value;
  }
  if (typeof value === "string") {
    if (value.match(/^-?[0-9]+$/)) {
      return new import_bn3.default(value);
    }
    logger9.logWithError(`invalid BigNumberish string: ${value}`);
  }
  if (typeof value === "number") {
    if (value % 1) {
      logger9.logWithError(`BigNumberish number underflow: ${value}`);
    }
    if (value >= MAX_SAFE || value <= -MAX_SAFE) {
      logger9.logWithError(`BigNumberish number overflow: ${value}`);
    }
    return new import_bn3.default(String(value));
  }
  if (typeof value === "bigint") {
    return new import_bn3.default(value.toString());
  }
  logger9.error(`invalid BigNumberish value: ${value}`);
...
var Fraction = class {
  numerator;
  denominator;
  constructor(numerator, denominator = new import_bn4.default(1)) {
...
var _100_PERCENT = new Fraction(new import_bn5.default(100));
...
function notInnerObject(v) {
  return typeof v === "object" && v !== null && ![Token, TokenAmount, import_web36.PublicKey, Fraction, import_bn6.default, Price, Percent].some((o) => typeof o === "object" && v instanceof o);
}
...
// src/common/bignumber.ts
var BN_ZERO = new import_bn7.default(0);
var BN_ONE = new import_bn7.default(1);
var BN_TWO = new import_bn7.default(2);
var BN_THREE = new import_bn7.default(3);
var BN_FIVE = new import_bn7.default(5);
var BN_TEN = new import_bn7.default(10);
var BN_100 = new import_bn7.default(100);
var BN_1000 = new import_bn7.default(1e3);
var BN_10000 = new import_bn7.default(1e4);
...
var logger4 = createLogger("Raydium_accountInfo_util");
...
var FARM_PROGRAM_ID_V3 = new import_web39.PublicKey("EhhTKczWMGQt46ynNeRX1WfeagwwJd7ufHvCDjRxjo5Q");
var FARM_PROGRAM_ID_V4 = new import_web39.PublicKey("CBuCnLe26faBpcBP2fktp4rp8abpcAnTWft6ZrP5Q4T");
var FARM_PROGRAM_ID_V5 = new import_web39.PublicKey("9KEPoZmtHUrBbhWN1v1KWLMkkvwY6WLtAVUCPRtRjP4z");
var FARM_PROGRAM_ID_V6 = new import_web39.PublicKey("FarmqiPv5eAj3j1GMdMCMUGXqPUvmquZtMy86QH6rzhG");
var UTIL1216 = new import_web39.PublicKey("CLaimxFqjHzgTJtAGHU47NPhg6qrc5sCnpC4tBLyABQS");
var OPEN_BOOK_PROGRAM = new import_web39.PublicKey("srmqPvymJeFKQ4zGQed1GFppgkRHL9kaELCbyksJtPX");
var SERUM_PROGRAM_ID_V3 = new import_web39.PublicKey("9xQeWvG816bUx9EPjHmaT23yvVM2ZWbrrpZb9PusVFin");
var AMM_V4 = new import_web39.PublicKey("675kPX9MHTjS2zt1qfr1NYHuzeLXfQM9H24wFSUt1Mp8");
var AMM_STABLE = new import_web39.PublicKey("5quBtoiQqxF9Jv6KYKctB59NT3gtJD2Y65kdnB1Uev3h");
var LIQUIDITY_POOL_PROGRAM_ID_V5_MODEL = new import_web39.PublicKey("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo");
var CLMM_PROGRAM_ID = new import_web39.PublicKey("CAMMCzo5YL8w4VFF8KVHrK22GGUsp5VTaW7grrKgrWqK");
var CLMM_LOCK_PROGRAM_ID = new import_web39.PublicKey("LockrWmn6K5twhz3y9w1dQERbmgSaRkfnTeTKbpofwE");
var CLMM_LOCK_AUTH_ID = new import_web39.PublicKey("kN1kEznaF5Xbd8LYuqtEFcxzWSBk5Fv6ygX6SqEGJVy");
var Router = new import_web39.PublicKey("routeUGWgWzqBWFcrCfv8tritsqukccJPu3q5GPP3xS");
var FEE_DESTINATION_ID = new import_web39.PublicKey("7YttLkHDoNj9wyDur5pM1ejNaAvT9X4eqaYcHQqtj2G5");
var IDO_PROGRAM_ID_V1 = new import_web39.PublicKey("6FJon3QE27qgPVggARueB22hLvoh22VzJpXv4rBEoSLF");
var IDO_PROGRAM_ID_V2 = new import_web39.PublicKey("CC12se5To1CdEuw7fDS27B7Geo5jJyL7t5UK2B44NgiH");
var IDO_PROGRAM_ID_V3 = new import_web39.PublicKey("9HzJyW1qZsEiSfMUf6L2jo3CcTKAyBmSyKdwQeYisHrC");
var IDO_PROGRAM_ID_V4 = new import_web39.PublicKey("DropEU8AvevN3UrXWXTMuz3rqnMczQVNjq3kcSdW2SQi");
var CREATE_CPMM_POOL_PROGRAM = new import_web39.PublicKey("CPMMoo8L3F4NbTegBCKVNunggL7H1ZpdTHKxQB5qKP1C");
var CREATE_CPMM_POOL_AUTH = new import_web39.PublicKey("GpMZbSM2GgvTKHJirzeGfMFoaZ8UR2X7F4v8vHTvxFbL");
var CREATE_CPMM_POOL_FEE_ACC = new import_web39.PublicKey("DNXgeM9EiiaAbaWvwjHj9fQQLAX5ZsfHyvmYUNRAdNC8");
var DEV_CREATE_CPMM_POOL_PROGRAM = new import_web39.PublicKey("CPMDWBwJDtYax9qW7AyRuVC19Cc4L4Vcy4n2BHAbHkCW");
var DEV_CREATE_CPMM_POOL_AUTH = new import_web39.PublicKey("7rQ1QFNosMkUCuh7Z7fPbTHvh73b68sQYdirycEzJVuw");
var DEV_CREATE_CPMM_POOL_FEE_ACC = new import_web39.PublicKey("G11FKBRaAkHAKuLCgLM6K6NUc9rTjPAznRCjZifrTQe2");
var LOCK_CPMM_PROGRAM = new import_web39.PublicKey("LockrWmn6K5twhz3y9w1dQERbmgSaRkfnTeTKbpofwE");
var DEV_LOCK_CPMM_PROGRAM = new import_web39.PublicKey("DLockwT7X7sxtLmGH9g5kmfcjaBtncdbUmi738m5bvQC");
var LOCK_CPMM_AUTH = new import_web39.PublicKey("3f7GcQFG397GAaEnv51zR6tsTVihYRydnydDD1cXekxH");
var DEV_LOCK_CPMM_AUTH = new import_web39.PublicKey("7AFUeLVRjBfzqK3tTGw8hN48KLQWSk6DTE8xprWdPqix");
var LAUNCHPAD_PROGRAM = new import_web39.PublicKey("LanMV9sAd7wArD4vJFi2qDdfnVhFxYSUg6eADduJ3uj");
var LAUNCHPAD_AUTH = new import_web39.PublicKey("WLHv2UAZm6z4KyaaELi5pjdbJh6RESMva1Rnn8pJVVh");
var DEV_LAUNCHPAD_PROGRAM = new import_web39.PublicKey("LanD8FpTBBvzZFXjTxsAoipkFsxPUCDB4qAqKxYDiNP");
var DEV_LAUNCHPAD_AUTH = new import_web39.PublicKey("HYNHiyKJ3gGVFvyxJAurK7qr7P2o5J9THmvCGMdULtpW");
var DEVNET_PROGRAM_ID = {
  SERUM_MARKET: import_web39.PublicKey.default,
  OPENBOOK_MARKET: new import_web39.PublicKey("EoTcMgcDRTJVZDMZWBoU6rhYHZfkNTVEAfz3uUJRcYGj"),
  UTIL1216: import_web39.PublicKey.default,
  FarmV3: new import_web39.PublicKey("85BFyr98MbCUU9MVTEgzx1nbhWACbJqLzho6zd6DZcWL"),
  FarmV5: new import_web39.PublicKey("EcLzTrNg9V7qhcdyXDe2qjtPkiGzDM2UbdRaeaadU5r2"),
  FarmV6: new import_web39.PublicKey("Farm2hJLcqPtPg8M4rR6DMrsRNc5TPm5Cs4bVQrMe2T7"),
  AmmV4: new import_web39.PublicKey("HWy1jotHpo6UqeQxx49dpYYdQB8wj9Qk9MdxwjLvDHB8"),
  AmmStable: new import_web39.PublicKey("DDg4VmQaJV9ogWce7LpcjBA9bv22wRp5uaTPa5pGjijF"),
  CLMM: new import_web39.PublicKey("devi51mZmdwUJGU9hjN27vEz64Gps7uUefqxg27EAtH"),
  CLMM_LOCK_PROGRAM_ID: new import_web39.PublicKey("DLockwT7X7sxtLmGH9g5kmfcjaBtncdbUmi738m5bvQC"),
  CLMM_LOCK_AUTH_ID: new import_web39.PublicKey("8qmHNvu2Kr2C7U8mJL4Vz1vTDxMhVuXKREwU7TNoaVEo"),
  Router: new import_web39.PublicKey("BVChZ3XFEwTMUk1o9i3HAf91H6mFxSwa5X2wFAWhYPhU"),
  ...
  FEE_DESTINATION_ID: new import_web39.PublicKey("3XMrhbv989VxAMi3DErLV9eJht1pHppW5LbKxe9fkEFR"),
...
var FEE_RATE_DENOMINATOR_VALUE = new import_bn9.default(1e6);
...
function splitNumber(num, decimals) {
  ...
  if (num.includes(".")) {
    ...
    if (splited.length === 2) {
      ...
      logger5.logWithError(`invalid number string, num: ${num}`);
    }
  } else {
    integral = num;
  }
...
var TokenAmount = class extends Fraction {
  ...
  constructor(token, amount, isRaw = true, name) {
    let parsedAmount = new import_bn10.default(0);
    const multiplier = BN_TEN.pow(new import_bn10.default(token.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new import_bn10.default(0);
      let fractionalAmount = new import_bn10.default(0);
...
var MODEL_DATA_PUBKEY = new import_web313.PublicKey("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo");
...
// src/raydium/liquidity/instruction.ts
var logger6 = createLogger("Raydium_liquidity_instruction");
function makeAddLiquidityInstruction(params) {
  ...
  const keys = [
    accountMeta({ pubkey: import_spl_token7.TOKEN_PROGRAM_ID, isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolInfo.id) }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.targetOrders) }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolInfo.lpMint.address) }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.vault.A) }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.vault.B) })
  ];
  if (poolInfo.pooltype.includes("StablePool")) {
    keys.push(accountMeta({ pubkey: MODEL_DATA_PUBKEY }));
  }
  keys.push(accountMeta({ pubkey: new import_web314.PublicKey(poolInfo.marketId), isWritable: false }), accountMeta({ pubkey: userKeys.baseTokenAccount }), accountMeta({ pubkey: userKeys.quoteTokenAccount }), accountMeta({ pubkey: userKeys.lpTokenAccount }), accountMeta({ pubkey: userKeys.owner, isWritable: false, isSigner: true }), accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.marketEventQueue), isWritable: false }));
  return new import_web314.TransactionInstruction({
    programId: new import_web314.PublicKey(poolInfo.programId),
    keys,
    data
  });
}
function removeLiquidityInstruction(params) {
  ...
  return new import_web314.TransactionInstruction({ programId: poolKeys.programId, keys: [] });
}
function createPoolV4InstructionV2({
  ...
  const keys = [
    ...
    { pubkey: import_spl_token7.ASSOCIATED_TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
  ...
  return {
    instruction: new import_web314.TransactionInstruction({
      keys,
      programId,
      data
    }),
...
function simulatePoolInfoInstruction(poolKeys) {
  ...
  const keys = [
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.id), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.vault.A), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.vault.B), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.mintLp.address), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.marketId), isWritable: false }),
    accountMeta({ pubkey: new import_web314.PublicKey(poolKeys.marketEventQueue), isWritable: false })
  ];
  return new import_web314.TransactionInstruction({
    programId: new import_web314.PublicKey(poolKeys.programId),
    keys,
    data
  });
}
...
function makeAMMSwapInstruction(params) {
  ...
  if (version === 4 || version === 5) {
    ...
    logger6.logWithError("invalid params", "params", params);
  }
  logger6.logWithError("invalid version", "poolKeys.version", version);
...
function makeInitPoolInstructionV4({
  ...
  const keys = [
    accountMeta({ pubkey: import_spl_token7.TOKEN_PROGRAM_ID, isWritable: false }),
    accountMeta({ pubkey: import_web314.SystemProgram.programId, isWritable: false }),
    accountMeta({ pubkey: import_web314.SYSVAR_RENT_PUBKEY, isWritable: false }),
    accountMeta({ pubkey: poolKeys.id }),
    accountMeta({ pubkey: poolKeys.authority, isWritable: false }),
    accountMeta({ pubkey: poolKeys.openOrders }),
    accountMeta({ pubkey: poolKeys.mintLp.address }),
...
var logger7 = createLogger("Raydium_liquidity_serum");
function getSerumAssociatedAuthority({ programId, marketId }) {
  ...
  while (nonce < 100) {
    try {
      ...
      publicKey2 = import_web315.PublicKey.createProgramAddressSync(seedsWithNonce, programId);
    } catch (err) {
  ...
  logger7.logWithError("unable to find a viable program address nonce", "params", {
    programId,
    marketId
  });
...
var logger8 = createLogger("Raydium_Util");
  ...
  return await Promise.all(poolKeysList.map(async (itemPoolKey) => {
    if (itemPoolKey.modelDataAccount) {
      ...
      const poolsInfo = logs.map((log3) => {
        ...
        const status = new import_bn13.default(parseSimulateValue(json, "status"));
        ...
        const baseReserve = new import_bn13.default(parseSimulateValue(json, "pool_coin_amount"));
        const quoteReserve = new import_bn13.default(parseSimulateValue(json, "pool_pc_amount"));
        const lpSupply = new import_bn13.default(parseSimulateValue(json, "pool_lp_supply"));
        let startTime = "0";
        try {
        ...
        return {
          ...
          quoteReserve,
          lpSupply,
          startTime: new import_bn13.default(startTime)
        };
      })[0];
      return poolsInfo;
    } else {
      const [poolAcc, vaultAccA, vaultAccB, mintAccLp] = await connection.getMultipleAccountsInfo([
        new import_web318.PublicKey(itemPoolKey.id),
        new import_web318.PublicKey(itemPoolKey.vault.A),
        new import_web318.PublicKey(itemPoolKey.vault.B),
        new import_web318.PublicKey(itemPoolKey.mintLp.address)
      ]);
      if (poolAcc === null)
...
var toAmmComputePoolInfo = (poolData) => {
  const data = {};
  const tokenProgramStr = import_spl_token10.TOKEN_PROGRAM_ID.toBase58();
