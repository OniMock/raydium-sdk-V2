...
var MODEL_DATA_PUBKEY = new import_web312.PublicKey("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo");
...
function makeAddLiquidityInstruction(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: new import_web313.PublicKey(poolInfo.id) }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.targetOrders) }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolInfo.lpMint.address) }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.vault.A) }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.vault.B) })
  ];
  if (poolInfo.pooltype.includes("StablePool")) {
    keys.push(accountMeta({ pubkey: MODEL_DATA_PUBKEY }));
  }
  keys.push(accountMeta({ pubkey: new import_web313.PublicKey(poolInfo.marketId), isWritable: false }), accountMeta({ pubkey: userKeys.baseTokenAccount }), accountMeta({ pubkey: userKeys.quoteTokenAccount }), accountMeta({ pubkey: userKeys.lpTokenAccount }), accountMeta({ pubkey: userKeys.owner, isWritable: false, isSigner: true }), accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.marketEventQueue), isWritable: false }));
  return new import_web313.TransactionInstruction({
    programId: new import_web313.PublicKey(poolInfo.programId),
    keys,
    data
  });
}
function removeLiquidityInstruction(params) {
  ...
  return new import_web313.TransactionInstruction({ programId: poolKeys.programId, keys: [] });
}
...
function simulatePoolInfoInstruction(poolKeys) {
  ...
  const keys = [
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.id), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.vault.A), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.vault.B), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.mintLp.address), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.marketId), isWritable: false }),
    accountMeta({ pubkey: new import_web313.PublicKey(poolKeys.marketEventQueue), isWritable: false })
  ];
  return new import_web313.TransactionInstruction({
    programId: new import_web313.PublicKey(poolKeys.programId),
    keys,
    data
  });
}
