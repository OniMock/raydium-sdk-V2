...
import { PublicKey as PublicKey13, SystemProgram as SystemProgram3, SYSVAR_RENT_PUBKEY as SYSVAR_RENT_PUBKEY2, TransactionInstruction as TransactionInstruction3 } from "@solana/web3.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID6 } from "@solana/spl-token";
...
var MODEL_DATA_PUBKEY = new PublicKey12("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo");
...
function makeAddLiquidityInstruction(params) {
  ...
  const keys = [
    ...
    accountMeta({ pubkey: new PublicKey13(poolInfo.id) }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.targetOrders) }),
    accountMeta({ pubkey: new PublicKey13(poolInfo.lpMint.address) }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.vault.A) }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.vault.B) })
  ];
  if (poolInfo.pooltype.includes("StablePool")) {
    keys.push(accountMeta({ pubkey: MODEL_DATA_PUBKEY }));
  }
  keys.push(accountMeta({ pubkey: new PublicKey13(poolInfo.marketId), isWritable: false }), accountMeta({ pubkey: userKeys.baseTokenAccount }), accountMeta({ pubkey: userKeys.quoteTokenAccount }), accountMeta({ pubkey: userKeys.lpTokenAccount }), accountMeta({ pubkey: userKeys.owner, isWritable: false, isSigner: true }), accountMeta({ pubkey: new PublicKey13(poolKeys.marketEventQueue), isWritable: false }));
  return new TransactionInstruction3({
    programId: new PublicKey13(poolInfo.programId),
    keys,
    data
  });
}
...
function simulatePoolInfoInstruction(poolKeys) {
  ...
  const keys = [
    accountMeta({ pubkey: new PublicKey13(poolKeys.id), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.authority), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.openOrders), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.vault.A), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.vault.B), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.mintLp.address), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.marketId), isWritable: false }),
    accountMeta({ pubkey: new PublicKey13(poolKeys.marketEventQueue), isWritable: false })
  ];
  return new TransactionInstruction3({
    programId: new PublicKey13(poolKeys.programId),
    keys,
    data
  });
}
