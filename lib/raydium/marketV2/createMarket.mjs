...
import { createInitializeAccountInstruction, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID7 } from "@solana/spl-token";
...
function initializeMarket({
  ...
  const keys = [
    ...
    {
      pubkey: marketInfo.authority ? marketInfo.quoteMint : SYSVAR_RENT_PUBKEY2,
      isSigner: false,
      isWritable: false
    }
...
async function makeCreateMarketInstruction({
  ...
  tx1.add(SystemProgram3.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
    ...
    space: 165,
    programId: TOKEN_PROGRAM_ID7
  }), createInitializeAccountInstruction(marketInfo.baseVault.publicKey, marketInfo.baseMint, marketInfo.vaultOwner), createInitializeAccountInstruction(marketInfo.quoteVault.publicKey, marketInfo.quoteMint, marketInfo.vaultOwner), SystemProgram3.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
  ...
  tx2.add(SystemProgram3.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
...
var MarketV2 = class extends ModuleBase {
  async create({
    ...
    const feeRateBps = 0;
    const quoteDustThreshold = new BN11(100);
    function getVaultOwnerAndNonce() {
      const vaultSignerNonce2 = new BN11(0);
      while (true) {
        try {
          const vaultOwner2 = PublicKey14.createProgramAddressSync([market.publicKey.toBuffer(), vaultSignerNonce2.toArrayLike(Buffer, "le", 8)], dexProgramId);
          return { vaultOwner: vaultOwner2, vaultSignerNonce: vaultSignerNonce2 };
        } catch (e) {
          vaultSignerNonce2.iaddn(1);
          if (vaultSignerNonce2.gt(new BN11(25555)))
            throw Error("find vault owner error");
        }
      }
    }
    const { vaultOwner, vaultSignerNonce } = getVaultOwnerAndNonce();
    const baseLotSize = new BN11(Math.round(10 ** baseInfo.decimals * lotSize));
    const quoteLotSize = new BN11(Math.round(lotSize * 10 ** quoteInfo.decimals * tickSize));
    ...
    if (txVersion === 0 /* V0 */)
      return txBuilder.sizeCheckBuildV0({
        ...
        address: {
          ...
          baseMint: new PublicKey14(baseInfo.mint),
          quoteMint: new PublicKey14(quoteInfo.mint)
        }
      });
