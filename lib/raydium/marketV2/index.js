...
// src/raydium/marketV2/index.ts
var marketV2_exports = {};
__export(marketV2_exports, {
...
module.exports = __toCommonJS(marketV2_exports);
...
// src/raydium/account/util.ts
var logger6 = createLogger("Raydium_Util");
...
function initializeMarket({
  ...
  const keys = [
    ...
    {
      pubkey: marketInfo.authority ? marketInfo.quoteMint : import_web313.SYSVAR_RENT_PUBKEY,
      isSigner: false,
      isWritable: false
    }
...
async function makeCreateMarketInstruction({
  ...
  const tx1 = new import_web313.Transaction();
  const accountLamports = await connection.getMinimumBalanceForRentExemption(165);
  tx1.add(import_web313.SystemProgram.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
    ...
    space: 165,
    programId: import_spl_token7.TOKEN_PROGRAM_ID
  }), (0, import_spl_token7.createInitializeAccountInstruction)(marketInfo.baseVault.publicKey, marketInfo.baseMint, marketInfo.vaultOwner), (0, import_spl_token7.createInitializeAccountInstruction)(marketInfo.quoteVault.publicKey, marketInfo.quoteMint, marketInfo.vaultOwner), import_web313.SystemProgram.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
  ...
  const tx2 = new import_web313.Transaction();
  tx2.add(import_web313.SystemProgram.createAccountWithSeed({
    fromPubkey: wallet,
    basePubkey: wallet,
...
var MarketV2 = class extends ModuleBase {
  async create({
    ...
    const feeRateBps = 0;
    const quoteDustThreshold = new import_bn11.default(100);
    function getVaultOwnerAndNonce() {
      const vaultSignerNonce2 = new import_bn11.default(0);
      while (true) {
        try {
          const vaultOwner2 = import_web314.PublicKey.createProgramAddressSync([market.publicKey.toBuffer(), vaultSignerNonce2.toArrayLike(Buffer, "le", 8)], dexProgramId);
          return { vaultOwner: vaultOwner2, vaultSignerNonce: vaultSignerNonce2 };
        } catch (e) {
          vaultSignerNonce2.iaddn(1);
          if (vaultSignerNonce2.gt(new import_bn11.default(25555)))
            throw Error("find vault owner error");
        }
      }
    }
    const { vaultOwner, vaultSignerNonce } = getVaultOwnerAndNonce();
    const baseLotSize = new import_bn11.default(Math.round(10 ** baseInfo.decimals * lotSize));
    const quoteLotSize = new import_bn11.default(Math.round(lotSize * 10 ** quoteInfo.decimals * tickSize));
    ...
    if (txVersion === 0 /* V0 */)
      return txBuilder.sizeCheckBuildV0({
        ...
        address: {
          ...
          baseMint: new import_web314.PublicKey(baseInfo.mint),
          quoteMint: new import_web314.PublicKey(quoteInfo.mint)
        }
      });
