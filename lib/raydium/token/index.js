...
var TokenAmount = class extends Fraction {
  ...
  constructor(token, amount, isRaw = true, name) {
    let parsedAmount = new import_bn9.default(0);
    const multiplier = BN_TEN.pow(new import_bn9.default(token.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new import_bn9.default(0);
      let fractionalAmount = new import_bn9.default(0);
...
var parseTokenInfo = async ({
  ...
  const accountData = await connection.getAccountInfo(new import_web312.PublicKey(mint));
  if (!accountData || accountData.data.length !== import_spl_token6.MintLayout.span)
    return;
  const tokenInfo = import_spl_token6.MintLayout.decode(accountData.data);
  return tokenInfo;
};
var toTokenInfo = ({
  mint,
