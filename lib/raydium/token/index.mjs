...
import { MINT_SIZE, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID3, getTransferFeeConfig, unpackMint } from "@solana/spl-token";
...
var TokenAmount = class extends Fraction {
  ...
  constructor(token, amount, isRaw = true, name) {
    let parsedAmount = new BN9(0);
    const multiplier = BN_TEN.pow(new BN9(token.decimals));
    if (isRaw) {
      parsedAmount = parseBigNumberish(amount);
    } else {
      let integralAmount = new BN9(0);
      let fractionalAmount = new BN9(0);
...
var parseTokenInfo = async ({
  ...
  const accountData = await connection.getAccountInfo(new PublicKey12(mint));
