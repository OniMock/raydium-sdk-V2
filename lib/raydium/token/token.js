...
var TokenModule = class extends ModuleBase {
  ...
  async load(params) {
    ...
    mintList.forEach((token) => {
      ...
      this._tokenMap.set(token.address, {
        ...
        type: "raydium",
        priority: 2,
        programId: token.programId ?? (token.tags.includes("token-2022") ? import_spl_token6.TOKEN_2022_PROGRAM_ID.toBase58() : import_spl_token6.TOKEN_PROGRAM_ID.toBase58())
      });
  ...
  async getTokenInfo(mint) {
    ...
    const onlineInfo = await this.scope.connection.getAccountInfo(new import_web311.PublicKey(mintStr));
    if (!onlineInfo)
      throw new Error(`mint address not found: ${mintStr}`);
    const data = import_spl_token6.MintLayout.decode(onlineInfo.data);
