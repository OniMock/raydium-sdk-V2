...
import { MintLayout, TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID6 } from "@solana/spl-token";
...
var TokenModule = class extends ModuleBase {
  ...
  async load(params) {
    ...
    mintList.forEach((token) => {
      ...
      this._tokenMap.set(token.address, {
        ...
        type: "raydium",
        priority: 2,
        programId: token.programId ?? (token.tags.includes("token-2022") ? TOKEN_2022_PROGRAM_ID.toBase58() : TOKEN_PROGRAM_ID6.toBase58())
      });
  ...
  async getTokenInfo(mint) {
    ...
    const onlineInfo = await this.scope.connection.getAccountInfo(new PublicKey11(mintStr));
    if (!onlineInfo)
