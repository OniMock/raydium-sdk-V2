...
var MODEL_DATA_PUBKEY = new import_web316.PublicKey("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo");
...
// src/raydium/liquidity/instruction.ts
var logger7 = createLogger("Raydium_liquidity_instruction");
...
function makeAMMSwapInstruction(params) {
  ...
  if (version === 4 || version === 5) {
    ...
    logger7.logWithError("invalid params", "params", params);
  }
  logger7.logWithError("invalid version", "poolKeys.version", version);
...
// src/raydium/cpmm/instruction.ts
var logger8 = createLogger("Raydium_cpmm");
var anchorDataBuf2 = {
...
function route1Instruction(programId, poolInfoA, poolKeyA, poolKeyB, userSourceToken, userRouteToken, userPdaAccount, ownerWallet, inputMint, amountIn, amountOut, tickArrayA) {
  ...
  const keys = [
    ...
    { pubkey: import_spl_token12.TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
    { pubkey: new import_web319.PublicKey(poolKeyA.programId), isSigner: false, isWritable: false },
    { pubkey: new import_web319.PublicKey(poolKeyA.id), isSigner: false, isWritable: true },
    { pubkey: new import_web319.PublicKey(poolKeyB.id), isSigner: false, isWritable: true },
  ...
  if (poolInfoA.type === "Concentrated") {
    ...
    keys.push(...[
      ...
      { pubkey: poolKey.id, isSigner: false, isWritable: true },
      { pubkey: new import_web319.PublicKey("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo"), isSigner: false, isWritable: false },
...
function route2Instruction(programId, poolInfoB, poolKeyA, poolKeyB, userRouteToken, userDestinationToken, userPdaAccount, ownerWallet, routeMint, tickArrayB) {
  ...
  const keys = [
    { pubkey: import_web319.SystemProgram.programId, isSigner: false, isWritable: false },
    { pubkey: import_spl_token12.TOKEN_PROGRAM_ID, isSigner: false, isWritable: false },
    { pubkey: new import_web319.PublicKey(String(poolKeyB.programId)), isSigner: false, isWritable: false },
    { pubkey: new import_web319.PublicKey(String(poolKeyB.id)), isSigner: false, isWritable: true },
    { pubkey: new import_web319.PublicKey(String(poolKeyA.id)), isSigner: false, isWritable: true },
    { pubkey: userRouteToken, isSigner: false, isWritable: true },
...
function routeInstruction(programId, wallet, userSourceToken, userRouteToken, userDestinationToken, inputMint, routeMint, outputMint, poolInfoA, poolInfoB, poolKeyA, poolKeyB, amountIn, amountOut, remainingAccounts) {
  ...
  const keys = [
    accountMeta({ pubkey: import_spl_token12.TOKEN_PROGRAM_ID, isWritable: false }),
    accountMeta({ pubkey: import_spl_token12.TOKEN_2022_PROGRAM_ID, isWritable: false }),
    accountMeta({ pubkey: import_spl_token12.ASSOCIATED_TOKEN_PROGRAM_ID, isWritable: false }),
    accountMeta({ pubkey: import_web319.SystemProgram.programId, isWritable: false }),
  ...
  for (let index = 0; index < poolInfos.length; index++) {
    ...
    keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolInfo.programId), isWritable: false }));
    if (index === poolInfos.length - 1) {
      keys.push(accountMeta({ pubkey: userDestinationToken }));
    } else {
      keys.push(accountMeta({ pubkey: userRouteToken }));
    }
    keys.push(accountMeta({ pubkey: new import_web319.PublicKey(routeMints[index]) }));
    keys.push(accountMeta({ pubkey: new import_web319.PublicKey(routeMints[index + 1]) }));
    if (_poolInfo.version === 6) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? _poolKey.vault.A : _poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? _poolKey.vault.B : _poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolInfo.observationId) }));
      keys.push(accountMeta({ pubkey: MEMO_PROGRAM_ID2 }));
      keys.push(accountMeta({
        pubkey: getPdaExBitmapAccount(new import_web319.PublicKey(_poolInfo.programId), new import_web319.PublicKey(_poolInfo.id)).publicKey
      }));
      ...
      for (const item of remainingAccounts[index] ?? []) {
        keys.push(accountMeta({ pubkey: new import_web319.PublicKey(item) }));
      }
    } else if (_poolInfo.version === 5) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketProgramId) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketAuthority) }));
      keys.push(accountMeta({ pubkey: LIQUIDITY_POOL_PROGRAM_ID_V5_MODEL, isWritable: false }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.marketQuoteVault) }));
    } else if (_poolInfo.version === 4) {
      const _poolKey = poolKeys[index];
      const isSupportIdOnly = _poolInfo.status !== 1;
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketProgramId) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketAuthority) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(isSupportIdOnly ? _poolKey.id : _poolKey.marketQuoteVault) }));
    } else if (_poolInfo.version === 7) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.authority) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? _poolKey.vault.A : _poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? _poolKey.vault.B : _poolKey.vault.A) }));
...
function clmmPriceLimitX64InsData(x64Price, inputIsA) {
  if (x64Price) {
    if (inputIsA) {
      const _m = new import_bn19.default(x64Price).div(new import_bn19.default(25));
      return _m.gt(MIN_SQRT_PRICE_X64_ADD_ONE) ? _m : MIN_SQRT_PRICE_X64_ADD_ONE;
    } else {
      const _m = new import_bn19.default(x64Price).mul(new import_bn19.default(25));
  ...
  if (swapInfo.routeType === "amm") {
    if (swapInfo.poolInfo[0].version === 6) {
      ...
      return ClmmInstrument.makeSwapBaseInInstructions({
        ...
        amountOutMin: swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new import_bn19.default(0)),
        sqrtPriceLimitX64,
      ...
      return {
        ...
        instructions: [
          makeAMMSwapInstruction({
            ...
            amountIn: swapInfo.amountIn.amount.raw,
            amountOut: swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new import_bn19.default(0)),
            fixedSide: "in"
          })
        ],
    ...
    return {
      signers: [],
      instructions: [
        routeInstruction(routeProgram, ownerInfo.wallet, ownerInfo.sourceToken, ownerInfo.routeToken, ownerInfo.destinationToken, inputMint.toString(), swapInfo.middleToken.mint.toString(), swapInfo.outputMint.toString(), poolInfo1, poolInfo2, poolKey1, poolKey2, swapInfo.amountIn.amount.raw, swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new import_bn19.default(0)), swapInfo.remainingAccounts)
      ],
...
function swapBaseInAutoAccount({
  ...
  for (let index = 0; index < poolKeys.length; index++) {
    ...
    keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.programId), isWritable: false }));
    if (index === poolKeys.length - 1) {
      ...
      if (cacheAccount[mint] === void 0) {
        const ata = getATAAddress(wallet, new import_web319.PublicKey(mint), _poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58() || _poolKey.programId === ALL_PROGRAM_ID.CREATE_CPMM_POOL_PROGRAM.toBase58() ? new import_web319.PublicKey(inputIsA ? _poolKey.mintB.programId : _poolKey.mintA.programId) : import_spl_token12.TOKEN_PROGRAM_ID).publicKey;
        cacheAccount[mint] = ata;
      }
      keys.push(accountMeta({ pubkey: cacheAccount[mint] }));
    }
    keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_routeInfo.inputMint) }));
    keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_routeInfo.outputMint) }));
    if (_poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58()) {
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? poolKey.vault.A : poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? poolKey.vault.B : poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.observationId) }));
      keys.push(accountMeta({ pubkey: MEMO_PROGRAM_ID2, isWritable: false }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.exBitmapAccount) }));
      ...
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketProgramId), isWritable: false }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketAuthority), isWritable: false }));
      keys.push(accountMeta({ pubkey: LIQUIDITY_POOL_PROGRAM_ID_V5_MODEL, isWritable: false }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.marketQuoteVault) }));
      ...
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.authority) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? poolKey.vault.A : poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new import_web319.PublicKey(inputIsA ? poolKey.vault.B : poolKey.vault.A) }));
  ...
  dataLayout.encode({
    insId: 0,
    amountIn: amount,
    amountOut: new import_bn19.default(routeInfo.data.otherAmountThreshold),
    clmmPriceLimit
  }, data);
  return new import_web319.TransactionInstruction({
    keys,
...
function swapBaseOutAutoAccount({
  ...
  for (let index = poolKeys.length - 1; index >= 0; index--) {
    ...
    const _poolKey = poolKeys[index];
    const inputIsA = _routeInfo.inputMint === _poolKey.mintA.address;
    keys.push(accountMeta({ pubkey: new import_web319.PublicKey(_poolKey.programId) }));
    if (index === 0) {
      keys.push(accountMeta({ pubkey: cacheAccount[_routeInfo.inputMint] }));
    } else {
      const mint = _routeInfo.inputMint;
      if (cacheAccount[mint] === void 0) {
        const ata = getATAAddress(wallet, new import_web319.PublicKey(mint), _poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58() || _poolKey.programId === ALL_PROGRAM_ID.CREATE_CPMM_POOL_PROGRAM.toBase58() ? new import_web319.PublicKey(inputIsA ? _poolKey.mintA.programId : _poolKey.mintB.programId) : import_spl_token12.TOKEN_PROGRAM_ID).publicKey;
        cacheAccount[mint] = ata;
      }
      keys.push(accountMeta({ pubkey: cacheAccount[mint] }));
    }
  ...
  dataLayout.encode({
    insId: 1,
    amountIn: new import_bn19.default(routeInfo.data.otherAmountThreshold),
    amountOut: new import_bn19.default(routeInfo.data.outputAmount),
    clmmPriceLimit
  }, data);
  return new import_web319.TransactionInstruction({
    keys,
