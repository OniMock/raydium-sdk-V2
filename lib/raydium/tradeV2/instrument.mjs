...
import { PublicKey as PublicKey19, SystemProgram as SystemProgram6, TransactionInstruction as TransactionInstruction6 } from "@solana/web3.js";
import BN19 from "bn.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID4, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID5, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID11 } from "@solana/spl-token";
...
import { PublicKey as PublicKey17, SystemProgram as SystemProgram4, SYSVAR_RENT_PUBKEY as SYSVAR_RENT_PUBKEY2, TransactionInstruction as TransactionInstruction4 } from "@solana/web3.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID2, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID9 } from "@solana/spl-token";
...
var MODEL_DATA_PUBKEY = new PublicKey16("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo");
...
import { PublicKey as PublicKey18, TransactionInstruction as TransactionInstruction5, Keypair as Keypair3, SystemProgram as SystemProgram5 } from "@solana/web3.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID as ASSOCIATED_TOKEN_PROGRAM_ID3, TOKEN_2022_PROGRAM_ID as TOKEN_2022_PROGRAM_ID4, TOKEN_PROGRAM_ID as TOKEN_PROGRAM_ID10 } from "@solana/spl-token";
...
function route1Instruction(programId, poolInfoA, poolKeyA, poolKeyB, userSourceToken, userRouteToken, userPdaAccount, ownerWallet, inputMint, amountIn, amountOut, tickArrayA) {
  ...
  const keys = [
    ...
    { pubkey: new PublicKey19(poolKeyA.programId), isSigner: false, isWritable: false },
    { pubkey: new PublicKey19(poolKeyA.id), isSigner: false, isWritable: true },
    { pubkey: new PublicKey19(poolKeyB.id), isSigner: false, isWritable: true },
  ...
  if (poolInfoA.type === "Concentrated") {
    ...
    keys.push(...[
      ...
      { pubkey: poolKey.id, isSigner: false, isWritable: true },
      { pubkey: new PublicKey19("CDSr3ssLcRB6XYPJwAfFt18MZvEZp4LjHcvzBVZ45duo"), isSigner: false, isWritable: false },
...
function route2Instruction(programId, poolInfoB, poolKeyA, poolKeyB, userRouteToken, userDestinationToken, userPdaAccount, ownerWallet, routeMint, tickArrayB) {
  ...
  const keys = [
    { pubkey: SystemProgram6.programId, isSigner: false, isWritable: false },
    { pubkey: TOKEN_PROGRAM_ID11, isSigner: false, isWritable: false },
    { pubkey: new PublicKey19(String(poolKeyB.programId)), isSigner: false, isWritable: false },
    { pubkey: new PublicKey19(String(poolKeyB.id)), isSigner: false, isWritable: true },
    { pubkey: new PublicKey19(String(poolKeyA.id)), isSigner: false, isWritable: true },
    { pubkey: userRouteToken, isSigner: false, isWritable: true },
...
function routeInstruction(programId, wallet, userSourceToken, userRouteToken, userDestinationToken, inputMint, routeMint, outputMint, poolInfoA, poolInfoB, poolKeyA, poolKeyB, amountIn, amountOut, remainingAccounts) {
  ...
  const keys = [
    accountMeta({ pubkey: TOKEN_PROGRAM_ID11, isWritable: false }),
    ...
    accountMeta({ pubkey: SystemProgram6.programId, isWritable: false }),
  ...
  for (let index = 0; index < poolInfos.length; index++) {
    ...
    keys.push(accountMeta({ pubkey: new PublicKey19(_poolInfo.programId), isWritable: false }));
    if (index === poolInfos.length - 1) {
      keys.push(accountMeta({ pubkey: userDestinationToken }));
    } else {
      keys.push(accountMeta({ pubkey: userRouteToken }));
    }
    keys.push(accountMeta({ pubkey: new PublicKey19(routeMints[index]) }));
    keys.push(accountMeta({ pubkey: new PublicKey19(routeMints[index + 1]) }));
    if (_poolInfo.version === 6) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? _poolKey.vault.A : _poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? _poolKey.vault.B : _poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolInfo.observationId) }));
      keys.push(accountMeta({ pubkey: MEMO_PROGRAM_ID2 }));
      keys.push(accountMeta({
        pubkey: getPdaExBitmapAccount(new PublicKey19(_poolInfo.programId), new PublicKey19(_poolInfo.id)).publicKey
      }));
      ...
      for (const item of remainingAccounts[index] ?? []) {
        keys.push(accountMeta({ pubkey: new PublicKey19(item) }));
      }
    } else if (_poolInfo.version === 5) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketProgramId) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketAuthority) }));
      keys.push(accountMeta({ pubkey: LIQUIDITY_POOL_PROGRAM_ID_V5_MODEL, isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.marketQuoteVault) }));
    } else if (_poolInfo.version === 4) {
      const _poolKey = poolKeys[index];
      const isSupportIdOnly = _poolInfo.status !== 1;
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketProgramId) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketAuthority) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(isSupportIdOnly ? _poolKey.id : _poolKey.marketQuoteVault) }));
    } else if (_poolInfo.version === 7) {
      const _poolKey = poolKeys[index];
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.authority) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? _poolKey.vault.A : _poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? _poolKey.vault.B : _poolKey.vault.A) }));
...
function clmmPriceLimitX64InsData(x64Price, inputIsA) {
  if (x64Price) {
    if (inputIsA) {
      const _m = new BN19(x64Price).div(new BN19(25));
      return _m.gt(MIN_SQRT_PRICE_X64_ADD_ONE) ? _m : MIN_SQRT_PRICE_X64_ADD_ONE;
    } else {
      const _m = new BN19(x64Price).mul(new BN19(25));
  ...
  if (swapInfo.routeType === "amm") {
    if (swapInfo.poolInfo[0].version === 6) {
      ...
      return ClmmInstrument.makeSwapBaseInInstructions({
        ...
        amountOutMin: swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new BN19(0)),
        sqrtPriceLimitX64,
      ...
      return {
        ...
        instructions: [
          makeAMMSwapInstruction({
            ...
            amountIn: swapInfo.amountIn.amount.raw,
            amountOut: swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new BN19(0)),
            fixedSide: "in"
          })
        ],
    ...
    return {
      signers: [],
      instructions: [
        routeInstruction(routeProgram, ownerInfo.wallet, ownerInfo.sourceToken, ownerInfo.routeToken, ownerInfo.destinationToken, inputMint.toString(), swapInfo.middleToken.mint.toString(), swapInfo.outputMint.toString(), poolInfo1, poolInfo2, poolKey1, poolKey2, swapInfo.amountIn.amount.raw, swapInfo.minAmountOut.amount.raw.sub(swapInfo.minAmountOut.fee?.raw ?? new BN19(0)), swapInfo.remainingAccounts)
      ],
...
function swapBaseInAutoAccount({
  ...
  for (let index = 0; index < poolKeys.length; index++) {
    ...
    keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.programId), isWritable: false }));
    if (index === poolKeys.length - 1) {
      ...
      if (cacheAccount[mint] === void 0) {
        const ata = getATAAddress(wallet, new PublicKey19(mint), _poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58() || _poolKey.programId === ALL_PROGRAM_ID.CREATE_CPMM_POOL_PROGRAM.toBase58() ? new PublicKey19(inputIsA ? _poolKey.mintB.programId : _poolKey.mintA.programId) : TOKEN_PROGRAM_ID11).publicKey;
        cacheAccount[mint] = ata;
      }
      keys.push(accountMeta({ pubkey: cacheAccount[mint] }));
    }
    keys.push(accountMeta({ pubkey: new PublicKey19(_routeInfo.inputMint) }));
    keys.push(accountMeta({ pubkey: new PublicKey19(_routeInfo.outputMint) }));
    if (_poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58()) {
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? poolKey.vault.A : poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? poolKey.vault.B : poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.observationId) }));
      keys.push(accountMeta({ pubkey: MEMO_PROGRAM_ID2, isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.exBitmapAccount) }));
      ...
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.authority), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketProgramId), isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketAuthority), isWritable: false }));
      keys.push(accountMeta({ pubkey: LIQUIDITY_POOL_PROGRAM_ID_V5_MODEL, isWritable: false }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.openOrders) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.vault.A) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketId) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketBids) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketAsks) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketEventQueue) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketBaseVault) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.marketQuoteVault) }));
      ...
      const poolKey = _poolKey;
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.authority) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.config.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(poolKey.id) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? poolKey.vault.A : poolKey.vault.B) }));
      keys.push(accountMeta({ pubkey: new PublicKey19(inputIsA ? poolKey.vault.B : poolKey.vault.A) }));
  ...
  dataLayout.encode({
    insId: 0,
    amountIn: amount,
    amountOut: new BN19(routeInfo.data.otherAmountThreshold),
    clmmPriceLimit
  }, data);
  return new TransactionInstruction6({
    keys,
...
function swapBaseOutAutoAccount({
  ...
  for (let index = poolKeys.length - 1; index >= 0; index--) {
    ...
    const _poolKey = poolKeys[index];
    const inputIsA = _routeInfo.inputMint === _poolKey.mintA.address;
    keys.push(accountMeta({ pubkey: new PublicKey19(_poolKey.programId) }));
    if (index === 0) {
      keys.push(accountMeta({ pubkey: cacheAccount[_routeInfo.inputMint] }));
    } else {
      const mint = _routeInfo.inputMint;
      if (cacheAccount[mint] === void 0) {
        const ata = getATAAddress(wallet, new PublicKey19(mint), _poolKey.programId === ALL_PROGRAM_ID.CLMM_PROGRAM_ID.toBase58() || _poolKey.programId === ALL_PROGRAM_ID.CREATE_CPMM_POOL_PROGRAM.toBase58() ? new PublicKey19(inputIsA ? _poolKey.mintA.programId : _poolKey.mintB.programId) : TOKEN_PROGRAM_ID11).publicKey;
        cacheAccount[mint] = ata;
      }
      keys.push(accountMeta({ pubkey: cacheAccount[mint] }));
    }
  ...
  dataLayout.encode({
    insId: 1,
    amountIn: new BN19(routeInfo.data.otherAmountThreshold),
    amountOut: new BN19(routeInfo.data.outputAmount),
    clmmPriceLimit
  }, data);
  return new TransactionInstruction6({
    keys,
