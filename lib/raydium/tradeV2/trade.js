...
function generatePubKey({
  fromPublicKey,
  programId = import_spl_token12.TOKEN_PROGRAM_ID,
  assignSeed
}) {
...
function initTokenAccountInstruction(params) {
  const { mint, tokenAccount, owner, programId = import_spl_token13.TOKEN_PROGRAM_ID } = params;
  return (0, import_spl_token13.createInitializeAccountInstruction)(tokenAccount, mint, owner, programId);
}
function closeAccountInstruction(params) {
  const { tokenAccount, payer, multiSigners = [], owner, programId = import_spl_token13.TOKEN_PROGRAM_ID } = params;
  return (0, import_spl_token13.createCloseAccountInstruction)(tokenAccount, payer, owner, multiSigners, programId);
}
async function createWSolAccountInstructions(params) {
  ...
  const lamports = parseBigNumberish(amount).add(new import_bn21.default(balanceNeeded));
  const newAccount = generatePubKey({ fromPublicKey: payer, programId: import_spl_token13.TOKEN_PROGRAM_ID });
  return {
    addresses: { newAccount: newAccount.publicKey },
    signers: [],
    instructions: [
      import_web321.SystemProgram.createAccountWithSeed({
        fromPubkey: payer,
        basePubkey: payer,
      ...
      initTokenAccountInstruction({
        mint: new import_web321.PublicKey(TOKEN_WSOL.address),
...
var toAmmComputePoolInfo = (poolData) => {
  const data = {};
  const tokenProgramStr = import_spl_token14.TOKEN_PROGRAM_ID.toBase58();
...
var Market = class {
  ...
  static getAssociatedAuthority({ programId, marketId }) {
    ...
    return { publicKey: import_web323.PublicKey.default, nonce };
  }
};
...
var TradeV2 = class extends ModuleBase {
  ...
  async swap({
    ...
    const { account: sourceAcc, instructionParams: sourceAccInsParams } = await this.scope.account.getOrCreateTokenAccount({
      tokenProgram: amountIn.amount.token.isToken2022 ? import_spl_token16.TOKEN_2022_PROGRAM_ID : import_spl_token16.TOKEN_PROGRAM_ID,
      mint: inputMint,
    ...
    if (swapInfo.routeType === "route" && !isOutputSol) {
      destinationAcc = this.scope.account.getAssociatedTokenAccount(outputMint, amountOut.amount.token.isToken2022 ? import_spl_token16.TOKEN_2022_PROGRAM_ID : import_spl_token16.TOKEN_PROGRAM_ID);
    } else {
      const { account, instructionParams } = await this.scope.account.getOrCreateTokenAccount({
        tokenProgram: amountOut.amount.token.isToken2022 ? import_spl_token16.TOKEN_2022_PROGRAM_ID : import_spl_token16.TOKEN_PROGRAM_ID,
        mint: outputMint,
    ...
    if (swapInfo.routeType === "route") {
      ...
      routeTokenAcc = this.scope.account.getAssociatedTokenAccount(middleMint.mint, middleMint.isToken2022 ? import_spl_token16.TOKEN_2022_PROGRAM_ID : import_spl_token16.TOKEN_PROGRAM_ID);
    }
    ...
    if (swapInfo.feeConfig !== void 0) {
      ...
      checkTxBuilder.addInstruction({
        instructions: [
          (0, import_spl_token16.createTransferInstruction)(sourceAcc, swapInfo.feeConfig.feeAccount, this.scope.ownerPubKey, swapInfo.feeConfig.feeAmount.toNumber())
        ],
  ...
  getAllRoute({
    ...
    inputMint = inputMint.toString() === import_web325.PublicKey.default.toString() ? WSOLMint : inputMint;
    outputMint = outputMint.toString() === import_web325.PublicKey.default.toString() ? WSOLMint : outputMint;
    const needSimulate = {};
    const needTickArray = {};
    const cpmmPoolList = {};
    const directPath = [];
    ...
    for (const itemClmmPool of clmmPools ?? []) {
      ...
      if (itemClmmPool.mintA.equals(inputMint)) {
        ...
        if (routePathDict[t] === void 0)
          routePathDict[t] = {
            mintProgram: import_spl_token16.TOKEN_PROGRAM_ID,
            in: [],
            out: [],
            mDecimals: 0
          };
  ...
  async fetchSwapRoutesData({
    ...
    Object.values(ammSimulateCache).forEach((p) => {
      ...
      mintInfos[p.mintA.address] = {
        address: new import_web325.PublicKey(p.mintA.address),
        programId: import_spl_token16.TOKEN_PROGRAM_ID,
        mintAuthority: null,
        supply: BigInt(0),
      ...
      mintInfos[p.mintB.address] = {
        address: new import_web325.PublicKey(p.mintB.address),
        programId: import_spl_token16.TOKEN_PROGRAM_ID,
        mintAuthority: null,
        supply: BigInt(0),
    ...
    Object.values(cpmmPoolsRpcInfo).forEach((p) => {
      ...
      if (p.mintProgramA.equals(import_spl_token16.TOKEN_PROGRAM_ID)) {
        ...
        mintSet.add(mintA);
      if (p.mintProgramB.equals(import_spl_token16.TOKEN_PROGRAM_ID)) {
    ...
    const fetchMintInfoRes = await fetchMultipleMintInfos({
      connection: this.scope.connection,
      mints: Array.from(mintSet).map((m) => new import_web325.PublicKey(m))
    });
    mintInfos = {
      ...mintInfos,
      ...fetchMintInfoRes
  ...
  getAllRouteComputeAmountOut({
    ...
    const _amountInFee = feeConfig === void 0 ? new import_bn24.default(0) : inputTokenAmount.raw.mul(new import_bn24.default(feeConfig.feeBps.toNumber())).div(new import_bn24.default(1e4));
  ...
  async computePoolToPoolKeys({
    ...
    if (ammMarketFetchKeys.size > 0) {
      const marketAccount = await getMultipleAccountsInfoWithCustomFlags(this.scope.connection, Array.from(ammMarketFetchKeys).map((p) => ({ pubkey: new import_web325.PublicKey(p) })));
    ...
    pools.forEach((pool) => {
      if (pool.version === 6) {
        ...
        const ammKeys = {
          ...
          authority: getLiquidityAssociatedAuthority({ programId: new import_web325.PublicKey(pool.programId) }).publicKey.toString(),
        ...
        poolKeys.push({
          ...
          mintLp: toApiV3Token({
            ...
            programId: import_spl_token16.TOKEN_PROGRAM_ID.toBase58(),
