...
// src/raydium/utils1216/index.ts
var utils1216_exports = {};
__export(utils1216_exports, {
  default: () => Utils1216
});
module.exports = __toCommonJS(utils1216_exports);
...
var _Utils1216 = class extends ModuleBase {
  ...
  static getPdaOwnerId(programId, poolId, owner, version) {
    return findProgramAddress([
      ...
      poolId.toBuffer(),
      owner.toBuffer(),
      Buffer.from(new import_bn10.default(version).toArray())
    ], programId);
  }
  static async getAllInfo({
    ...
    for (let index = 0; index < pdaInfo.length; index++) {
      ...
      const hasCanClaimToken = itemOwnerInfo.tokenInfo.map((i2) => i2.debtAmount.gt(new import_bn10.default(0))).filter((i2) => !i2).length !== 3;
  ...
  async makeClaimAllTransaction({
    ...
    if (forecastTransactionSize(instructions, [wallet, ...signers.map((s) => s.publicKey)])) {
      ...
      return [
        {
          transaction: new import_web312.Transaction().add(...instructions.slice(0, txBuilder.AllTxData.instructions.length - 1)),
          signer: signers
        },
        {
          transaction: new import_web312.Transaction().add(...instructions.slice(txBuilder.AllTxData.instructions.length - 1)),
          signer: []
        },
        { transaction: new import_web312.Transaction().add(...txBuilder.AllTxData.endInstructions), signer: [] }
      ];
    }
  }
...
].map((i) => new import_web312.PublicKey(i)));
