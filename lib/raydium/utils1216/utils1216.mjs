...
import { PublicKey as PublicKey12, Transaction as Transaction3, TransactionInstruction as TransactionInstruction3 } from "@solana/web3.js";
...
var _Utils1216 = class extends ModuleBase {
  ...
  static getPdaOwnerId(programId, poolId, owner, version) {
    return findProgramAddress([
      ...
      poolId.toBuffer(),
      owner.toBuffer(),
      Buffer.from(new BN10(version).toArray())
    ], programId);
  }
  static async getAllInfo({
    ...
    for (let index = 0; index < pdaInfo.length; index++) {
      ...
      const hasCanClaimToken = itemOwnerInfo.tokenInfo.map((i2) => i2.debtAmount.gt(new BN10(0))).filter((i2) => !i2).length !== 3;
  ...
  async makeClaimAllTransaction({
    ...
    if (forecastTransactionSize(instructions, [wallet, ...signers.map((s) => s.publicKey)])) {
      ...
      return [
        {
          transaction: new Transaction3().add(...instructions.slice(0, txBuilder.AllTxData.instructions.length - 1)),
          signer: signers
        },
        {
          transaction: new Transaction3().add(...instructions.slice(txBuilder.AllTxData.instructions.length - 1)),
          signer: []
        },
        { transaction: new Transaction3().add(...txBuilder.AllTxData.endInstructions), signer: [] }
      ];
    }
  }
